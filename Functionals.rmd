...
While the pieces of `x` are always supplied as the first argument to `f`, you can override this, using R's regular function calling semantics, by supplying named arguments. For example, imagine you wanted to compute various trimmed means of the same data. While `trim` is, by default, the second argument of `mean()` (the data, `x`, is the first), we can make it the first argument by explicitly using the its argument name. Thus, the following two calls are equivalent:
