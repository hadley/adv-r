...
Chances are that you've already used a functional without knowing the name for it: the three most frequently used are `lapply()`, `apply()` and `tapply()`. All three take a function as input (among other things) and return a vector as output.

A common use of functionals is as an alternative to for loops. For loops have a bad rap in R. They have a reputation for being slow (that reputation is only partly true, see [modification in place](#modification) for more details). But the real downside of for loops is that they're not very expressive. A for loop conveys that it's iterating over something, but does't clearly convey the high level goal. Instead of using a for loop, it's better to use a functional. Each functional is tailored for a specific task, so when you recognise the functional you know immediately why it's being used. Functionals play other roles as well as replacements for for loops. They are useful for encapsulating common data manipulation tasks like split-apply-combine, for thinking "functionally", and for working with mathematical functions. 

Functionals reduce bugs in your code by better communicating intent. They're also well tested (i.e., bug-free) and implemented efficiently because they're used by so many people. Many functionals in base R are written in C, and often use tricks to enhance performance. That said, using the code in this chapter will not always produce the fastest code. The focus here is on clearly communicating with your code and building tools that solve a wide range of problems. It's a mistake to focus on speed until you know it'll be a problem. Once you have clear, correct code you can make it fast using the techniques you'll learn in [improving the speed of your code](#profiling).
...
* [For loop functionals](#functionals-loop) shows you variants of `lapply()` 
  that produce different outputs, take different inputs and distribution 
  computation in different way.

* [Data structure functionals](#functionals-ds) discusses functionals that 
  work with richer data strutures like matrices and arrays.

* [Functional programming](#functionals-fp) teaches you about the powerful
  `Map()`, `Reduce()` and `Filter()` functions which are common in other
  FP languages.

* [Mathematical functionals](#functionals-math) translates in R functionals you 
...
* [Loops that shouldn't be converted to functions](#functionals-not) provides
  some important caveats about when you shouldn't attemp to convert a loop into
  a functional.
...
The code in this chapter only uses built in packages. 

You'll closure them frequently used in conjunction with functionals. If you're not familiar with them, make sure to review [closures](#closures).
...
The pieces of `x` are always supplied as the first argument to `f`. What if you want to vary a different argument? You can use an anonymous function. The following example varies the amount of trimming applied when computing the mean of a fixed `x`.
...
The first form is usually not a good choice for a for loop because it leads to inefficient way of saving output. With this form it's very natural to save the output by extending an a datastructure, like in this example:
...
This is slow because each time you extend the vector, R has to copy all of the existing elements. [Avoid copies](#avoid-copies) discusses this problem in more depth.Instead, it's much better to create the space you'll need for the output and then fill it in. This is easiest with the second form:
...
1.  Use both for loop and `lapply` techniques to fit linear models to the
    `mtcars` using the formulas stored in the list below.
...
1.  Fit the model `mpg ~ disp` to each of the bootstrap replicates of `mtcars` 
    in the list below by using for loop and `lapply()`, respectively. Can you 
    do so without an anonymous function?
...
One interesting thing about the implementation of `lapply()` is that because each iteration is isolated from all others, the order in which they are computed doesn't matter. For example, while `lapply3()`, defined below, scrambles the order of computation, the results are always the same:
...
In this case, `mclapply()` is actually slower than `lapply()`. This is because the cost of the individual computations is low, and additional work is needed to send the computation to the different cores and to collect the results. These functions are not available in Windows, but you can use the similar `parLapply()` with a bit more work. See [parallelise](#parallelise) for more details.
...
1.  The following code simulates the performance of a t-test for non-normal 
    ...
    Extra challenge: get rid of the anonymous function and use 
    the `'[['` function.
...
## Data structure functionals {#functionals-ds}
...
* functionals which summarise a vector by groups defined by values in another 
  vector: `tapply()`,

* the `plyr` package: a generalisation of the ideas behind `tapply()` which 
  allow you to work with data frames, lists or arrays as inputs, and data 
...
Being able to rewrite `tapply()` as a combination of `split()` and `sapply()` is a good indication that we've identified some useful primitives that we can recombine to solve new problems.
...
1.  Why isn't `is.na()` a predicate function? What base R function is closest
    to be a predicate version of `is.na()`?
...
In this section we'll explore some of R's built-in mathematical functionals. There are three functionals that work with functions to return single numeric values:
...
We can use this function factory to generate specific NLL functions for input data. Then `optimise()` allows use to find the best values (the maximum likelihood estimates), given a generous starting range.
...
## Loops that shouldn't be converted to functionals {#functionals-not}

Some for loops have no natural functional equivalent. In this section you'll learn about three common cases:

* modifying in place
* recursive functions
* while loops

It's possible to torture these the problems to use an existing functional, but it's not a good idea. You'll create code that is harder to understand, eliminating the main reason for using functionals in the first case.
...
We wouldn't normally use `lapply()` to replace this loop directly, but it is _possible_. Just replace the loop with `lapply()` by using `<<-`:
...
The for loop is gone, but the code is longer and much harder to understand. The reader needs to understand `<<-` and what `x[[y]] <<- z` does behind the scenes. In short, we've taken a simple, easily understood for loop, and turned it into something few people will understand: not a good idea!

### Recursive relationships

It's hard to a convert for loop into a functional when the relationship between elements is not independent, or is defined recursively. For example, exponential smoothing works by taking a weighted average of the current and previous data points. The `exps()` function below implements exponential smoothing with a for loop.
...
One way to eliminate the for loop in this cases is to [solve the recurrence relation](http://en.wikipedia.org/wiki/Recurrence_relation#Solving) by removing the recursion and replacing it with explicit references. This requires a new set of mathematical tools, and is challenging, but it can pay off by producing a simpler function. For exponential smoothing, it's possible to rewrite the function in terms of `i`:
...
It's arguable whether or not this is more understandable here, but it may prove useful for your specific problem.

### While loops

Another type of looping construct in R is the `while` loop. It keeps running until some condition is met. `while` loops are more general than `for` loops: you can rewrite every for loop as a while loop, but you can't do the reverse.  For example, we could turn the for loop below into a while loop:
...
Now that we have the basics working, lets extend the function to deal with more complicated inputs. One obvious generalisation is to add more than two numbers. We can do this by iteratively adding two numbers: if the input is `c(1, 2, 3)` we compute `add(add(1, 2), 3)`. This is a simple application of `Reduce()`:
...
```{r}
row_sum <- function(x, na.rm = FALSE) apply(x, 1, add, na.rm = na.rm)
col_sum <- function(x, na.rm = FALSE) apply(x, 2, add, na.rm = na.rm)
arr_sum <- function(x, dim, na.rm = FALSE) apply(x, dim, add, na.rm = na.rm)
```
...
* Since all variants were implemented by combining a simple binary operator 
  (`add()`) and a well-tested functional (`Reduce()`, `Map()` and `apply()`), 
...
The downside of this approach is that these implementations are unlikely to be efficient. (For example, `colSums(x)` is much faster than `apply(x, 2, sum)`.) However, even if they don't turn out to be fast, simple implementations are still a good starting point because they're less likely to have bugs. When you create faster versions, you can compare the results to make sure your fast versions are still correct.
...
1.  Create a table that has _and_, _or_, _add_, _multiply_, _smaller_, and 
    ...
    * Fill in the cells with the names of base R functions that perform each of
      the roles.
...
1.  How does `paste()` fit into this structure? What is the scalar binary 
    ...
    arguments to `paste()` equivalent to? Are there are any `paste` variants 
