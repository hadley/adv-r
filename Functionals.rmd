...
In statistics, optimisation is often used for maximum likelihood estimation. Maximum likelihood estimation (MLE) is a natural fit for functional programming because we have a well defined problem and a general technique to solve it. In MLE, we have two sets of parameters: the data, which is fixed for a given problem, and the parameters, which vary as we try to find the maximum. Because we have two layers of parameters, the problem calls for closures. Closures plus optimisation gives rise to the following approach to solving MLE problems.
...
Second, with the general NLL in hand, we create two specific NLL functions for two datasets, and use `optimise()` to find the best values, given a generous starting range.
...
We can verify these values are correct by using the analytic solution: in this case, it's just the mean of the data, 32.1 and 5.45.
...
There are a wide class of for loops that do not naturally match the functionals we've described. While it's possible to torture your code to make it work, it's usually not worth the effort: while for loops are verbose and not very expressive, R programmers are, nonetheless, familiar with them. 

It takes a while before you can identify whether or not a for loop has a related vectorised solution, or a matching functional. It's also easy to go too far: converting for loops that really should stay as loops. This section provides resources for learning more about this problem, and highlights three types of loop that you shouldn't try and convert into a functional.
...
You can also look for more current posts on this topic with a [search](http://stackoverflow.com/search?tab=votes&q=%5br%5d%20for%20loop).
...
We've eliminated the for loop, but our code is longer and we've had to use an unusual language feature, `<<-`. Also, to understand what `mtcars[[var]] <<- ...` does, you have to understand not only how `<<-` works, but also what `x[[y]] <<- z` does behind the scenes. In short, we've taken a simple, easily understood for loop, and turned it into something few people will understand: not a good idea!

### Recursive relationships

Another case where it's hard to convert a for loop into a functional is when the relationship is defined recursively. For example, exponential smoothing works by taking a weighted average of the current and previous data points. The `exps()` function below implements exponential smoothing with a for loop.
...
This is only worthwhile if it's a frequently needed function. Otherwise it just places an additional cognitive burden on the reader.

Another way to eliminate the for loop in these cases is to [solve the recurrence relation](http://en.wikipedia.org/wiki/Recurrence_relation#Solving) by removing the recursion and replacing it with explicit references. This requires a new set of tools, and is mathematically challenging, but it can pay off by producing a simpler function. For exponential smoothing, it's possible to rewrite the function in terms of `i`:
...
It's arguable whether or not this is more understandable for this case, but it may prove useful for your specific problem.
...
However, not every while loop can be turned into a for loop. This is because many while loops don't know in advance how many times they will be run:
...
This is a common situation when you're writing simulations and one of the random parameters represents how many times a process occurs.

In some cases, like above, you may be able to remove the loop by recognising some special feature of the problem. For example, the above problem is counting how many times a Bernoulli trial with p = 0.1 is run before a success is observed. Note that since this is a geometric random variable, you could replace the above code with `i <- rgeom(1, 0.1)`. Similar to solving recurrence relations, this is generally extremely difficult to do. But if you can get it to work for your problem, you'll benefit greatly.

## A family of functions

The following case study shows how you can use functionals to start small, with very simple functions, and then build up more complicated and feature-filled functions. We'll start with a simple idea, adding two numbers together, and then extend it to summing multiple numbers, computing parallel and cumulative sums, and summing across array dimensions. While we're applying this idea to addition, the important thing to realise is that you can also apply this idea to multiplication, to finding the smaller or larger value, to string concatenation as a way to generate a wide variety of families of functions, including the 20+ functions provided in base R.
...
This allows us to write a version of `add` that can deal with missing values if needed (and it often is!):
...
Why did we pick an identity of `0`? Why should `add(NA, NA, na.rm = TRUE)` return 0? Well, for every other input it returns a numeric vector of length 1, so even if both arguments are missing values, it should do at least that. We next need to figure out what that number should be. To do that, we can use the fact that addition is associative. This means that the order in which quantities are added doesn't matter. Thus, the following two function calls should return the same value:
...
This implies that `add(NA, NA, na.rm = TRUE)` must be 0.

Now that we have the basics working, we can extend our function to deal with more complicated inputs. The first thing we might want to do is to add more than two numbers. For example, if the input is `c(1, 2, 3)` we want to compute `add(add(1, 2), 3)`. We could do this as a simple application of `Reduce`:
...
The two problems are related. If we give `Reduce()` a length one vector, it doesn't have anything to reduce, so it just returns the input. If we give it an input of length zero, it always returns `NULL`. There are two, equivalent ways to fix this. We can concatenate `0` to every input vector, or we can use the `init` argument in `Reduce()`. Here's the latter:
...
It would also be nice to have a vectorised version of `add` so that we can perform the addition of two vectors of numbers in element-wise fashion. We could use `Map()` or `vapply()` to implement this, but neither is perfect. `Map()` returns a list (we want a numeric vector). `vapply()` returns a vector but it requires us to loop over a set of indices.

A few test cases can help to ensure that it behaves as we expect. We're a bit stricter than base R here because we don't do recycling. (You could add that if you wanted, but I find that recycling is a frequent source of silent bugs.)
...
(While all of these behaviors are captured in R's implementation, `+`, our's gives greater control over missing values.)

Another variant on `add()` would be the cumulative sum. We can create a reductive version, which shows every intermediate step along the way to the final result, by using `Reduce()` and setting the `accumulate` argument to `TRUE`:
...
Finally, we might want to define versions for more complicated data structures like matrices. We could create `row` and `col` variants that sum across rows and columns respectively, or we could go the whole hog and define an array version that could sum across any arbitrary set of dimensions. These are easily implemented as combinations of `add()` and `apply()`.
...
The first two are equivalents to `rowSums()` and `colSums()`.

If every function we have created has an existing equivalent in base R, why did we bother? There are two main reasons:

* Having created all our variants using a very simple binary operator (`add`) and a well-tested functional (`Reduce`, `Map` and `apply`), we know that our variants will behave consistently.

* Having seen the infrastructure for addition, we can now apply the strategy to other operators, especially those that might not have the full suite of variants in base R.

The downside of this approach is that these implementations are unlikely to be efficient. (For example, `colSums(x)` is much faster than `apply(x, 2, sum)`.) However, even if they don't turn out to be fast enough, simple implementations are still a good starting point. Their simplicity makes them less likely to have bugs and more likely to return correct results. When you create faster versions (maybe using Rcpp, as in [Rcpp](#rcpp)), you can compare the results to make sure your fast versions are still correct.
...
* Create a table that has _and_, _or_, _add_, _multiply_, _smaller_, and _larger_ in the columns and _binary operator_, _reducing variant_, _vectorised variant_, and _array variants_ in the rows.
