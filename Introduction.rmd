...
* A strong foundation in functional programming. The ideas of functional
  ...
  you write concise yet descriptive code.
...
Reading source code is a tremendously useful because it exposes you to new ways of doing things. Over time you'll develop a sense of taste as an R programmer, and even if you find something that violates that sensibility, that exposure will still be helpful: you'll emulate the things you like and avoid the things you don't. I think the clarity of my code increased considerably once I started grading code in the classroom, and was exposed to a lot of code I couldn't make heads nor tails of! I think it's a great idea to start by reading the source code for the functions and packages that you use most frequently. This becomes particularly important when you start using more esoteric parts of R. Often the documentation will be lacking, so the only way you'll need to figure out how a function works is by reading the source and experimenting.
...
Throughout this book I use `f()` to refer to functions, `g` to return to function parameters, and `h/` to paths. Larger code blocks intermingle input and output. Output is placed in comments in the hope that you'll have access to an electronic version of this book where you can copy and paste. Output comments look like `#>` to distinguish them regular comments.
