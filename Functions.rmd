...
The only package you'll need is `pryr`, which is used to explore what happens when modifying vectors in place. Install it with `require(devtools); install_github("pryr")`.
...
As well as returning a value, functions can set up other triggers to occur when the function is finished using `on.exit()`. This is often used as a way to guarantee that changes to the global state are restored when the function exists. The code in `on.exit()` is run regardless of how the function exits, whether with an explicit (early) return, an error, or simply reaching the end of the function body.
...
1.  We can use `on.exit()` to implement a simple version of `capture.output()`.
    ...
    Compare `capture.output()` to `capture.output2()` and think about the simplifications I've made. Is the code easier to understand 
    or harder? What features have I removed to make the key ideas easier to see? 
