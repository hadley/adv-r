...
Primitive functions are only found in the `base` package, and since they operate at a low level, they can be more efficient (primitive replacement functions don't have to make copies), and can have different rules for argument matching (e.g. `switch` and `call`).  This, however, comes at a cost of behaving differently to all other functions in R, and so R core generally avoids creating them unless there is no other option.
