...
Sometimes you want to add a non-trivial default value, which might take several lines of code to compute. Instead of insert that code in the function definition, you could use `missing()` to conditionally compute it if needed. However, this makes it hard to which arguments are required and which are optional without carefully reading the documentation. Instead, I usually set the default value to `NULL` and use `is.null()` to check if the argument was supplied.
