...
* [Every operation in a function call](#all-calls) shows you that everything
...
The only package you'll need is `pryr`, which is used to explore what happens when modifying vectors in place. Install it with `devtools::install_packages("hadley/github")`.
...
There is one exception to the rule that functions have three components. Primitive functions, like `sum`, call C code directly with `.Primitive()` and contain no R code. Therefore their `formals()`, `body()`, and `environment()` are all `NULL`: \indexc{.Primitive()} \index{Functions!Primitive}
...
Scoping is the set of rules that govern how R looks up the value of a symbol. In the example below, scoping is the set of rules that R applies to go from the symbol `x` to its value `10`: \index{Scoping!Lexical} \index{Lexical scoping}
...
That everything in R is represented as a function call is important to know for [metaprogramming](#metaprogramming).
...
Function arguments in R can have default values. \index{Functions!Arguments!default values}
