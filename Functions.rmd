...
2. Create a list of all primitive functions in R. (Hint: use `ls("package:base", all = TRUE)` to get a list of all objects in the base package and `get()` to retrieve an object given its name.)
...
* build tools by composing functions, as described in [functional programming](#functional-programming).

* overrule the usual evaluation rules and do metaprogramming, as described in [metaprogramming](#metaprogramming).
...
Sometimes you want to add a non-trivial default value, which might take several lines of code to compute. Instead of insert that code in the function definition, you could use `missing()` to conditionally compute it if needed. However, this makes it hard to know which arguments are required and which are optional without carefully reading the documentation. Instead, I usually set the default value to `NULL` and use `is.null()` to check if the argument was supplied.
...
* the expression which gives rise to the delayed computation. (It can be accessed with `substitute`. See [non-standard evaluation](#non-standard-evaluation) for more details)
