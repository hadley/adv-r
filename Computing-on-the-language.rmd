...
Before we can explore `eval()`, we need one more useful function: `quote()`. It captures an unevaluated expression like `substitute()`, but doesn't have do any of the advanced transformations that can make `substitute()` confusing. `quote()` always returns its input as is:
