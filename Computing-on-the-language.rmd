...
The following chapters ([expressions](#expressions) and [special environments](#special-environments)), expand on these ideas, discussing the underlying data structures and how you can understand and manipulate them to create new tools.
...
We won't worry yet about exactly what sort of object `substitute()` returns (that's the topic of the [Expressions](#expressions) chapter), but we'll call it an expression.  (Note that it's not the same thing as returned by the `expression()` function: we'll call that an expression _object_.)
...
* Write an alternative to `<-` that never overrides an existing binding (refer back to [bindings](#binding) for more details). This would be useful if you are running a test script multiple times and only want to generate the test data once.
...
nrow(subset2(mtcars, cyl == 4))
nrow(subset2(mtcars, cyl == y))
nrow(subset2(mtcars, cyl == x))
nrow(subset2(mtcars, cyl == condition))
nrow(subset2(mtcars, cyl == condition_call))
```
...
subset3(mtcars, quote(cyl == 4))
subset3(mtcars, cyl == 4)
```
...
Again, we can turn to substitute and use it for another purpose:  modifying expressions.  So far we've just used `substitute()` to capture the unevaluated expression associated with arguments, but it can actually do much much more, and is very useful for manipulating expressions in general.
...
And you can use substitute to insert any arbitrary object into an expression. This is technically ok, but often results in surprising and undesirable behaviour.  In the example below, the expression we create doesn't print correctly, but it returns the correct result when we evaluate it:
...
* If the underlying values are the same, the results will be the same. i.e. the three results will all be the same, regadless of what `f` is:
...
There are a number of other R functions that use `substitute()` and `deparse()` in this way: `ls()`, `rm()`, `data()`, `demo()`, `example()`, `vignette()`. These all use non-standard evaluation and then have special ways of enforcing the usual rules. To me, eliminating two quotes is not worth the cognitive cost of non-standard evaluation, and I don't recommend you use `substitute()` for this purpose.
