...
`[` and `[[` differ slightly in their behaviour when the index is out of bounds (OOB), for example, when you try and try to extract the fifth element of a length four vector, or subset a vector with `NA` or `NULL`:
