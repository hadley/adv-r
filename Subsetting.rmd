...
R's subsetting operators are powerful and fast, and mastering them allows you to express complex operations in a succinct fashion that few other languages can match. Such operations are a natural complement to `str()`: `str()` shows you the structure of any object, and subsetting allows you to pull out the pieces that you're interested in.
...
1.  When would you use `mtcars[] <- x` instead of `mtcars <- x`?
...
There are five types of thing that you can subset any vector with:
...
*   __Nothing__ returns the original vector unchanged. This is not useful 
...
Subsetting a list works in the same way as subsetting an atomic vector. Subsetting a list with `[` will always return a list: `[[` and `$`, as described below, let you pull out the components of the list.

### Matrices and arrays {#matrix-subsetting}

You can subset higher-dimensional structures in three ways, with:

* Multiple vectors.
* A single vector.
* A matrix.

The most common way of subsetting matrices (2d) and arrays (>2d) is a simple generalisation of 1d subsetting: you supply a 1d index for each dimension, separated by a comma. Blank subsetting is now useful because you use it when you want to return all the rows or all the columns.
...
By default, `[` will simplify the results to the lowest possible dimensionality. See [simplifying vs. preserving](#simplify-preserve) subsetting if you want to avoid this.
...
There are two other subsetting operators apart from `[`: `[[` and `$`. `[[` is similar to `[`, except it can only return a single value and it allows you to pull pieces out of a list. `$` is a useful shorthand for `[[` combined with character subsetting.

You need `[[` when working with lists. This is because when applied to a list `[` always returns a list: it never gives you the contents of the list. To get the contents, you need`[[`:
...
If you want to avoid this behaviour you can set `options(warnPartialMatchDollar = TRUE)`. NB: this is a global option and may affect behaviour in other code you have loaded (e.g. packages).
...
1.  Positive integers select elements at specify positions, negative integers
...
1.  If `x` is a list, `mtcars[] <- x` will keep `mtcars` as a data frame, 
    replacing each column in `mtcars` with the corresponding element of `x`.
