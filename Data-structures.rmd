...
The basic data structure in R is the vector. Vectors comes in two flavours: atomic vectors and lists. They have three common properties: 
...
There are four common types of atomic vector that I'll discuss in detail: logical, integer, double (often called numeric), and character. There are two rare types that I will not discuss further: complex and raw. 
...
Missing values are specified with `NA`, which is a logical vector of length 1. `NA` will always be coerced to the correct type if used inside `c()`, or you can create NA's of a specific types with `NA_real_` (a double vector), `NA_integer_` and `NA_character_`.
...
Names don't have to be unique. However, character subsetting, [subsetting](#lookup-tables), is most important reason to use names and it is most useful when the names are unique.
