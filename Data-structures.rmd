...
Lists are used to build up many of the more complicated data structures in R: both data frames (described below), and linear models objects (as produced by `lm()`) are lists:
...
Vectors are not the only 1d dimensional datastructure. You can have matrices with a single row or single column, or arrays with a single dimension. They may print similarly, but will behave differently. The differences aren't too important, but it's useful to know they exist in case you get strange output from a function. As always, use `str()` to reveal the differences.
