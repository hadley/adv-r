...
Almost all other objects in R are built upon these foundations, and in the [OO field guide](#oo-field-guide) you'll see how R's object oriented tools build on top of these basics. There are also a few types of more esoteric objects that I don't describe here, but you'll learn about in depth in other parts of the book:
...
A factor is a vector that can contain only predefined values, and is R's structure for dealing with qualitative data. A factor is not a basic vector, but is built on top of an integer vector using the S3 class system, as described in the [OO field guide](#oo-essentials). Factors have two key attributes: their `class()`, "factor", which controls their behaviour; and their `levels()`, the set of allowed values.
...
A vector gains multi-dimensional behavior if it has a `dim()` attribute. The terminology can be a little confusing but generally, we call `x`:

* a vector if `length(dim(x))` is 0, or
* a __array__ if `length(dim(x))` is greater than 0.

A __matrix__ is a special case of an array where `length(dim(x))` is 2. Matrices and arrays can be created using the `matrix()` and `array()` functions, or by using the replacement form of `dim()`:
...
`c()` generalises to `cbind()` and `rbind()` for matrices, and to `abind()` (available in the `abind` package) for arrays.
...
NB: vectors are not the only 1d dimensional datastructure. You can have matrices with a single row or single column, or arrays with a single dimension. They may print similarly, but will behave differently. The differences aren't too important, but it's useful to know they exist in case you get strange output from a function. As always, use `str()` to reveal the differences.
