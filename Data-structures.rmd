...
Not all elements of a vector need to have a name. If some names are not specified when creating a vector using `c()`, `names()` will return an empty string for those elements. Names not set using `names()` or `setNames()` will result in a `NA` value. If all names are missing, `names()` will return `NULL`.
