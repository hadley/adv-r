...
Tests like `is.vector`, `is.numeric`, and `is.atomic` have broader definitions than tests for specific types.  If `is.list(x)==T`, then `typeof(x)=="list"` and if `is.integer(x)==T` then `type.of()=="integer"` but that’s not true of `is.vector` or `is.numeric`.
...
Vectors are not the only 1-dimensional data structure. You can have matrices with a single row or single column, or arrays with a single dimension. They may print similarly, but will behave differently. The differences aren't too important, but it's useful to know they exist in case you get strange output from a function. As always, use `str()` to reveal the differences.
