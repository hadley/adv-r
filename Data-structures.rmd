...
A factor is a vector that can contain only predefined values. It is R's structure for dealing with qualitative data. A factor is not an atomic vector, but it's built on top of an integer vector using with an S3 class (as described in the [OO field guide](#oo-essentials)). Factors have two key attributes: their `class()`, "factor", which controls their behaviour; and their `levels()`, the set of allowed values.
