...
R represents missing or unknown values with a special sentinel value: `NA` (short for not applicable). Missing values tend to be infectious: most computations involving a missing value will return another missing value.
...
NB: Technically there are four missing values[^NA_complex], one for each of the atomic types: `NA` (logical), `NA_integer_` (integer), `NA_real_` (double), and `NA_character_` (character). This distinction is usually unimportant because `NA` will be automatically coerced to the correct type when needed.

[^NA_complex]: Well, technically, five if we include `NA_complex_`, but we ignore this "rare" type, as mentioned above.
...
You can think of attributes as name&ndash;value pairs[^pairlist] that attach metadata to an object. Individual attributes can be retrieved and modified with `attr()`, or retrieved en masse with `attributes()`, and set en masse with `structure()`.
...
* When you attempt to extract a single column with `df$x` and there is no 
  ...
  exist[^warnPartialMatch].
  
[^warnPartialMatch]: We can make R warn about this behavior (called partial matching) by setting `options(warnPartialMatchDollar = TRUE)`.
