...
[^node]: Collectively, all the other data types are known as "node" data, which include things like functions and environments. You're most likely to come across this highly technical term when using `gc()`: the "N" in `Ncells` stands for nodes and the "V" in `Vcells` stands for vectors.
...
* Doubles can be specified in decimal (`0.1234`), scientific (`1.23e4`), or 
  ...
  defined by the floating point standard. The double missing value is `NA_real_`.
...
[^mode]: You may have heard of the related `mode()` and `storage.mode()` functions. Do not use them: they exist only for S compatibility. 
