...
Two of the most important S3 vectors built on top of lists are data frames and tibbles. 

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/vectors/summary-tree-s3-2.png")
```

A data frame is the most common way of storing data in R, and is crucial for effective data analysis. A data frame is a named list of equal-length vectors. It has attributes for (column) `names`, `row.names`[^rownames], and its class, "data.frame": 
...
Because each element of a list has the same length, data frames have a rectangular structure, and hence shares properties of both the matrix and the list:
...
*   To represent an empty vector (a vector of length 0) of arbitrary type.
    ...
    `NULL`, and concatenating `NULL` to a vector leaves it unchanged:
