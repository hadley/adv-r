...
Data frames are one of the biggest and most powerful ideas of R, and one of the things that makes R different from other programming languages. However, in the over 20 years since their creation, the ways people use R have changed, and some of the design decision that made sense at the time now cause frustration.

This frustration lead to the creation of the tibble, a modern reimagining of the data frame. Tibbles have been designed to be (as much as possible) drop-in replacements for data frames, while still fixing the greatest frustrations. A concise (but fun) way to summarise the main difference is that tibbles are lazy and surly: they tend to do less and complain more. 
...
Data frames allow you to optionally label each row with a "name":
...
You can get and set with `rownames()`, and use to subset rows:

```{r}
df3["Bob", ]
```

Tibbles do not support row names for three reasons:

*  Storing metadata in a different way to data is fundamentally a bad idea. 
   You have to learn different functions to get and set row names rather than
   using the tools you already know to get and set other variables.

*  Row names are poor abstraction for labelling rows because it requires that 
   there be a single string that identifies a row; this is only true in a
   minority of cases. Many times labelling a row is most naturally done with
   multiple variables, or with non-character variables.
   
*  Rownames must be unique, so any replication of rows (e.g. during 
   bootstrapping) will cause row names to be changed. If you want to match 
   rows from before and after the transformation you'll need to perform
...
The tibble package provides `rownames_to_column()`, and a `rownames` argument to `as_tibble()`:
...
* Each column is labelled with its type (abbreviated).

* Wide columns are truncated to avoid a single long string occupying an entire
  row. (This is still a work in progress: it's tricky to the get tradeoff 
  right between showing many columns and showing one column fully).
...
Tibbles tweak these behaviours so that `[` always returns a tibble, and `$` doesn't partial match, and warns if it can't find a variable.
...
A tibble's insistence on returning a data frame from `[` can cause problems with legacy code, which often uses `df[, "col"]` to extract a single column. To fix this, switch `df[["col"]]`, this is more expressive (since `[[` always extracts a single element) and works with both data frames and tibbles.
...
Typically, it should not matter if you have a tibble or data frame, but if you do need to distinguish, use `is.tibble()`:
...
Since a data frame is a list of vectors, it is possible for a data frame to have a column that is a list. This is a powerful technique because a list can contain any other R object, so you can have a column of data frames or models. This allows you to keep related objects together in a row, no matter how complex the individual objects are. You can see an application of this in the Many Models chapter of "R for Data Sicence", <http://r4ds.had.co.nz/many-models.html>.
...
To finish up the chapter, I wanted to talk about a final important data struture that's closely related to vectors: `NULL`. It's a little special: it has type `NULL`,  length 0, and can not have attributes:
...
You can test for it with `is.null()`:
...
`NULL` is often used to represent a vector of length 0. For example, if you use `c()` but don't include any arguments, you get `NULL`, and concatenating `NULL` to a vector leaves it unchange[^identity]:
...
`NULL` is often used as a way to explicitly say that an entire vector is absent. Contrast this with `NA` which is used to indicate that an _element_ of a vector is absent. For example, `NULL` is often used as a default function argument, when the argument is optional but the default value requires some computation.
