...
bench_show <- function(x) setNames(as.list(x$min), x$expression)
```
...
* Section \@ref(lang-vs-impl) introduces the distinction between R-the-language,
  vs R-the-implementation. This distinction helps us to understand some of the
  reasons behind R's performance.
  
* Section \@ref(microbenchmarking) introduces the microbenchmark with the 
  bench package. This chapter will use microbenchmarks to concretely explore 
  performance.
  
* Section \@ref(language-performance) discusses four reasons why R-the-language
  is slow.

* Section \@ref(implementation-performance) disuccses two reasons why 
  R-the-implementation is slow.

### Prerequisites {-}

I'll show examples that make use of basic subsetting and scoping concepts, so make sure that you're familiar with contents of Chapter \@ref(subsetting) and Chapter \@ref(functions).

## Language vs implementation {#lang-vs-impl}

To understand R's performance, it helps to think about R as both a language and as an implementation of that language. The R-language is abstract: it defines what R code means and how it should work. The implementation is concrete: it reads R code and computes a result. The most popular implementation is the one from [r-project.org](http://r-project.org). I'll call that implementation __GNU-R__ to distinguish it from R-language, and from the other implementations I'll discuss later in the chapter. \index{language definition}
...
* `n_itr` and `total_time` tells you how many times the expression was 
  ...
  be greater than the `min_time` paramter.
...
```{r bench-dispatch, dependson = "oo-setup"}
(lb <- bench::mark(
  fun = f(),
  S3 = s3(1L),
  S4 = s4(a)
...
```{r, dependson = "bench-dispatch", include = FALSE}
dyn <- bench_show(lb)
```

The bare function takes about `r ns(dyn$fun)`. S3 method dispatch takes an additional `r ns(dyn$S3 - dyn$fun)` and S4 dispatch an additional `r ns(dyn$S4 - dyn$fun)`. S3 and S4 method dispatch are expensive because R must search for the right method every time the generic is called; it might have changed between this call and the last. R could do better by caching methods between calls, but caching is hard to do correctly and a notorious source of bugs.
...
```{r bench-env}
env_random <- function(parent = global_env()) {
  letter_list <- set_names(as.list(runif(26)), LETTERS)
  as_environment(letter_list, parent)
}

f2 <- set_env(f, env_random())
f3 <- set_env(f, env_random(get_env(f2)))
f4 <- set_env(f, env_random(get_env(f3)))
...
1.  Read _"Evaluating the Design of the R Language"_ [@r-design]. What other 
    aspects of the R-language slow it down? Construct microbenchmarks to 
    illustrate. 
...
x <- runif(100, -1.5, 1.5)
(lb <- bench::mark(
  ife      = squish_ife(x, -1, 1),
  p        = squish_p(x, -1, 1),
  in_place = squish_in_place(x, -1, 1)
))
```

```{r, dependson = "bench-squish", include = FALSE}
squish <- bench_show(lb)
```

Using `pmin()` and `pmax()` is about `r num(squish$ife / squish$p)`x faster than `ifelse()`, and using subsetting directly is about `r num(squish$p / squish$in_place)`x as fast again.
...
* [pqR](http://www.pqr-project.org/) (pretty quick R) by Radford Neal. Built 
  ...
  better memory management (including a faster garbage collector). It also 
  provides some small extensions to the R language.
...
* [FastR](https://github.com/oracle/fastr) by a team from Oracle and Purdue. 
  FastR is similar to Renjin, but is built on the GraalVM, which makes it 
  possible to interoperate with other langugaes.
...
* [Riposte](https://github.com/jtalbot/riposte) by Justin Talbot and
  ...
  _"Riposte: A Trace-Driven Compiler and Parallel VM for Vector Code"_
  [@riposte].
...
However, if we rewrote the function using a loop in a language like C++, we only need one intermediate value: the sum of all the values we've seen. Even if you've never used C++, you should still be able to follow the basic strategy, and you'll more learn more in Chapter \@ref(rcpp).

```{r cond-sum-cpp, engine = "Rcpp"}
#include <Rcpp.h>
...
```{r bench-cond-sum, dependson = "cond-sum-cpp"}
cond_sum_r <- function(x, y, z) {
  sum((x + y)[z])
}

(lb <- bench::mark(
  sum_cpp = cond_sum_cpp(x, y, z),
  sum_r = cond_sum_r(x, y, z)
))
```

```{r, dependson = "bench-cond-sum", include = FALSE}
cond <- bench_show(lb)
```

On my computer, this approach is about `r num(cond$sum_r / cond$sum_cpp)`x faster than the vectorised R equivalent, which is already pretty fast.
