...
S4 implements functional OOP, like S3, but is much more rigorous and strict. There are three main differences between S3 and S4:

* S4 classes are defined formally with a call to `setClass()`.
  An S4 class can have multiple parents (multiple inheritance).
  
* The fields of an S4 object called __slots__ and are accessed with the 
  special `@` operator.
  
* Methods are defined by a call to `setMethod()`, instead of using a naming
  convention. S4 generics can dispatch on multiple arguments (multiple dispatch).

S4 is a rich system and it's not possible to cover all of it in one chapter. Like the other OO chapters, the focus here will be on S4 works, not how to deploy it most effectively.
...
Unfortunately there is not one good reference for S4, which is one of the major downsides to the system. As you move towards more advanced usage, you will need to piece together needed information by carefully reading the documentation and performing experiments. Some good places to start are:
...
Where there is conflict between the documentation and advice given by Bioconductor, follow the advice from Bioconductor.

### Outline {-}

### Prerequisites {-}

All funtions related to S4  live in the methods package. This package is always available when you're running R interactively, but may not be available when running R in batch mode (i.e. from `Rrcript`). For this reason, it's a good idea to call `library(methods)` whenever you use S4. This also signals to the reader that you'll be using the S4 object system.
...
We'll start with a quick overview of the main components of S4 and then dive into the details below. You define a S4 class by calling `setClass()` with the class name along with its slots and their classes.
...
You can construct new objects from this class by calling `new()`:

```{r}
john <- new("Person", name = "John Smith", age = NA_real_)
```

Most classes should be accompanied by a user-friendly helper (Section \@ref(s4-helper)) and a validator (Section \@ref(s4-validator)).

Slots can be listed with `slotNames()` and accessed with `@`. You can access the slots with `@` or `slot()`: `@` is equivalent to `$`, and `slot()` to `[[`. 
...
Slots should be considered an internal implementation detail. That means as a user, you should not reach into someone else's object with `@`, but instead look for a function that provides the information you want. As a developer, you should make sure that all public facing slots have their own accessor functions (Section \@ref(accessors)). Commonly, your accessor function will be an S4 generic so many different classes can have the same external interface.

You create generics with `setGeneric()`:
...
And then define methods with `setMethod()`:
...
If you're using a document method (i.e. a method from a package), the easiest way to get help on it is to construct the call then put `?` in front of it. `?` will use the arguments to figure out which help file you need:
...
To define an S4 class, you must define two key properties:
...
It is good practice to supply a `prototype`, a list of default values for each slot. This ensures that you can construct a valid object without any additional arguments, which important property that makes subclassing easier.

`setClass()` has 9 other arguments, but they are all either deprecated or not recommended. If you have existing S4 code that uses them, I'd recommend carefully reading the documentation and upgrading to modern practice. Note that the documentation recommends that you avoid `prototype` in favour of the `initialize()` method but unfortunately this has significant drawbacks and should be avoided.

To create a class, you call `setClass()`, supplying these three properties. Lets make this concrete with an example. Below we create a person class with character `name` and numeric `age` slots. We 
...
```{r}
hadley2 <- new("Person", name = "Hadley")
hadley2
```

### Helper

`new()` is a low-level constructor that requires precise knowledge of the class to use correctly. User-facing classes should always be paired with a user-friendly helper. A helper should always:
...
The constructor automatically checks[^new-s3] that the slots have correct classes:

[new-s3]: `new()` has the same job as the S3 constructors described in Section \@ref(s3-constrcutor). However, instead of writing the code ourselves, `new()` uses the class definition to perform the checks on our behalf.

```{r, error = TRUE}
Person(mtcars)
```

But you will need to implement any more complicated checks yourself. For example, we might want to make it clear that the Person class is a vector class, and can store data about multiple people. That's not currently clear because `@name` and `@age` can be different lengths:

```{r}
Person("Hadley", age = c(30, 37))
```

To enforce additional constraints we need a validator function, which takes a special form in S4. A validator should be created with `setValidity()` which takes the name of the class and a specially formed function. If the object is well-formed, it should return `TRUE`; if there are problems it should return character vector describing them. 
...
Note that the validity method is only called automatically by `new()`, so you can still create an invalid object by modifying it after the fact:
...
You can explicitly check the validity yourself by calling `validObject()`, and in Section \@ref(accessors), you'll see how to create modification functions that do this automatically:
...
If you are using a documented class (i.e. one provided by a package) you can get help on it with `class?Person`.
...
1.  Imagine you were going to reimplement factors, dates, and, data frames in 
    ...
    classes. What should they inherit from? What slots should they use?
...
It is bad practice to use `{` in the generic function. This triggers a special case that is more expensive, and generally best avoided.
...
If the slot is also writeable, you should provide a setter function. This will be slightly more complicated than the getter because you'll need to check that the new object is still valid.

<!-- GVW: this is my first encounter with `name<-` notation - worth a back ref to remind people what it's for, or will most readers know? -->
...
<!-- GVW: say explicitly that we do this with `setAs` (I initially assumed this was a typo...) -->
...
<!-- GVW: maybe worth saying that if you have to draw diagrams to figure out what method is actually going to be called, you should go back and simplify your design? -->
...
However, it's generally better to be a little more specific and provide a full S4
