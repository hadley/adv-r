...
S4 allows you to define a "validity" method, that check if your object is valid. You set this method by call `setValidity()`. Here's a port of our factor validity checking to S4. A validity method should a character vector containing a list of all the problem. If there are a no problems, return a character vector of length 0.
...
* If you define your own intializer method, and want to check validity, you 
...
* If the validation method is expensive, you may want to make it optional so
  ...
  valid object. You could do this by creating an initializer that only calls
...
The error is not very informative, but you might remember that we defined an `initialize()` for Person that request a name. The default coercion method generates a default value for `@boss` by calling `new("Person")`. We can override the default coercion to supply a better error.
