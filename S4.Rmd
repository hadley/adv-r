...
Unfortunately there is not one good reference for S4 and as you move towards more advanced usage, you will need to piece together needed information by carefully reading the documentation and performing experiments.
...
* A string giving the class it inherits from, or, in S4 terminology,
  ...
  inheritance. We'll come back to this in [method dispatch], but in brief
  I recommend avoiding mulitple inheritance because it makes reasoning
  about your code significantly harder.

S4 objects can inherit from and include as slots base types and S3 classes. We'll come back to the details at the end of the chapter in [S4 and existing code].
...
*   If you don't supply values for the slots, `new()` will automatically 
    ...
    In this case, you might prefer that `name` is always required, and that
    `age` defaults to `NA`.
...
We can override the default behaviour by providing an `initialize()` method. You'll learn more about the details of creating methods with `setMethod()` in the next section, but in brief, the following code defines a `Person` method for the `intialize` generic.
...
As in S3, it's not necessary to provide a helper function, but you should if helps the user. For example, you could imagine creating a S4 version of the factor class. Just like in the S3 class, you still want to supply a `Factor()` helper that takes a more natural specification of the inputs.
...
It's possible to use the built-in `setValidity()` function to define a validity method. However, I do not recommend using this feature because it the validation method is always run by `new()`. This means that you no longer have a cheap way to create objects if you already know the values are valid.

Instead, follow the same principle as for S3: create a separate validate function that you can call when needed. Let's make this concrete by porting our S3 `validate_factor()` function to a hypothetical S4 Factor class. 
...
If you are using a class defined by a package you can get help on it using the special form
