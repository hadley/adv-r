...
setClass() returns a constructor that wraps new(); my own convention is

## internal use
.Person <- setClass("Person", ...)

## public constructor, with named & typed arguments
Person <- function(name=character(), age=numeric()) {
    stopifnot(length(name) == length(age))  # or in validity method
...
```{r}
hadley <- new("Person", name = c("Hadley", "Charlotte"), age = c(37, NA))
hadley
```
...
<!-- FIXME: introduce and use accessors here -->

As a user, you should generally not extract values out of someone else's object using `@`. The names of the slots are considered an implementation detail and may change without warning. Instead, look for a method that provides the information you want.
...
<!-- FIXME: initialize has a complicated contract that is not easily satisfied; most experienced Bioconductor developers don't actually write initialize methods, but rather a constructor Person() that checks / etc and then calls new() (like Factor(), below). For instance, S4Vectors defines 35 classes but no initialize methods, IRanges has 85 classes and no initialize methods.

A 'best practice' would be to ensure that validObject(new("Person")) succeeds. This is because, internally, the implementation of derived classes will sometimes require construction of the base class via this simple call. When modeling rows, it doesn't make sense to have a zero-length slot, but it does when modeling columns.

Slot assignment triggers whole-object copies, so your style here is expensive; better to

    callNextMethod(.Object, name=name, age=age, ...)
...
setAs("Person", "Employee", function(from) {
  stop("Can not coerce a Person to an Employee", call. = FALSE)
})
as(mary, "Employee")
```
...
To explain method dispatch, we'll start with single inheritance and single dispatch, and work our way up to the more complicated cases. To illustrate the ideas without getting bogged down in the details, we'll use an imaginary __class graph__ that uses emoji:
