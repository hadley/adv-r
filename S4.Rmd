...
* [S4 system development in Bioconductor][S4-bioconductor], a slide deck
  by Patrick Aboyoun.
JH: this resource seems a little old, Patrick Aboyoun hasn't been with Bioconductor since 2010.
...
JH: The `slot` argument really accepts a named character vector, not a named
list. But the latter is coerced to the former so either works in practice.
...
JH: This advice seems somewhat dangerous; `S4Vectors::setValidity2` is used in Bioconductor to give more control when validity methods are actually run, although it is not well documented. <https://github.com/Bioconductor-mirror/S4Vectors/blob/master/R/S4-utils.R#L83>
...
Note that the classes are arranged in a different order in the class graph and the method graph. in different orders in the generic call vs. the method list. This is because you understand a class hierarchy by moving from the generic to the specific, but method dispatch proceeds in the opposite direction, from the most specific to the most general.
...
JH: You never mention that double red methods in the diagrams are those without a definition.
