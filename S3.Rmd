...
There are two special types of generics present in base R:
...
You can not create these types of generic yourself, but they're important to know about because you might want to create methods for them.
...
In other words, a method is just a function with a special name.
...
Because of this you should check in your methods that the inputs are the correct types.
...
We've hinted at how the `class` attribute can have more than a single value. This is used in method dispatch: if an object has class `c("a", "b", "c")` R looks first for `generic.a()`, then `generic.b()`, then `generic.c()`. 
