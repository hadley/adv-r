...
*   Some complex objects are most naturally specified by multiple simple    
    ...
    ```{r}
    POSIXct <- function(year = integer(), month = integer(), day = integer(), 
                        hour = 0L, minute = 0L, sec = 0, tzone = "") {
...
*   Scalar objects typically use a list to represent a single "thing". 
    ...
    Scalar objects can also be built on top of functions and calls[^pairlist]. 
    This is less generally useful, but you can see one application in
    `stats::ecdf()` and another in `rlang::quo()` (Chapter \@(quasiquotation)).
