...
```{r}
# Works because the function is in the base package
mean.Date
...
To avoid foot-bullet intersections when creating your own class, you should provide three functions: 

* __Constructor__: (`new_x()`) efficiently creates new objects with correct basic structure. 
* __Validator__: (`validate_x()`) enforces valid object structure (for more complicated classes).
* __Helper__: (`x`) creates new objects in a user-friendly manner (for more complicated classes).
...
This function is called primarily for its side-effects (throwing an error if the object is invalid) so you'd expect it to invisibly return its primary input. Unlike most functions called for their side-effects, with validation methods you'll often want to return the value visibly, as we'll see next.
...
These helpers fill a useful role, but are not computationally efficient: behind the scenes `ISODatetime()` works by pasting the components into a string and then using `strptime()`. More efficient equivalents are available in `lubridate::make_date()` and `lubridate::make_datetime()`.
...
In the __scalar__-style, a single object represents a single "value", with the underlying representation using a named list. This is the style that you are most likely to use in practice. The constructor for the scalar type is slightly different because the arguments become named elements of the list, rather than attributes.
