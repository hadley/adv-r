...
S3 gives you the freedom to build a new class on top of any existing base type. So far, we've focussed on the "vector" style of S3 object where you take an existing vector type and add some attributes, and a single object can represent multiple data points.

Another common style is the "scalar" style, where you use a list with named elements, and each object represents a single "thing". The constructor for the scalar type is slightly different because the arguments become named elements of the list, rather than attributes.
...
The S3 package also provides helper for this case:
...
When creating your own classes, you should pick the vector style if your class closely resembles an existing vector type. Otherwise, use a scalar (list) style. The scalar type is generally easier to work with because implementing a full range of convenient vectorised methods is usually a lot of work.
...
Let's create a new generic and some methods to solve a real problem. Many S3 objects can be naturally created from an existing object through __coercion__. If this is the case for your class, you should provide a coercion function, an S3 generic called `as_class_name`.
...
A method should always have the same arguments as the generic. This is not enforced, but it is good practice because it will avoid confusing behaviour, and if you eventually turn your code in to package, it is required by R CMD check.

The exception to the rule is if the generic has `...`. In that case, the method still needs to have all the arguments of the generic (including `...`), but can also have its own additional arguments. For this reason, you should always include `...` in the generic. This allows methods to take additional arguments, which is important because you don't know what additional arguments that a method for someone else's class might need. One downside of using `...` is that it silently swallows misspelled arguments. 
...
You can think of the glm class "inheriting" behaviour from the lm class, and the ordered class inheriting behaviour from the factor class because of the way method dispatch works. If there is method available for the subclass, R will use it, otherwise it will fallback to the "parent" class. We can write a little to which methods glm overrides from lm:

```{r}
setdiff(
  attr(methods(class = "lm"), "info")$generic,
  attr(methods(class = "glm"), "info")$generic
)
```
...
*   Group generics, which allow you to define methods for multiple generics
    with one function.

*   Environments and namespaces which affect precisely where `UseMethod()` 
    looks for method matches.
    
*   Generics that use double dispatch

*   Generics that use dots dispatch

These are explained in more detail below, but feel free to skip if this is your first time learning about S3.
...
y <- structure(sample(10), class = "integer")
class(y) 
s3_dispatch(y[1])
```  
