...
* A __validator__, `validate_x()`, that performs more expensive checks to 
...
S3 doesn't provide a formal definition of a class, so it has no built-in way to ensure that all objects of a given class have the same structure (i.e. the same base and the same attributes with the same types). Instead, you should enforce a consistent structure yourself by using a __constructor__, a function that constructs objects of a given class, ensuring that they have a consistent structure. 
...
Base R generally does not provide constructors[^base-constructors] so we'll demonstrate these ideas by creating constructors for base classes. We'll start with one of the simplest S3 classes: `Date`, which is just a double with a class attribute. The constructor is simple because `Date`s have no additional attributes:

[^base-constructors]: Recent versions of R do have `.Date()`, `.difftime()`, `.POSIXct()`, and `.POSIXlt()` functions but they are internal and not well documented.
...
The purpose of the constructor is to help you, the developer. That means you can keep them simple, and you don't need to optimise the error messages for public consumption. If you expect users to also create objects, you should create a friendly helper function, called `class_name()`, which I'll describe shortly.

A slightly more complicated constructor is that for `difftime`, which is used to represent time difference. It is again built on a double, but has a units attribute  that must take one of a small set of values.

```{r}
new_difftime <- function(x = double(), 
...
The constructor is a developer function: it will be called in many places, by an experienced user. That means it's ok to trade a little safety in return for performance, and generally you should avoid potentially time-consuming checks in the constructor.

### Validators

More complicated classes require more complicated checks for validity. Take factors, for example. A constructor might only check that types are correct, making it possible to create malformed factors:
...
If you want users to construct objects from your class, you should also provide a helper method that makes their life as easy as possible. This should have the same name as the class, and should be parameterised in a convenient way: exactly what this means will vary from case to case so it's hard to give general advice. Instead, we'll look at a couple of concrete examples.

Take `factor()`, for example. The simplest possible implementation looks something like the code below. It takes a character vector, and guesses the levels should the unique values. This is not always correct (since some levels might not be seen in the data), but it's a useful default.
...
What about `POSIXct()`? There's no `POSIXct()` in base R: you must either parse a string with `as.POSIXct()` or `strptime()`, or call `ISODatetime()` to create it from components. Either string or components could make for a natural helper, but I think it's slightly more flexible with components, leading to the helper below:
...
(Note that this helper is not efficient: behind the scenes `ISODatetime()` works by pasting the components into a string and then using `strptime()`. A more efficient equivalents is available in `lubridate::make_datetime()`).
...
The job of an S3 generic is to perform method dispatch, i.e. find the implementation for a class. S3 generics have a simple structure: they call `UseMethod()`, which finds and calls the right method. 
...
How does `UseMethod()` work? It basically creates a vector of method names, `paste0("generic", ".", c(class(x), "default"))`, and then looks for each method in turn. As soon as it finds a matching method, it calls it. If no matching method is found, it throws an error. 

We can see this in action with `sloop::s3_dispatch()`. You give it a call to an S3 generic, and it lists all the possible methods. It shows a `=>` next to the method that is called (the first method that exists), and a `*` next to all other method that exist. For example, what method is called when you print a `Date` object?
...
As you can see, the essence of method dispatch is quite simple. However, as the chapter proceeds you'll see method dispatch get progressively more complicated to encompass inheritance, objects without a class, internal generics, and group generics. The code below shows a couple of more complicated examples which we'll come back to in Sections \@ref(inheritance) and \@ref(dispatch-details). 
...
### Method arguments {#s3-arguments}

Note that a method must have the same arguments as its generic. This is enforced in packages by `R CMD check`, but it's good practice even if you're not creating a package. 

There is one exception to this rule: if the generic has `...`, the method can contain a superset of the arguments. This allows methods to take arbitrary additional arguments. The downside of using `...`, however, is that any misspelled arguments will be silently swallowed[^ellipsis].

[^ellipsis]: See <https://github.com/hadley/ellipsis> for an experimental way of warning when method arguments in `...` are not used.
...
*   "Record style" objects use a list of equal-length vectors to represent 
    components of the object. to represent like `POSIXlt`, are implementing 
    using a list of fixed components, and override the `length()` method.
   
    ```{r}
    x <- as.POSIXlt(Sys.time() + 1:100)
    length(x)
    length(unclass(x))
    ```

*   Data frames are similar to records style objects in that they're 
    implemented on top of lists of equal length vectors. However, data frames
    are conceptually 2d, and the individual components are readly exposed to
    the user. The number of observations is the number of rows, not the length:
...
*   Scalar objects use a list to represent a single "thing". For example,
...
As well as classes built on top of a vector type, you can also build classes on top of functions. This is less generally useful, but you can see one application in `stats::ecdf()`. You can also build a class on top of a pairlists or call. I have yet to find a reason to add a class to pairlist, but formula and quosures (Chapter \@(quasiquotation)) are built on top of calls.

Unfortunately, describing the appropriate use of each of these object styles is beyond the scope of this book. However, you can learn more from the documentation of the vctrs package (<https://vctrs.r-lib.org>); the package also provides constructors and helper that make implementation of the different styles considerably simpler.

### Exercises

1.  Categorise the objects returned by `lm()`, `factor()`, `table()`, 
    `as.Date()`, `ecdf()`, `ordered()`, `I()` into "vector", "record", 
    "data frame" and "other".
...
*   The class can be a character _vector_. For example, the `POSIXct` and 
    `ordered` classes have two classes:
...
`NextMethod()` is the hardest part of inheritance to understand so we'll dive into an example. The most common use case of `NextMethod()` is `[`: how do you implement a subsetting method for your own class, without using `[`? `NextMethod()` resolves the issue by allowing you to delegate most of the work to the underlying C code. Let's demonstrate with a simple toy class: a `secret` class that hides its output when printed:
...
To fix this, we need to provide a `[.secret` method. This uses `NextMethod()` to call the internal `[` code, then wraps the results back into the secret class:
...
Like for `UseMethod()`, the precise semantics of `NextMethod()` are complex. In particular, it tracks the list of potential next methods with a special variable, which means that modifying the object that's being dispatched upon will have no impact on which method gets called next.
...
You also need to think carefully about your methods as you can no longer use the constructor, as that limits methods to returning the parent type, and forces a subclasser to override every method, even if the implementation is the same. 

Concretely, that means we need to revise the `[.secret` method because currently it always returns a `secret()`:
...
We want to make sure that `[.secret` returns the same class as `x` even if it's a subclass. As far as I can tell, there is no way to solve this problem using base R. Instead, you'll need to use the vctrs package, which provides a solution in the form of the `vctrs::vec_restore()` generic. This generic takes two inputsIt takes two objects: an object of the most specific subclass to restore to, and the object to restore.
...
Then we can use `vec_restore()` in our `[.secret` method to ensure that we also return an object the same class as `x`:
...
1.  Why is the following code surprising?
...
Some S3 generics, like `[`, `sum()`, and `cbind()`, don't call `UseMethod()` because they are implemented in C. Instead, they call the C functions `DispatchGroup()` or `DispatchOrEval()`. These functions are called __internal generics__, because they dispatch internally, in C code. Internal generics only exist in base R, and you cannot create an internal generic in a package.

`s3_dispatch()` shows internal generics by including the name of the generic at the bottom of the method class:
