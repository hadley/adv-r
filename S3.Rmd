...
**Question:** Shouldn't the first call `new_POSIXct(1)` fail `stopifnot(is.character(tzone), length(tzone) == 1)`? Currently, it produces the same output as the second call `new_POSIXct(1, tzone = "UTC")`.
...
Generally, the constructor should not check that the values are valid because such checks are often expensive. For example, our `new_POSIXct()` constructor does not check that `tzone` is a valid value, and **we get a warning when the object is printed** (no error is printed from this command).
