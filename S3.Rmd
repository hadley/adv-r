...
An S3 object consists of a base type with the "class" attribute set. The base type is typically a vector, although we will see later that it's possible to use other types of classes.  For example, a factor is built on top of an integer vector; the value of its class attribute is "factor", and it stores information about the "levels" in another attribute.
...
`str()` shows the internal structure of S3 objects. Be careful when using it: some S3 classes provide a custom `str()` method which can hide the underlying details. For example, take the `POSIXlt` class, which is one of two classes used to represent date-time data:
...
If you have done OOP in other languages, you may be surprised to learn that S3 has no formal definition of a class. To make an object an instance of a class, you simply take an existing object and set its class attribute. You can do this during creation with `structure()`, or after the fact with `class()<-`: \index{S3!classes} \index{classes!S3}
...
The class name can be any character vector, but I recommend using only letters and `_`. Opinion is mixed whether to use underscores (`my_class`) or CamelCase (`MyClass`) for multi-word class names. Pick one convention and stick with it.

Avoid `.` in class names.

It's possible to provide a vector of class names, which helps reduce your workload by allowing classes to share code. We'll come back to this idea in [inheritance].

S3 doesn't check your use of the class attribute, which means you can change the class of existing objects:
...
Since S3 doesn't provide a formal definition of a class, it can't ensure that all objects of a given class have the same structure (i.e., the same attributes with the same types). Instead, you should enforce a consistent structure yourself by using a __constructor__. A constructor is a function that creates objects of a given class and ensures that they always have the same structure.  Every constructor should:

1.  Be called `new_class_name()`.
1.  Have one argument for the base object, and one for each attribute (or more if the class can be subclassed, see [inheritance]).
...
The constructor checks that `x` is a double, and that `tzone` is a length 1 character vector. Again, we use `stopifnot()` since the constructor is a developer-focussed function not meant for the average user. Note that POSIXct uses a class _vector_; we'll come back to what that means in [inheritance].
...
More complicated classes will require more complicated checks for validity. For example, the constructor for factors only checks that the structure is correct:
...
This means it can be used to create invalid factors:
...
**GVW:** do you explain somewhere else how to create good error messages?
...
These helpers fill a useful role, but are not efficient: behind the scenes `ISODatetime()` works by pasting the components into a string and then using `strptime()`. More efficient equivalents are `lubridate::make_datetime()` and `lubridate::make_date()`.

### Object styles

**GVW:** why is it important that a singl vector-style object represents multipl values?

S3 gives you the freedom to build a new class on top of any existing base type. So far, we've focussed on vector-style objects that add attributes to an existing vector type. Importantly, a single vector-style object represents multiple values.

**GVW:** does a scalar-style object correspond to a struct in a language like C? If so, worth mentioning?

There are two other important styles: scalar-style and data-frame-style.  Each __scalar-style__ object represents a single "value", and these are built on top of named lists. This is the style that you are most likely to use in practice. The constructor for the scalar type is slightly different because the arguments become named elements of the list, rather than attributes.
...
**GVW:** took me a moment to realize that "df_class" is the name of the class...
...
**GVW:** OO.Rmd said, "The class defines the __fields__, which are the data that every instance of that class possesses."  This doesn't seem to be quite the same definition?
...
Note that you don't pass any of the arguments of the generic to `UseMethod()`; it uses black magic to pass them on to the method itself. Generally, you should avoid doing any computation in a generic, because the semantics are complicated and few people know the details. In particular, any modifications to the arguments of the generic will be undone, leading to much confusion.

A generic isn't useful without some methods, which are just functions that follow the naming scheme `generic.class`. Because a method is just a function with a special name, you _can_ call methods directly, but you generally _shouldn't_. The main reason to do it is to improve performance by avoiding repeated lookups (see [performance](#be-lazy) for an example).  The main reason not to is that as your code evolves, you can easily find yourself calling the wrong method for a new kind of object.
...
S3 objects can often be created from existing objects through __coercion__. If this is the case for your class, you should provide a coercion function, an S3 generic called `as_class_name`. Base R generally does not follow this convention, which can cause problems as illustrated by `as.factor()`:
...
Every `as_y()` generic should have an `as_y.y` method that returns its input unchanged:

```{r}
as_factor.factor <- function(x, ...) x
```

This ensures that coercion works if the input is already the right type.

Two useful coercion methods for factors would be for character and integer vectors.
...
Typically the coercion methods will either call the constructor or the helper, whichever makes the code simpler; here the helper is simplest. If you use the constructor, remember to also call the validator function.

If you think your coercion function will be frequently used, it's worth providing a __default method__ that gives a better error message. Default methods are called when no other method is appropriate, and are discussed in more detail in [inheritance].
...
If we create a dummy object with classes "b" and "a", `s3_dispatch()` shows that all three potential methods are available:
...
**GVW:** I _think_ I understand the difference in the preceding sentence between "the argument that is dispatched upon" and "the object that is being dispatched on", but I'm not sure.
...
**GVW:** illustrate this with an example?
...
These properties should be enforced by the constructor.
...
**GVW:** I think it would help here to call classes things other than "my_class" and "subclass", because expressions like 'subclass = "subclass"' are becoming confusing.
...
**GVW:** I'd remove the mention of NextMethod from the preceding paragraph and introduce it later - there's already a lot going on here.

**GVW:** need example here of overriding a method in a subclass before going on to the more complex example upcalling to the parent method and implementing provenance.
...
**GVW: whoa - "an instance of the parent class, typically created by NextMethod" threw me at this point. I thought NextMethod called the next available method - I don't see how it's creating an instance of the parent class.  Oh, that's a side effect of arrange, mutate, etc., rather than something that NextMethod always does.  OK, I guess I understand, but a gentler example before this one would help.
