...
An S3 object is built on top of a base type (typically a vector) with the "class" attribute set. For example, take the factor. It is built on top of an integer vector, and the value of the class attribute is "factor". It stores information about the "levels" in another attribute.
...
The class can be any character vector, but I recommend using only letters and `_`. Avoid `.`. Opinion is mixed whether to use underscores (`my_class`) or CamelCase (`MyClass`) for multi-word class names. Pick one convention and stick with it.
...
To avoid foot-bullet intersections when creating your own class, you should always provide a __constructor__ function which enforces the structure of your class. For more complicated classes, you should typically also provide a __validator__ and __helper__,  as described below.
...
These helpers fill a useful role, but are not computationally efficient: behind the scene `ISODatetime()` works by pasting the components into a string and then using `strptime()`. A more efficient equivalent is available in `lubridate::make_datetime()` and `lubridate::make_date()`.
...
The __scalar__-style, which is based on a named list, and a single object represent a single "value". This is the style that you are mostly likely to use in practice. The constructor for the scalar type is slightly different because the arguments become named elements of the list, rather than attributes.
...
new_scalar_class <- function(x, y, z) {
  structure(
    ...
    class = "my_scalar_class"
  )
}
```

(For a real constructor, you'd also check that `x`, `y`, and `z` fields are the types that you expect.)
...
The __data-frame-style__ builds on top of a data frame (a named list where each element is a vector of the same lengrth), and adds additional attributes to store important metadata. A data-frame-style constructor looks like:
...
The job of an S3 generic is to perform method dispatch, i.e. find the function designed to work specifically for the given class. S3 generics have a simple structure: they call `UseMethod()` which does the hard work of method dispatch. `UseMethod()` takes two arguments: the name of the generic function (required), and the argument to use for method dispatch (optional). If you omit the second argument it will dispatch based on the first argument, which is what I generally advise. \indexc{UseMethod()} \index{S3!new generic}
...
Note that you don't pass any of the arguments of the generic to `UseMethod()`; it uses black magic to pass them on automatically. Generally, you should avoid doing any computation in a generic, because the semantics are complicated and few people know the details. Rather than creating a new evaluation enviroment, dispatch arranges for the method to use the evaluation environment of the generic, with the exception that the arguments to the method are passed as they were recieved (i.e. any modifications you've performed in the generic will be undone).

A generic isn't useful without some methods, which are just functions that follow a naming scheme (`generic.class`). Because a method is just a function with a special name, you _can_ call methods directly, but you generally _shouldn't_. The main reason to call the method directly is it sometimes leads to considerable performance improvements. See [performance](#be-lazy) for an example.
...
You can see all the methods defined for a generic with `methods()`: <!-- FIXME: methods() is not used here -->

```{r}
s3_methods_generic("generic")
```

Note the false positive: `generic.skeleton()` is not a method for our generic but an existing function in the methods package. It's picked up because method definition relies only on a naming convention. This is another reasons that you should avoid using `.` in non-method function names.
...
Let's create a new generic and some methods to solve a real problem. Many S3 objects can be naturally created from an existing object through __coercion__. If this is the case for your class, you should provide a coercion function, an S3 generic called `as_class_name`. Base R generally does not follow this convention. For example, take `as.factor()`:
...
We can fix these isspues by creating our own generic:
...
Methods should always have the same arguments as their generics. This is not usually enforced, but it is good practice because it will avoid confusing behaviour. If you do eventually turn your code in to package, R CMD check will enforce it, so it's good to get into the habit now.

<!-- FIXME: adding ... to generics means that the methods have ..., which means that spurious / mistyped arguments are silently ignored. This seems like a big cost, so ... should only be added when necessary (methods are known to have different signatures) -->
The only exception is if the generic has `...`. In that case, the method still needs to have all the arguments of the generic (including `...`), but can also have its own additional arguments. For this reason, you should always include `...` in the generic. This allows methods to take additional arguments, which is important because you don't know what additional arguments that a method for someone else's class might need. One downside of using `...` is that it silently swallows misspelled arguments. 
...
# swallowed by ...
foo("This is a long string", trncate = 2)
```

<!-- FIXME: but checking that ... is empty defeats the 'rationale' for ... in the first place -- to allow arguments to be passed to next methods, in 'your' method or some other method on the same generic -->
To be friendly to your users, it's a good idea to check that `...` is empty. An easy way to do so is with `S3::check_method_dots()`:

```{r, error = TRUE}
foo.character <- function(x, ..., truncate = 10) {
  S3::check_method_dots(...)
  substr(x, 1, truncate)
}
foo("This is a long string", trncate = 5)
```
...
Like `UseMethod()`, the precise semantics of `NextMethod()` are complex. It doesn't actually work with the class attribute of the object, but instead uses a special global variable (`.Class`) to keep track of which method to call next. This means that modifying the argument that is dispatched upon has no impact, and you should avoiding modifying the object that is being dispatched on.

Generally, you call `NextMethod()` without any arguments. Howver, if you do give arguments, they are passed on to the next method, as if they'd be supplied to the generic.
...
* Generalised linear models are a generalisation of linear models that allow
  the error term belong to a richer set of distributions, not just the normal
  distribution like the linear model. In R, `glm()` returns objects of 
  class `c("glm", "lm")`.
...
You can think of the glm class "inheriting" behaviour from the lm class, and the ordered class inheriting behaviour from the factor class because of the way method dispatch works. If there is method available for the subclass, R will use it, otherwise it will fallback to the "parent" class. For example, if you "plot" a glm object, it falls back to the lm method, but if you compute the ANOVA, it uses a specific method.
...
There are three principles to adhere when creating a subclass:

* A subclass should always be built on the same base type as a parent.

* The `class()` of the subclass should be of the form 
  `c(sub_class, parent_class)`

* The fields of the subclass should be a superset of the fields of the parent.
...
Then the implementation of the subclass constructor is simple: it checks the types of the new fields, then calls the parent constructor. If your subclass is more complicated, you'd also provide validator and helper functions, as described previously
...
And now you want to make this class work with dplyr. The class doesn't change any of the computation related to data frame, it just needs to preserve the attributes, which  dplyr doesn't know anything about. That means you need to provide a method for each dplyr generic. The computation is unchanged, so you can use `NextMethod()` to do all the hard work, but you need to manually reconstruct the pronvenance object.
...
This chapter concludes with a few additional details about method dispatch. It is safe to skip these details if you're new to S3, but they provide important information that is not well documented elsewhere.
...
But unfortunately this is just a convenient fiction, and dispatch actually occurs on the __implicit class__. There is no base function that will compute the implicit class, but you can use a helper from S3: \index{implicit class} \index{base types!implicit class}
...
For performance reasons, internal generics do not dispatch to methods unless `is.object()` is true, i.e. the class attribute has been set. This means that internal generics do not use the implicit class. Again, if you're confused, rely on `s3_dispatch()` to show you the difference.
...
The generics in the "Ops" group, which includes the two-argument mathematical and logical operators like `-` and `&`, implements a special type of method dispatch. They dispatch on the type of _both_ of the arguments, so called __double dispatch__. This is necessary to preserve the commutative property of many operators, i.e. `a + b` should equal `b + a`. Take the following simple example:
...
Let's take a look at another case. What happens if you try and add a date and a factor? There is no method in common, so R calls the internal `+` method (which preserves the attributes of the LHS), with a warning.
