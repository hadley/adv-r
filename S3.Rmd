...
It's important to know which functions are internally generic, so you can write methods for them. It's not easy to tell if a function is internally generic, because it just looks like a typical call to a C function:
...
You can look in the documentation, in the global variable `.S3PrimitiveGenerics`, and in `?InternalMethods`.
