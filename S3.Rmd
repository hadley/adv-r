...
Generally, constructors will be used by developers (i.e. you). That means they can be quite simple, and you don't need to optimise the error messages for user friendliness.

Typically, you'll also provide a user friendly way to create objects, often by coercing from an existing type. If this code is in a package, you may want to export the constructor and use it to document the structure of the class.
...
(If your code works a lot with base objects that don't have a constructor, you might consider writing one yourself, just to keep your code consistent.)
...
If you find the checks growing long, and you want to re-use them in other places, consider moving out into a separate validity checking function.

```{r}
check_factor <- function(x) {
  values <- unclass(x) # blech
...
If you are constructing many known-correct object elsewhere in your code you may want to make expensive checks explicit by putting in a separate function and calling.  This is a little less safe (because you might forget to call when it is important), but for complex objects the performance benefit may be worth. Don't do it proactively; only do it if [profiling] reveals object construction to be a bottleneck.
...
Constrast this with `Date` and `POSIXct`. Neither of these have helpers in base R because there's no particularly natural way for them to be constructed. Instead they provide a coercion function that lets you create from existing base types.

### Object styles

S3 gives you the freedom to build a new class on top of an existing base type. Often the only way to find out which base type you should use is to pick the wrong one and 

However, there are two common styles that it's best to start with:
...
So far the constructors have created the vector type where you check the type of the base object and then add attributes (including class). The constructors for a scalar type object is slightly different, as you always put the elements in a list:
...
*   "Vector" lists, like data frames. This is technically a subtype of the
    "vector type", where your vector is a list.
...
```{r}
as_factor <- function(x) UseMethod(x)
