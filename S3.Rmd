...
We'll use the sloop package for interactive helpers.
...
Instead, we need some way to call the the underlying `[` code, i.e. the implementation that would get called if we didn't have a `[.secret` method. One approach would be to `unclass()` the object:
