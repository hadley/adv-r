...
More complicated classes will require more complicated checks for validity. Take factors, for example. The constructor function only checks that structure is correct:
...
The __scalar__-style is based on a named list, and a single object represents a single "value". This is the style that you are most likely to use in practice. The constructor for the scalar type is slightly different because the arguments become named elements of the list, rather than attributes.
