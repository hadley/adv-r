...
An S3 object is built on top of a base type with the "class" attribute set. The base type is typically a vector (though we will see later that it is also possible to create other types of classes). For example, take the factor. It is built on top of an integer vector, and the value of the class attribute is "factor". It stores information about the "levels" in another attribute.
...
* __Constructor__: (`new_x()`) to efficiently create new objects with correct basic structure. 
* __Validator__: (`validate_x()`) to verify that objects are well-formed (for more complicated classes).
* __Helper__: (`x`) to create new objects in a user-friendly manner (for more complicated classes).
...
The job of an S3 generic is to perform method dispatch, i.e. find the function designed to work specifically for the given class. S3 generics have a simple structure: they call `UseMethod()` which finds the right function to use. `UseMethod()` takes two arguments: the name of the generic function (required), and the argument to use for method dispatch (optional). If you omit the second argument it will dispatch based on the first argument, which is what I generally advise. \indexc{UseMethod()} \index{S3!new generic}
...
Many S3 objects can be created from an existing object through __coercion__. If this is the case for your class, you should provide a coercion function, an S3 generic called `as_class_name`. Base R generally does not follow this convention, which can cause problems, as illustrated by `as.factor()`:
...
We can fix these issues by creating a new generic coercion function and some methods:
...
Methods should always have the same arguments as their generics. This is not enforced by the S3 system, but it is good practice because it will avoid confusing behaviour. If you do eventually turn your code into a package, R CMD check will enforce it, so it's good to get into the habit now.
...
The only exception is if the generic has `...`. In that case, the method still needs to have all the arguments of the generic (including `...`), but can also have its own additional arguments. For this reason, you should always include `...` in the generic, even if your methods don't use it. This allows users who build off of your generic to write methods that take additional arguments as needed for their classes. One downside of using `...` is that it silently swallows misspelled arguments. 
...
# Silently returns wrong result because the misspelled argument
# gets swallowed by ...
...
* Generalised linear models are a generalisation of linear models. This is 
  a natural case for the use of inheritance and indeed, in R, `glm()` returns 
  objects of class `c("glm", "lm")`.
...
1. In the special `.__S3MethodsTable__.` object in the function environment of 
   the generic. Every package has an `.__S3MethodsTable__` that lists all of
   the S3 methods that the package exports.
...
But unfortunately dispatch actually occurs on the __implicit class__. The implicit class includes three components:

* "array" or "matrix" (if the object has dimensions).
* `typeof()` (with a few minor tweaks).
* If it's "integer" or "double", "numeric".

There is no base function that will tell you the implicit class of an object, but you can use a helper from the S3 package: \index{implicit class} \index{base types!implicit class}
...
For performance reasons, internal generics do not dispatch to methods unless the class attribute has been set (i.e., `is.object()` is true). This means that internal generics do not use the implicit class. Again, if you're confused, rely on `s3_dispatch()` to show you the difference.
...
Defining a single group generic for your class gives you access to all of the member generics of the group. Methods for group generics are looked for only if the methods for the specific generic do not exist:
...
Let's take a look at another case. What happens if you try to add a date and a factor? There is no method in common, so R calls the internal `+` method (which preserves the attributes of the left-hand side), with a warning.
