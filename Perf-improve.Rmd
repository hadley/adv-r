...
* Section \@ref(code-organisation) teaches you how to organise 
  ...
  strategy and organisational style that's useful when working on performance. 
...
We'll use [bench](https://bench.r-lib.org/) to perform experiments on small self-contained code chunks.
...
I recommend that you keep a record of everything you try, even the failures. If a similar problem occurs in the future, it'll be useful to see everything you've tried. To do this I recommend R Markdown, which makes it easy to intermingle code with detailed comments and notes.
...
Below, I explore how you might improve the performance of `mean()`, `as.data.frame()`, and `diff()` by doing less work.

### `mean()`

Sometimes you can make a function faster by avoiding method dispatch. If you're calling a method in a tight loop, you can avoid some of the costs by doing the method lookup only once: \index{methods!cost of dispatch, avoiding}
...
In most cases, you can expect byte code compilation to give a 5-10% improvement. All functions in packages are byte code compiled by default. 
