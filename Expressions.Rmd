...
It would be nice if we could capture the intent of the code, without executing the code. In other words, how can we separate our description of the action from performing it? One way is to use `base::quote()`: \indexc{quote()}
...
`quote()` returns a quoted __expression__: the R code that captures our intent.
...
For more complex code, you can also use RStudio's tree viewer to explore the AST interactively, e.g. `View(quote(y <- x * 10))`.
