...
So we'll start by introducing some conventions for drawing ASTs, starting with a very simple call `f(x, "y", 1)`. I'll show trees in two ways:
...
*   The output of `lobstr::ast()`:

    ```{r}
    lobstr::ast(f(x, "y", 1))
    ```

For more complex code, you can also use RStudio's tree viewer which allows you to interactively explore the AST, e.g. `View(f(x, "y", 1))`.

Both approaches share conventions as much as possible:

*   The leaves are either symbols, like `f` and `x`, or constants, like `1` or
    `"y"`. Symbols are drawn in purple and have rounded corners. Constants, 
    which are atomic vectors of length one, have black borders and square 
    corners. Strings are always surrounded in quotes to emphasise their 
    difference from symbols. 

*   The branches are function calls, represtended by call objects, drawn as 
    orange squares. The first child (`f`) is the function that gets called; 
    the second and subsequent children (`x`, `"y"`, and `1`) are the arguments 
    to that function. 

There are a handful of more exotic objects that you'll encounter very rarely. I'll touch on those in Section \@ref(expression-exotic).

The above example only contained one function call, making for a very shallow tree. Most expressions will contain considerably more calls, making for a deeper tree. For example, consider `f(g(1, 2), h(3, 4, i()))`:
...
In particular, note the appearance of `i()`; it's a branch with a single (symbol) leaf.

Named arguments don't change the tree, but just add some labels:

```{r}
lobstr::ast(f(x = x, y = 1))
```

You might have wondered why these are _abstract_ syntax trees. They are abstract because they only capture important structural details, not whitespace or comments:
...
[^ast-infix]: The names of non-prefix functions are non-syntactic so I surround them with `` ` ``, as discussed in Section \@ref(non-syntactic).
...
All R code can always be written in prefix form, no matter how special the syntax:
...
1.  How are function factories (Chapter \@ref(function-factories)) displayed
...
## Other components {#expression-exotic}
