...
Quoted expressions are also called abstract syntax trees (AST) because the structure of R code is fundamentally hierarchy of nodes where each node represents a fundamental piece of the code. It can be naturally represented as a tree. To make that more obvious we're going to introduce some graphical conventions, illustrated with the very simple call `f(x, "y", 1)`. \index{abstract syntax tree}

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/expression-simple.png", dpi = 450)
```
  
*   Functions __calls__ are represented as orange nodes in the tree.  All the other nodes are represented as literal string that came from the code. Here the function name is the is shown as the letter f. This creates a parent child relationship.

R uses the AST to determine the proper order to use the pieces of the code in the program. Specifically it walks the AST from top to bottom, then left to to right. In the examples that follow you will see what this means and why pieces of the code cannot be used in the order they appear in the program.
