...
Constants and symbols are the leaves of the AST. Symbols represent variable names, and constant represent literal values.
...
There's one special symbol that needs a little extra discussion: the empty symbol, constructed with `expr()`. It's used to represent missing function arguments so has some odd behaviour. You can store an empty symbol in an variable, but once you've done so, if you try and refer to it in any way you'll get an error about missing arguments:
...
The only thing you can do that doesn't trigger an error is to check if it's `missing()`:

```{r}
missing(x)
```

You only need to know about the missing symbol if you're programmatically creating functions a function with missing arguments. \index{names|empty}
...
The names of a call are empty, except for unnamed arguments:
...
We'll learn about creating calls from scratch in [construct-calls]; an understanding of quasiquotation makes it much easier.
...
Fortuantely, whenever you encounter a pairlist, you can treat it just like a regular list:
...
To write functions like `findGlobals()` and `checkUsage()`, we need recursion, as recursive functions pair naturally with trees. There a two parts to the a recursive function:

* The __recursive case__: you divide the problem up in to smaller pieces,
  re-call the function on each piece, then combine the results back together
  again. For expressions, the recursive case includes handle calls and 
  pairlists.
  
* The __base case__: to ensure that function eventually terminates, you need to
  solve the simplest cases directly. When working with ASTs, this typically 
  means that you've reached the leaves of the tree; the symbols and names.

To make this pattern easier to see, we'll use two helper functions. First we define `expr_type()` which will return "constant" for constant, "symbol" for symbols, "call", for calls, pairlist" for pairlists, and the type of anything else that might be in the AST:
...
With these two functions in hand, the basic form of any function for walking the AST will be:
...
It's usually fairly easy to solve the problem for the base cases. The recursive cases are a little more tricky, but typically it's just a matter of finding the correct purrr function.
...
Note that our recursive function works with expressions which means we always need to use it with `expr()`. When writing a recursive function it's common to write a wrapper that provides defaults or makes the function a little easier to use. Here we'll typically make a wrapper that quotes its input (we'll learn more about that shortly), so we don't need to use `expr()` every time.
...
Next we need to implement the recursive cases. Here it's simple because we want to do the same thing for calls and for pairlists: recursively apply the function to each subcomponent, and return `TRUE` is any subcomponent contains a logical abbreviation. We can use `purrr::some()` to implement this pattern.
...
`logical_abbr()` is very simple: it only returns a single `TRUE` or `FALSE`. The next task, listing all variables created by assignment, is a little more complicated. We'll start simply, and then make the function progressively more rigorous. \indexc{find\_assign()} Again, we start by looking at the AST for assignment:

```{r}
ast(x <- 10)
```

Assignment is a call where the first element is the name `<-`, the second is the object the name is assigned to, and the third is the value to be assigned. 

Next, we need to decide what data structure we're going to use for the results. Here I think it will be easiest it we return a character vector giving all the variables that were created. With that in hand we can start by implementing the base cases and providing a helpful wrapper around the recursive function. The base cases here are really simple!
