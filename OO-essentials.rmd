...
All reference classes eventually inherit from `envRefClass`, which provides useful methods like `copy()` (shown above), `callSuper()` (to call the parent field), `field()` (to the value of a field given its name), `export()` (equivalent to `as`) and `show()` (overridden to control printing). See the inheritance section in `setRefClass()` for more details.
