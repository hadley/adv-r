...
You'll need the pryr package, `devtools::install_github("hadley/pryr")`, to access useful functions for examining OO properties.
...
In S3, methods belong to functions, called __generic functions__, or generics for short. S3 methods do not belong to objects or classes. This is different from most other programming languages, but is a legitimate OO style.
...
Given a class, the job of an S3 generic is to call the write S3 method. You can recognise S3 methods by their names, which look like `generic.class()`. For example, the Date method for the `mean()` generic is called `mean.Date()`, and the factor method for `print()` is called `print.factor()`. 
...
1.  Internal generics don't dispatch on the implicit class of base types.
    ...
    and `g` are different in the example below. What function helps 
...
An object is not very useful without some behaviour defined by __methods__. RC methods are associated with a class and can modify its fields in place. In the following example, note that you access the value of fields with their name, and modify them with `<<-`. You'll learn more about `<<-` in [Environments](#binding).
...
1.  To determine the OO system of an object, you use a process of eliminate.
    If `!is.object(x)`, it's a base object. If `!isS4(x)`, it's S3. It 
