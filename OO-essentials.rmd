...
All S4 related code is stored in the `methods` package. This package is always available when you're running R interactively, but may not be available when you're running R when running R in batch mode. For this reason, it's a good idea to include an explicit `library(methods)` whenever you're using S4.
...
You use `is()` with one argument to list all classes that an object inherits from. You use `is()` with two arguments to test if an object inherits from a specific class.
...
S4 classes have other optional properties like a `validity` method that tests if an object is valid, and a `prototype` object that defines slot default values. See `?setClass` for more details.
