...
All S4 related code is stored in the `methods` package. This package is always available when you're running R interactively, but may not be available when you're running R from the command line. For this reason, it's a good idea to include an explicit `library(methods)` whenever you're using S4.
