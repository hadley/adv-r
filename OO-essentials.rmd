...
Most objects that you encounter are S3 objects. You can check with `is.object(x) & !isS4(x)` (the second expression tests whether it's _not_ an S4 object). An easier way to determine the OO system of an object is to use `pryr::otype()`:
