...
* Classes have formal definitions which describing their fields and inheritance structures (parent classes).

* Method dispatch can be based on multiple, not just single, arguments to a generic function.

* There is a special operator, `@`, for extracting fields (aka slots) from an S4 object.

All S4 related code is stored in the `methods` package. This package is always available when you're running R interactively, but may not be when you're running R from the command line. For this reason, it's a good idea to include an explicit `library(methods)` whenever you're using S4.

S4 is a rich and complex system. So there's no way to explain it fully in a few pages. Here I'll focus on the key ideas underlying S4 so you can use existing S4 objects effectively. To learn more, some good references are:
...
You can determine the class of an S4 object with `class()`. You can test if an object inherits from a specific class with `is()`:
...
You can get a list of all S4 generics with `getGenerics()`, and a list of all S4 classes with `getClasses()`. Note that this list includes shim classes for S3 classes and base types. You can list all S4 methods with `showMethods()`, optionally restricting selection either by `generic` or by `class` (or both). It's also a good idea to supply `where = search()` to restrict search to methods available in the global environment.

### Defining classes and creating objects

In S3, you can turn any object into an object of a particular class just by setting the class attribute. S4 is much stricter: you must define the representation of a class with `setClass()`, and create an new object with `new()`. You can find the documentation for a class with a special syntax: `class?className`, e.g. `class?mle`. 

An S4 class has three key properties:

* a __name__: an alpha-numeric class identifier. Note that S4 class names should
  use UpperCamelCase.

* a named list of __slots__ (fields) which provides slot names and
...
S4 classes have other optional properties like a `validity` method that tests if an object is valid, and a `prototype` method that defines default slot values. See `?setClass` for more details.
