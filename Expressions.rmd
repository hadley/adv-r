...
In [metaprogramming](#metaprogramming), you learned the basics of accessing the expressions underlying computation in R, and evaluating them in new ways. In this chapter, you'll learn more about the underlying structure of expressions, and how you can compute on them directly.
...
* __calls__ represent the action of calling a function, not its result. These are suffixed with `()`.  The arguments to the function are listed below it, and can be constants, names or other calls.
    ...
    As mentioned in [Functions](#functions), even things that don't look like function calls still follow this same hierarchical structure:
...
Note that the second example produces the name ` `a b` `: the backticks are the standard way of escape non-standard names in R. (And are explained more in [Functions](#functions))
