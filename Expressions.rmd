...
There are two components of an expression: constants, names, calls and pairlists.
...
* __calls__ represent the action of calling a function. Calls are recursive
  ...
  `x[[i]]`, not its result. `ast()` prints `()` at the top-level of call,
...
* __pairlists__, short for dotted pair lists, and a holdover of R's past.
  They are only used in one place: for the formal arguments of a function.
  `ast()` prints `[]` at the top-level of a pairlist. Arguments are always
  named.
...
Together, constants, names, calls and pairlists define the structure of all R code. The following two sections provides more detail about names, calls and pairlists.
...
Pairlists are a holdover from R's past. They behave identically to a list, but have a different internal representation (as a linked list rather than a vector). Pairlists have been replaced by the list everywhere except in function arguments.

The only place you need to care about the difference between a list and a pairlist is if you're going to construct functions by hand by constructing and then evaluating a call to `function()`. The following functions allows you to construct a function from its component pieces: a list of formal arguments, a body and an environment. It uses `as.pairlist()` to convert a list to a pairlist.
...
One useful application of `make_function()` is in anaphoric functions.

### Anaphoric functions

An [anaphoric](http://en.wikipedia.org/wiki/Anaphora_(linguistics)) function is a function that uses a pronoun. This is easiest to understand with an example using an example from base R: `curve()`.`curve()` draws a plot of the specified function, but interestingly you don't need to use a function, you just supply an expression that uses `x`:
...
Here `x` plays the role of pronoun: it doesn't represent a single concrete value, but instead is a placeholder that varies over the range of the plot. Note that it doesn't matter what the value of `x` outside of `curve()` is: the expression is evaluated in a special environment where `x` has a special meaning:
...
One way to implement a function like `curve()` would be to use `make_function()`:
