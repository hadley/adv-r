...
* __constants__ are length one atomic vectors, like `"a"` or `10`.
  `ast()` displays them as is.
    ...
    Note that quoting an length one atomic vector returns it unchanged:
...
* __names__ which represent the name of an object, not its value. Names are
  also called symbols. Names are prefixed with a backtick by `call_tree()`.
...
* __calls__ represent the action of calling a function. Calls are recursive
  like lists. The first element is the function to call, and the remaining
  elements are the arguments to that function. `ast()` prints `()` at the
  top-level of call, then lists the children.
...
* __pairlists__, short for dotted pair lists, are a holdover of R's past.
...
Together these four components define the structure of all R code. The following two sections provides more detail about names, calls and pairlists.
...
Modelling functions often use `match.call()` to capture the call used to create the model. This makes it possible to `update()` a model, re-fitting the model after modifying a selection of the original parameters. Here's an example of `update()` in action:
...
This function is also available in pryr, where it does a little extra checking. `make_function()` is best used in conjunction with `alist()`, the **a**rgument list function. `alist()` doesn't evaluate its arguments, meaning that `alist(x = a)` shorthand for `list(x = quote(a))`.
...
# To have an argument with no default, you still need an explicit =
...
Here `x` plays the role of pronoun: it doesn't represent a single concrete value, but instead is a placeholder that varies over the range of the plot. It doesn't matter what the value of `x` outside of `curve()` is: the expression is evaluated in a special environment where `x` has a special meaning.

One way to implement `curve()` would be to use `make_function()`:
...
1.  What's the difference between `alist(a)` and `alist(a = )`?
    Think about both the input and the output.

1.  Read the documentation and source for `pryr::partial()`. What does it
    do? How does it work?

1.  Read the document and source for `pryr::unenclose()`. What does it do and
    how does it work? Why does it use `substitute2()` and not `substitute()`?
    Can you modify modify `unenclose` so it only substitutes in atomic vectors,
    not more complicated objects?
...
While it's always better to work with call trees, sometime you don't get a call tree, but instead get a string that represents some code. You can convert calls back and forth between text with `parse()` and `deparse()`. You've seen `deparse()` already it: takes an expression and returns a character vector. `parse()` does the opposite: it takes a character vector and returns an expression object.
...
1.  What are the differences between `source()` and `sys.source()`?
...
To write functions like `findGlobals()` and `checkUsage()` you'll need a new tool.  Because calls have a tree structure, the natural tool for the job is a recursive function. The key to writing recursive functions is getting the recursion right, which means making sure that you know what the base case is (the leaves of the tree) and figuring out how to combine the results from the recursive case. For calls, there are two base cases (atomic vectors and names) and two recursive cases (calls and pairlists). This usually means that a recursive function will look like:
...
The nodes of a tree are always calls (except in the rare case of function arguments, which are pairlists), and the leaves are names, single argument calls or constants. The following examples show you how to create recursive functions that work with calls.
...
`TRUE` is parsed as a logical vector of length 1, while `T` is parsed as a name. This allows us to write our base cases for the recursive function: an atomic vector is never a logical abbreviation, and for names we test the possible abbreviation. The recursive cases can be combined because we do the same thing in either case: recursively call `logical_abbr()` on each element of the object.
...
Assignment is a call where the first element is the name `<-`, and the name being assigned to is the second element. This makes the base cases simple: constants and names don't create assignments so we return NULL. We have two other cases: we have hit a call, in which case we should check if it's `<-`, otherwise it's some other recursive structure and we should call the function recursively on each element.
...
This function works for these simple cases, but the output is rather verbose and includes some extraneous `NULL`s. Instead of returning a list, let's keep it simple and stick with a character vector. We'll also test it with two slightly more complicated examples:
...
The main difference between this and the previous recursive functions is that after we process each element of calls and pairlists we need to coerce them back to the original type.
...
These tools are somewhat similar to Lisp macros, as discussed in [Programmer’s Niche: Macros in R](http://www.r-project.org/doc/Rnews/Rnews_2001-3.pdf#page=10) by Thomas Lumley. However, macros are run at compile-time, which doesn't have any meaning in R, and only return modified code. Functions in R are somewhat like [fexprs](http://en.wikipedia.org/wiki/Fexpr) a fexpr is like a function where the arguments aren't evaluated by default; or a macro where the result is a value, not code. It's useful to know these names because then you can learn more about how they are used in other programming languages and bring some of the ideas to R.
