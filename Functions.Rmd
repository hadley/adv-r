...
The rule gets a little more complicated when a name is bound to a function and a non-function in different environments. When you use a name in function call, R will ignore non-function objects while looking for the value. For example, here `n` takes on two different values:
...
This behaviour is a consequence of lazy evaluation. The `x` argument is evaluated once `g1()` is called, and then its value is cached. 
