...
Or you could save results to intermediate variables:
...
The magrittr package [@magrittr] provdes `%>%` (called the pipe, pronounced "and then"), which provides a third option:
...
`x %>% f()` is equivalent to `f(x)`; `x %>% f(y)` is equivalent `f(x, y)`. The pipe is related to __tacit__ or __point-free programming__[^point-free]. In this style of programming, you don't explicitly refer to variables. Instead, you focus on the high-level composition of functions rather than the low-level flow of data; the focus is on what's being done (the verbs), not on the objects being modified (the nouns). This style is common in Haskell and F# (the main inspiration for magrittr), and is the default style in stack based programming languages like Forth and Factor. \index{point-free programming} \index{tacit programming}

[^point-free]: Point-free programming is related to point-free topology, it's coincidental than many forms of point-free progrmaming use `.` extensively. Point-free programming is sometimes humorously called pointless programming.

Each of the three options has strengths and weaknesses:

* Nesting, `f(g(x))`, is concise, and well suited to short sequences. Longer 
  sequences are hard to read because you read from inside-out, from right to 
  left, and arguments can be spread out over long distances (the "[Dagwood 
  sandwhich](https://en.wikipedia.org/wiki/Dagwood_sandwich)" problem.
  
* Intermediate objects, `y <- f(x); g(y)`, forces each intermediate to be 
  named. This is a strength when objects are important, but a weakness when
  values are truly intermediate 

* Piping, `x %>% f() %>% g()`, is read from left-to-right, and doesn't require
  intermediate objects be named. But you can only use it to express sequences
  of transformation of a single object, and it requires an additional package 
  and the that reader understand piping.

Most code will use a combination of all three styles. Piping is more common in data analysis code, much of an analysis consists of a sequence of transformations of an object (like a data frame or plot). I tend to use piping in frequently in packages; not because it is a bad idea, but because it's often a less natural fit.
...
But using the same name for two different things will make confusing code, and is best avoided!
...
The subtlety here is that the value of `x` will be captured not when you call `capture()`, but when you call the function that `capture()` returns:
