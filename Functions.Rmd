...
The rule gets a little more complicated when a name is bound to a function and a non-function that lie in different environments. When you use a name in a function call, R will ignore non-function objects while looking for that value. For example, here `g09` takes on two different values:
