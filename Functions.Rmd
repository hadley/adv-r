...
There are also a variety of __special forms__ like `[[`, `if`, and `for`. These are built-in to R; you can only add new forms by modifying R's parser, which means it can only be done by R-core, and you have to make sure the new form doesn't add any new ambiguities.

\indexc{`} \index{backticks|see{\texttt{`}}}

### Rewriting to prefix form

An interesting feature of R is that underlying any infix, replacement or special form, is a regular prefix function. You can can rewrite any R expression to use prefix form by wrapping the function name in 
backticks, `` ` `` so that following pairs of calls do exactly the same thing:
...
And you can override any function, even special forms. If you ever are feeling particularly evil, run the following code while your friend is away from their computer:
...
Of course, this is usually a bad idea, but it is possible to apply it only selected code blocks. This provides a clean and elegant approach to writing domain specific languages and translators to other languages, as you'll learn about in [metaprogramming].
...
Prefix functions can have any number of arguments; infix functions must have exactly two. R comes with a number of built-in infix operators: `:, ::, :::, $, @, ^, *, /, +, -, >, >=, <, <=, ==, !=, !, &, &&, |, ||, ~, <-, <<-`. You can also create your own infix functions must start and end with `%`, and base R uses this to additionally define `%%`, `%*%`, `%/%`, `%in%`, `%o%`, and `%x%` functions.
...
Finally, there are a bunch of language features that have their own special forms but behind the scenes can be considered as ordinary (prefix) functions:

* Parentheses: `()`, `{}`
* Sbsetting: `x[]`, `x[[]]`, 
* Control flow: `if`, `for`, `while`, `repeat`, `next`, `break`, `switch`
* Function: `function`
