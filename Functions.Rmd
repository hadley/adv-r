...
In R, functions are also objects. This means the scoping principles that apply to any other object also apply to functions:
...
The rule gets a little more complicated when a function and a non-function, which reside in different environments, share the same name. When you use a name in a function call, R will ignore non-function objects while looking for that value. For example, here `g09` takes on two different values:
