...
They have typeof `builtin` or `special`:
...
It looks like both `x` and `size` are required, but in fact if the latter is not supplied, `sample()` uses `missing()` to provide a default. If I were to rewrite sample, I'd use an explicit `NULL` to indicate that `size` is not required but can be supplied:
...
Functions can have a special argument `...` (pronounced dot-dot-dot). With it, a function can take any number of additional arguments. In other programming languages, this type of argument is often called a _varargs_ (variable arguments), and a function that uses it is said to be variadic. 
...
Infix functions get their name from the fact the function name comes between its arguments, and hence have two arguments. R comes with a number of built-in infix operators: `:`, `::`, `:::`, `$`, `@`, `^`, `*`, `/`, `+`, `-`, `>`, `>=`, `<`, `<=`, `==`, `!=`, `!`, `&`, `&&`, `|`, `||`, `~`, `<-`, and `<<-`. You can also create your own infix functions that start and end with `%`. Base R uses this pattern to define `%%`, `%*%`, `%/%`, `%in%`, `%o%`, and `%x%`.
...
The names of infix functions are more flexible than regular R functions: they can contain any sequence of characters except the percent symbol (%). You will need to escape any special characters in the string used to define the function, but not when you call it:
...
I say they act like they modify their arguments in place, because, as explained in Section \@ref(modify-in-place) about modify-in-place behaviour, they actually create a modified copy. We can see that by using `tracemem()`:
