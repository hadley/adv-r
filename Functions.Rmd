...
What happens to values between invocations of a function? What happens the first time you run a function? What happens the second time?[^answer4] (If you haven't seen `exists()` before: it returns `TRUE` if there's a variable with that name, otherwise it returns `FALSE`.)
