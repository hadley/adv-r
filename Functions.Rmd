...
To determine if an argument value comes from the user or from a default, you can use `missing()`:
...
It looks like both `x` and `size` are required, but in fact `sample()` uses `missing()` to provide a default for `size` if it's not supplied. If I were to rewrite sample myself[^sample], I'd use an explicit `NULL` to indicate that `size` is not required but can be supplied:

[^sample]: Note that this only one way to implement a call to `sample()`: you can also call it by using a single integer, like `sample(10)`. This unfortunately makes `sample()` prone to silent errors in situations like `sample(x[i])`
...
You can simplify this task with the help of the infix `%||%` function, which uses the LHS if it's not `NULL`, and the RHS otherwise:
