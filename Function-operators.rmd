...
In this chapter, we'll explore four types of functionality offered by function operators (FOs):

* __add behaviour__. While leaving the function otherwise unchanged, FOs can automatically log when the function is run, ensure that a function is run only once, and delay the operation of a function.

* __change output__. FOs can return different values depending on whether a function throws an error, or negates the result of a logical predicate.

* __change input__. FOs can modify inputs like partially evaluating a function, convert a function that takes multiple arguments to one that takes a list, or automatically vectorise a function.

* __combine functions__. FOs can combine the results of predicate functions with boolean operators, or compose multiple function calls.
