...
vapply(models, function(x) coef(x)[[1]], double(1))

sapply(split(mtcars, mtcars$cyl), function(x) coef(lm(mpg ~ wt, x))[[1]])
```
-->
...
|-----------------------|----------|---------------------------------|-------------|-----------|
| One argument          | `map()`  | `map_lgl()`, ..., `map_chr()`   | `modify()`  | `walk()`  |
| Two arguments         | `map2()` | `map2_lgl()`, ..., `map2_chr()` | `imodify()` | `walk2()` |
| One argument + index  | `imap()` | `imap_lgl()`, ..., `imap_chr()` | ---         | `iwalk()` |
| N arguments           | `pmap()` | `pmap_lgl()`, ..., `pmap_chr()` | ---         | `pwalk()` |
...
Just as there are three basic ways to use a for loop, there are three basic ways to use `map()`:

```{r, eval = FALSE}
map(xs, function(x) {})
map(seq_along(xs), function(i) {})
map(names(xs), function(nm) {})
```
...
Useful way to take a function that works with two elements, and generalise it to work with `n` elements.
