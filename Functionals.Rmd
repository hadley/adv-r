...
[^Map]: Not to be confused with `base::Map()`, which is considerably more complex and we'll come back to in Section \ref(pmap). `purrr::map()` is equivalent to `base::lapply()`.

The implementation of `map()` is quite simple. We allocate a list the same length as the input, and then fill in the list with a for loop. The simplest implementation is only a handful of lines of code:
...
The real `purrr::map()` function is written in C to eke out every last iota of performance, preserves names, and has a few helpers that you'll learn about shortly.

::: sidebar
You might wonder why this function is called `map()`: what does it have to do with depicting physical features of land or sea `r emo::ji("world_map")`? In fact, the meaning here comes from mathematics where map refers to "an operation that associates each element of a given set with one or more elements of a second set". This makes sense here because `map()` defines a mapping from one vector to another. ("Map" also has the nice property of being short, which useful for such a fundamental building block.)
:::

### Producing atomic vectors

`map()` returns a list. It's the most general function because anything can go in a list. But what if you want something simpler? You can use `map_lgl()`, `map_int()`, `map_dbl()` and `map_chr()` to return an atomic vector of the specified type and the same length as the input. 
...
The function arguments are a little quirky but allow you to refer to `.`, `.x` and `.y.` or `..1`, `..2`.

There are other shorthands that are useful when extracting elements from nested lists (as often arises when working with json or xml)

```{r, eval = FALSE}
map(x, "x")
# equivalent to
map(x, function(x) x[["x"]])
...
map(x, list("x", 1))
# equivalent to
map(x, function(x) x[["x"]][[1]])
```

These are powered by the `purrr::pluck()`; see the documentation for more details.

### Passing arguments with `...` {#passing-arguments}

What happens if you want to pass additional arguments along to the function. For example, you want to pass `na.rm = TRUE` along to `mean()`. You could use an anonymous function:
...
This is easiest to understand with a picture: any arguments that come after `f` in the call to `map()` after inserted in the call to `f()`:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/functionals/map-arg.png")
```

Note that `map()` is only vectorised over its first argument. That means if you supply a vector as an additional argument, it will be passed as is to `f()`. You'll learn how to split up other arguments in Sections \@ref(map2) and \@ref(pmap).
...
So far the first argument to `map()` has always become the first argument to the function. But what happens if the first argument should be constant, and you want to vary a different argument? There are two options which I'll illustrate with this example of trimming different amounts when computing the mean:
...
*   The simplest technique is to use an anonymous function:

    ```{r}
    map_dbl(trims, ~ mean(x, trim = .x))
    ```
    
    I recommend this technique. Just note that when you put arguments 
    inside an anonymous function they will be evaluate every time, not once
    as when you supply to `map()`. Compare the following calls:
...
*   Sometimes, if you want to be (too) clever, you can take advantage of R's 
    ...
    Generally, I don't recommend this technique as it relies on the reader
...
Because the design of purrr you can combine 
