...
We can build a function on top of `mem()` to tells us how memory changes during the execution of a block of code. We use a little non-standard evaluation to make the code behave in the same way as running it directly. Positive numbers represent an increase in the memory used by R, and negative numbers represent a decrease.
...
This explanation is a slight oversimplification: the memory is not actually freed as soon as the function is run, but as soon as garbage collection is run. Our `mem_change()` calls `gc()`, explicitly triggering it and making a process that is normally lazy happen immediately. In ordinary, R might hold onto the memory after the function has terminated, only releasing it when it's needed.
