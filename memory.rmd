...
We'll start with a surprising plot: a line plot of vector length vs. memory size (in bytes) for an integer vector. You might have expected that the size of an empty vector would be 0 and that the memory usage would grow proportionately with length. Neither of those things are true!
...
While `object.size()` tells you the size of a single object, `gc()` tells you the total size of all objects in memory (among other things):
...
R breaks down memory usage into Vcells (memory used by vectors) and Ncells (memory used by everything else). However, neither this distinction nor the "gc trigger" and "max used" columns are typically important. What we're usually most interested in is the the first column: the total memory used. 

The function below wraps around `gc()` to return the total amount of memory (in megabytes) that R is currently using.
...
Don't expect this number to agree with the amount of memory reported by your operating system:

* Some overhead associated with the R interpreter is not captured.

* Both R and the operating system are lazy: they won't reclaim memory until it's actually needed. So R might be holding on to memory because the OS hasn't yet asked for it back.

* R counts the memory occupied by objects. Thus, there may be gaps due to deleted objects. This problem is known as memory fragmentation.

We can build a function on top of `mem()` to tells us how memory changes during the execution of a block of code. We use a little special evaluation to make the code behave in the same way as running it directly. Positive numbers represent an increase in the memory used by R, and negative numbers represent a decrease.
...
In some languages, you have to explicitly delete unused objects for their memory to be returned. R uses an alternative approach: garbage collection (or GC for short). GC automatically releases memory when an object is no longer used. It does this based on environments and the regular scoping rules: when an environment goes out of scope (for example, when a function finishes executing), all of the contents of that environment are deleted and their memory is released.
...
This is a bit of an oversimplification: in order to find out how much memory is available, our `mem()` function actually calls `gc()`. As well as returning the amount of memory currently used, `gc()` also triggers garbage collection. Garbage collection normally happens lazily: R calls `gc()` when it needs more space. In reality, R might hold onto the memory after the function has terminated. But once it's needed, it will be released.
...
* We can make a more efficient `as.data.frame()` when the input is a list by using special knowledge about the structure of a data frame. A data frame is a list with class `data.frame` and special attribute `row.names`. `row.names` is either a character vector with length matching the columns, or when the row names are sequential integers, the row names are stored in a special format created by `.set_row_names()`. This leads to an alternative `as.data.frame()`:
