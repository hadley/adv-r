...
A solid understanding of R's memory management helps you predict how much memory you'll need for a given task and helps you make the most of the memory you have. It can even help you write faster code (accidental copies are a major cause of slow code). The goal is this chapter is to help you understand the basics of memory management in R, moving from individual objects to functions to large blocks of code. Along the way, you'll learn about some common myths like that you need to call `gc()` to free up memory, or that for loops are always slow.
...
* [Memory usage and garbage collection](#gc) introduces you to the `mem_used()`
  and `mem_changed()` functions that will help you understand how R allocates 
  and frees memory.
...
In this chapter, we'll use tools from the pryr and lineprof packages to understand memory usage, and a sample dataset from ggplot2. Run this code to get the packages you need:
...
(This function is better than the built-in `object.size()` because it does a better job accounting for shared elements within an object and includes the size of environments.)
...
```{r size-q, fig.height = 4, fig.width = 6}
...
If you're keeping count you'll notice that only this adds up to 36 bytes. The remaining 4 bytes are used for padding so that the pointers start on 8 byte (= 64-bit) boundaries. Most cpu architectures require pointers to be aligned in this way, and even if they don't require it, accessing non-aligned pointers tends to be rather slow. (If you're interested, you can read more about it in [C structure packing](http://www.catb.org/esr/structure-packing/).)
...
```{r size-a, fig.height = 4, fig.width = 6}
...
`y` isn't three times as big as `x` because R is smart enough to not to copy `x` three times, instead it just points to the same location in memory. This makes it complicated to say exactly how much memory an object needs. 

It's also misleading to look at the sizes of `x` and `y` individually. If you want to know how much space they take up together, you have to supply them to the same `object_size()` call:
...
The same issue also comes up with strings, because R has a global string pool. This means that each unique string is only stored in one place, and therefor character vectors take up less memory than you might expect:
...
1.  If a data frame has 1 million rows, and three variables (two numeric, and 
    one integer), how much space will it take up? Work out it out from theory, 
...
This explanation slightly oversimplifies what happens. Memory is not automatically released after a function is run. It's released when garbage collection occurs. In the above example, because `mem_change()` explicitly calls `gc()`, `mem_change()` triggers garbage collection and forces what is normally a lazy process to run immediately. More typically, R holds onto the memory after the function has terminated, and releases it only when needed.
...
Generally, GC takes care of releasing previously used memory. However, you do need to be aware of possible memory leaks. You might think you've removed all references to an object, but if some are still hanging around, the object will never get freed. In R, the two main causes of such memory leaks are formulas and closures because they both capture the enclosing environment. The following code illustrates the problem. `f1()` returns the object `10`, so the large vector allocated inside the function will go out of scope and get reclaimed. The net memory change will be 0. `f2()` and `f3()` both return objects that capture environments. Here, the net memory change is almost 4 megabytes.
...
Using lineprof is straightforward. `source()` the code, apply `lineprof()` to an expression, then use `shine()` to view the results. Note that you _must_ use `source()` to load the code: you can not do this on the command line. This is because lineprof uses srcrefs to match up the code and run times, and the needed srcrefs are only created when you load code from disk.
...
1. `read_delim()` only takes around half a second, but profiling can, at best, 
   capture memory usage every 1ms. This means we'll only get about 500 samples.
...
Note that `refs()` is only an estimate. It can only distinguish between one and more than one reference (future versions of R might do better). This means that `refs()` returns 2 in both of the following cases:
...
It's slightly more useful for interactive use than `refs()`, but, because it just prints a message, it's harder to program with. I don't use it in this book because it interacts poorly with [knitr](http://yihui.name/knitr/), the tool I use to interleave text and code.
...
This behaviour was substantially more problematic prior to R 3.1.0, because every copy of the dataframe was a deep copy. This made the motivating example take around 5s, compared to 0.01s that it takes today.
