...
A solid understanding of R's memory management will help you predict how much memory you'll need for a given task and help you to make the most of the memory you have. It can even help you write faster code because accidental copies are a major cause of slow code. The goal is this chapter is to help you understand the basics of memory management in R, moving from individual objects to functions to larger blocks of code. Along the way, you'll learn about some common myths like that you need to call `gc()` to free up memory, or that for loops are always slow.
...
```{r size-q, fig.height = 2.5, fig.width = 5}
...
```{r size-a, fig.height = 2.5, fig.width = 5}
...
`y` isn't three times as big as `x` because R is smart enough to not to copy `x` three times, instead it just points to the existing `x`. 
...
>>>>>>> 73b89efdec0c62ebd9901576cbcbcdd4b757d274
...
It does this by tracking how many names point to each object, and when there are no names point to an object, it deletes that object. 
...
This behaviour was substantially more problematic prior to R 3.1.0, because every copy of the dataframe was a deep copy. This made the motivating example take around 5s, compared to 0.01s today.
