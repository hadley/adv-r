...
But this leads to a lot of duplication - each function is almost identical apart from a different function name. A better approach would be to modify our original `lapply()` call to include the argument:
...
Of course it's also possible to generate our list of functions that remove missing values in a more functional way, avoiding repetition.
