...
library(pryr)
source("_plugins/options.R"); set_knitr_options()
```

# Functional programming

R, at it's heart, is a functional programming (FP) language. This means that it provides many tools for the creation and manipulation of functions. In particular, R has what's known as first class functions. That means that you can do anything with functions that you can do with vectors: you can assign them to variables, store them in lists, pass them as arguments to other functions, create them inside functions and even return them as the result of a function.

The chapter starts by showing a motivating example, removing redundancy and duplication in code used to clean and summarise data. Then you'll learn about the three building blocks of functional programming: anonymous functions, closures (functions written by functions), and lists of functions. These pieces are twined together in the conclusion which shows how to build a suite of tools for numerical integration, starting from very simple primitives. This is a recurring theme in FP: start with small, easy-to-understand building blocks, combine them into more complex structures, then apply them with confidence.

The discussion of functional programming continues in the following two chapters: [functionals](#functionals) explores functions that take functions as arguments and return vectors as output, and [function operators](#function-operators) explores functions take functions are input and return them as output.
...
* [Anonymous functions](#anonymous-functions) shows you a side of functions
  that you might not have realised existed: you can use functions without 
  giving them a name.

* [Closures](#closures) introduces the closure, a function written by another
  function. A closure can access both its own arguments, and any variable 
  defined in its parents.

* [Lists of functions](#lists-of-functions) demonstrates that you can put
  functions in a list, and illustrates why you might want to.
...
Make sure you've installed the pryr package; `install_github("hadley/pryr")`. You'll also need to be familiar with the basic rules of lexical scoping, as described in [lexical scoping](#lexical-scoping).
...
Imagine you've loaded a data file, like the one below, that uses -99 to represent missing values. You want to replace all the -99s with `NA`s.
...
When you first started writing R code, you might have solved the problem with copy-and-paste, approaching the problem as a series of repetitive tasks:
...
One problem with copy-and-paste is that it's easy to make mistakes. Can you spot the two in the block above? These mistakes are inconsistencies that arose because we didn't have a authorative description of the desired action (replace -99 with `NA`). Duplicating an action makes make bugs more likely and makes it harder to change code. For example, if the code for a missing value changes from -99 to 9999, you'd need to make the change in multiple places.
...
This reduces the scope of possible mistakes, but it doesn't eliminate them: you can no longer accidentally type -98 instead of -99, but you can still mess up the name of variable. The next step is to remove this possible source of error by combining two functions. One function, `fix_missing()` knows how to correct a single vector; the other, `lapply()` knows how to do something to each column in a data frame.
...
We apply `lapply()` to this problem because because data frames are lists. We just need a neat little trick to make sure we get back a dataframe, not a list. Instead of assigning the results of `lapply()` to `df`, we'll assign them to `df[]`. R's usual ensuring we get a data frame, not a list. (If this comes as a surprise, you might want to read [subsetting and assignment](#subsetting-and-assignment).) Putting these pieces together gives us:
...
This code has fives advantages over the previous code:
...
* It works regardless of the number of columns in the data frame. There is
  no way to accidentally miss a column.
...
The key idea is composition. Take two simple functions, one which does something to every column and one which fixes missing values, and combine them to fix missing values in every column. Writing simple functions that can be understood in isolation and then composed is a powerful technique.
...
This chapter discusses these techniques in more detail. But before you can start learning those techniques, you need to start with a simple FP tool, the anonymous function.

## Anonymous functions

In R, functions are objects in their own right. They aren't automatically bound to a name. Unlike many languages (e.g. C, C++, Python and Ruby), R doesn't have a special syntax for creating a named function: when you create a function, you use the regular assignment operator to give it a name. If you choose not to give the function a name, you get what's called an __anonymous function__. You use an anonymous function when it's not worth the effort to give it a name:
...
You can call an anonymous functions without giving it a name, but the code is a little tricky to read because you must use parentheses in two different ways: first, to call a function, and second to make it clear that you want to call the anonymous function itself, as opposed to calling a (possibly invalid) function _inside_ the anonymous function:
...
1. Given a function, like `"mean"`, `match.fun()` lets you find a function. 
   Given a function, can you find it's name? Why doesn't that make sense in R?
...
One use of anonymous functions is to create small functions that are not worth naming. Another important use is to create closures, functions written by functions. Closures get their name because they __enclose__ the environment of the parent function and can access all its variables. This is useful because it allows us to have two levels of parameters: a parent level that controls operation and a child level that does the work.. The following example shows how you can use this idea to generate a family of power functions in which a parent function (`power()`) creates two child functions (`square()` and `cube()`).
...
This environment normally disappears once the function finishes executing, but because a function is returned, the environment is captured and attached to the new function. Each time we re-run `power()` a new environment is created, so each function produced by `power()` is independent. (This has important consequences for memory use, see [memory usage](#gc) for details.)

In R, almost every function is a closure. All functions remember the environment in which they were created, typically either the global environment, if it's a function that you've written, or a package environment, if it's a function that someone else has written. The only exception are primitive functions, which call C code directly.

Closures are useful for making function factories, and are one way to manage mutable state in R.

### Function factories

We've already seen two example of function factories, `missing_fixer()` and `power()`. A function factory is a factor for making new functions. You call it with arguments that describe the descired actions, and it returns a function that will do the work for you. For `missing_fixer()` and `power()`, there's not much benefit is using a function factory instead of a single function with multiple arguments. Function factories are most useful when:
...
Function factories are particularly well suited to maximum likelihood problems, and you'll see a more compelling use of them in [mathematical functionals](#mathematical-functionals).

### Mutable state

Having variables at two levels allows you to maintain state across function invocations. This is possible because while the function environment is refreshed every time, the parent environment is constant. The key to managing variables at different levels is the double arrow assignment operator (`<<-`). Unlike the usual single arrow assignment (`<-`) that always assigns in the current environment, the double arrow operator will keep looking up the chain of parent environments until it finds a matching name. ([Environments](#environments) has more details on how it works.)
...
The new function is a closure, and its enclosing environment is the environment created when `new_counter()` is run. Ordinarily, function execution environments are temporary, but a closure maintains access to the environment it was created in. In the example below, closure `counter_one()` and `counter_two()` each get their own enclosing environemnt are run, so they can maintain different counts.
...
You can use environment inspection tools to see what's going on:
...
Modifying values in a parent environment is an important technique because it is one way to generate "mutable state" in R. Mutable state is normally hard to achieve. This is because every time it looks like you're modifying an object, you're actually creating and then modfiying a copy. However, if you do need mutable objects and your code is not super simple, it's usually better to use the RC OO system. RC objects are easier to document, and provide easier ways to inherit behaviour. RC objects are described in more detail in [the OO field guide](#rc).
...
1.  Create a function `pick()`, that takes an index, `i`, as an argument and 
    returns a function with an argument `x` that subsets `x` by `i`.
...
In R, functions can be stored in lists. In the same way a data frame makes it easier to work with groups of related vectors, this makes it easier to work with groups of related functions.
...
Another use for a lists of functions is to summarise an object in multiple ways. To do that, we could store each summary function in a list, and then run them all with `lapply()`:
...
To conclude this chapter, I'll develop a simple numerical integration tool using many properties of first-class functions. Each step in the development of the tool is driven by a desire to reduce duplication and to make the approach more general.
...
Neither of these functions gives a very good approximation. To make them more accurate using the idea that underlies calcululs: we'll break up the range into smaller pieces and integrate each piece using one of the simple rules. This is called __composite integration__. I'll implement it using two new functions:
...
Mathematically, the next step in improving numerical integration is to move from a grid of evenly spaced points to a grid where the points are closer together near the end of the range, such as __Gaussian quadrature__. That's beyond the scope of this case study, but you could implemenet it with similar techniques.
...
1.  The tradeoff between integration rules is that more complex rules are 
    slower to compute, but need fewer pieces. For `sin()` in the range [0, pi],
...
1.  For each of the Newton-Cotes rules, how many pieces do you need to get 
    within 0.1% of the true answer for `sin()` in the range [0, pi]. Write a 
    function that determines this automatically for any function. (Hint: Look 
    at `optim()` and construct a one-argument function with closures.)
