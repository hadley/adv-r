...
If multiple inputs have the same output, you can leave the right hand side of `=`  empty and the input will "fall through" to the next value. This mimics that behaviour of C's `switch` statment:
...
`perform_action` is called once for each value in `vector`, rebinding the value of `index` each time.
...
(When iterating over integers, it's conventional to use very short indices like `i`, `j`, or `k`.)
...
There are three common problems that you might encounter when using `for`. First, if you're generating data, make sure to preallocate the output. Otherewise the loop will be very slow; see Sections \@ref(memory-profiling) and \@ref(avoid-copies) for more details. The `vector()` function is helpful here.
...
You can rewrite any `for` loop to use `while` instead, and you can rewrite any `while` loop to use `repeat`, but the converses are not true. That means `while` is more expressive than `for`, and `repeat` is more expressive than `for`. It's good practice to use the least-expression solution to a problem, which means that you should use `for` where possible.
...
1.  What does the following code tell you about whe the index is updated?
