...
For each item in `vector`, `perform_action` is called once; rebinding the value of `item` each time.
...
* `next` stops the current iteration and advances to the next one.
* `break` simply exits the entire for loop.
...
This occurs because `:` works with both positive and increasing sequences:
