...
NB: `for` assigns the index in the current environment, so that an existing variable with the same name will be overwritten:
...
You can rewrite any `for` loop to use `while` instead, and you can rewrite any `while` loop to use `repeat`, but the converses are not true. That means `while` is more expressive than `for`, and `repeat` is more expressive than `while`. It's good practice to use the least-expressive solution to a problem, which means that you should use `for` where possible.
