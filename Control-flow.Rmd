...
(When iterating over a vector of indicies, it's conventional to use very short variable names like `i`, `j`, or `k`.)

Note that `for` assigns `item` in the current environment, so that an existing variable with the same name will be overwritten:
