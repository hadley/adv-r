...
Or if you have a character string giving a slot name with the `slot` function:
...
`getSlots` will return a description of all the slots of a clas:
...
How does this actually work?  There is a global object that stores class definitions. `setClass` creations class object and stores in that object.
...
To find methods for an S4 object, use `showMethods`:
...
Common methods that you may want to define for your objects are:

  * `show`: the equivalent of the S3 `print` method.
  * `plot`: for plotting your object.
