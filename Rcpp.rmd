...
When you run this code, Rcpp will compile the C++ code and construct an R function that connects to the compiled C++ function. We're going to use this simple interface to learn how to write C++. C++ is a large language, and there's no way to cover it all in just one chpater. Instead, you'll get the basics so that you can start writing useful functions to address bottlenecks in your R code. 
...
With doubles, you may be able to get away with ignoring missing values and working with NaNs (not a number). This is because R's NA is is a special type of IEEE 754 floating point number NaN. So any logical expression that involves a NaN always evaluates as FALSE:
...
While C++'s `bool` has two possible values (true or false), a logical vector in R has three (TRUE, FALSE and NA). So, if you coerce a length 1 logical vector, make sure it doesn't contain any missing values otherwise they will be converted to TRUE.
...
Rcpp provides a lot of syntactic "sugar" to ensure that C++ functions work very similarly to their R equivalents. In fact, Rcpp sugar makes it possible to write efficient C++ code that looks almost identical to its R equivalent. If there's a sugar version of the function you're interested in, you should use it: it'll both expressive and well tested. Sugar functions aren't always faster than a hand-written equivalent, but they will get faster in the future as more time is spent on optimising Rcpp.
...
The sugar function `any()` and `all()` are fully lazy so that `any(x == 0)`, for example, might only need to evaluate one element of a vector, and return a special type that can be converted into a `bool` using `.is_true()`, `.is_false()`, or `.is_na()`. We could also use this sugar to write an efficient fucntion to determine whether or not a numeric vector contains any missing values. To do this in R, we could use `any(is.na(x))`:
...
Our C++ `any_naC()` function is slightly slower than `any_naR()` when there is either no missing values or a missing value is at the end. But it's much faster when the first value is missing.
...
The real strength of C++ reveals itself when you implement more complex algorithms. The standard template library (STL) provides a set of extremely useful data structures and algorithms. This section will explain some of the most important algorithms and data structures, and will point you in the right direction to learn more. I can't teach you everything you need to know about the STL, but hopefully the examples will show you the power of the STL, and persuade you that it's useful to learn more.
...
Iterators are used extensively in the STL: many functions either accept or return iterators. By abstracting away the details of the underlying data structure, they are the next step up from basic loops. Iterators have three main operators: they can be advanced with `++`, dereferenced (to get the value they refer to) with `*` and compared using `==`. For example we could re-write our sum function using iterators:
...
* We start at `x.begin()` and loop until we get to `x.end()`. A small 
  optimization trick is to store the value of the end iterator so we don't 
  need to look it up each time. This only saves about 2 ns per iteration, 
  so it's most useful when the calculations in the loop are very simple.

* To change the values in `out`, we can use assignment via a dereferenced iterator (`out_it`): `*it`.
...
Iterators also allow us to use the C++ equivalents of the `apply()` family of functions. For example, we could again rewrite `sum()` to use the `accumulate()` function, which takes a starting and ending iterator and adds all the values in between. The third argument to `accumulate()` gives the initial value: it's particularly important because this also determines the data type that `accumulate()` uses (so we use `0.0` and not `0` so that accumulate uses a `double`, not an `int`.). To use `accumulate()` we need to include the `<numeric>` header.
...
* We can assign a dereferenced iterator (`out_it`) to change the values 
  in `out`.

* `upper_bound()` returns an iterator. If we want the value of the 
  `upper_bound()` we could dereference it. To figure out its location, we 
  use the `distance()` function.

* Small note: if we want this function to be as fast as R's `findInterval()` 
  ...
  that's slightly faster than R's `findInterval()`, but would end up taking 
  about 10% of the code.

It's generally better to use algorithms from the STL than to hand roll your own loops. In "Effective STL", Scott Meyer gives three reasons: efficiency, correctness and maintainability. Because algorithms from the STL are written by C++ experts, they're extremely efficient, and because they have been around for a long time, they are also well-tested. Using standard algorithms also makes the intent of your code clearer, which ultimately makes it more readable and easier to maintain. 

### Data structures {#data-structures-rcpp}

The STL provides a large set of data structures: `array`, `bitset`, `list`, `forward_list`, `map`, `multimap`, `multiset`, `priority_queue`, `queue`, `dequeue`, `set`, `stack`, `unordered_map`, `unordered_set`, `unordered_multimap`, `unordered_multiset`, and `vector`. The most important are `vector`, `unordered_set`, and `unordered_map`. We'll focus on them in this section. All the other structures work similarly to those three, so learning to use them is pretty straightforward. They just have different performance tradeoffs. For example, `deque` (pronounced "deck") has a very similar interface to `vector` but a different implementation that has different performance trade-offs. You may want to experiment with them for your specific problem. A good reference for STL data structures is <http://www.cplusplus.com/reference/stl/> - I recommend you keep this page open while working with the STL.

Rcpp knows how to convert many STL data structures to their R equivalents. This means that there's no need to explicitly convert them to R data structures. You can simmply return them from your function.

### Vectors

An STL vector is very similar to an R vector, except that it grows efficiently. This makes vectors appropriate to use when you don't know in advance how big the output will be. Vectors are templated, which means that you need to specify the type of object the vector will contain when you create it: `vector<int>`, `vector<bool>`, `vector<double>`, `vector<String>`. You can access individual elements of a vector using the standard `[]` notation, and you can add a new element to the end of the vector by using `.push_back()`. If you have some idea in advance how big the vector will be, you can use `.reserve()` to allocate sufficient storage.

The following code implements run length encoding (`rle()`). It produces two vectors of output: a vector of `values` and a vector of `lengths` which contains the frequency of each value. It works by looping through the input vector `x` comparing each element to the previous one: if it's the same, then it increments the last value in `lengths`; if it's different, it adds the value to the end of `values`, and sets the corresponding length to 1.
...
(An alternative implementation would be to replace `i` with the iterator `lengths.rbegin()` which always points to the last element of the vector - you might want to try implementing this yourself.)

Other vector methods are described at <http://www.cplusplus.com/reference/vector/vector/>.

### Sets

Sets maintain a unique set of values. They can efficiently tell if they've seen a value before. They are useful for problems that involve duplicates or unique values (like `unique`, `duplicated`, or `in`). C++ provides both ordered (`std::set`) and unordered sets (`std::unorded_set`), depending on your needs. Unordered sets tend to be much faster (because they use a hash table rather than a tree). So even if you need an ordered set, you should consider using an unordered set and then sorting the output. Like vectors, sets are templated, so you need to request the appropriate type of set for your purposes: `unordered_set<int>`, `unordered_set<bool>`, etc. More details are available at <http://www.cplusplus.com/reference/set/set/> and <http://www.cplusplus.com/reference/unordered_set/unordered_set/>.

The following function uses an unordered set to implement an equivalent to `duplicated()` for integer vectors. Note the use of `seen.insert(x[i]).second`. `insert()` returns a pair, the `.first` value is an iterator that points to an element and the `.second` value is a boolean that's true if the value is a new addition to the set.
...
A map is similar to a set, but besides storing presence or absence, it can store additional data. It's useful for functions like `table()` or `match()` that need to look up a value. As with sets, there are ordered (`std::map`) and unordered (`std::unorded_map`) versions. Since maps have a value and a key, you need to specify both types when initialising a map: `map<double, int>`, `unordered_map<int, double>`, and so on. The following example shows how you could use a `map` to implement `table()` for numeric vectors:
...
Note that unordered maps are also only available in C++ 11, so to use them, you'll again need `[[Rcpp::plugins(cpp11)]]`.

### Exercises

To practice using the STL algorithms and data structures, implement the following R functions in C++. Use the hints provided.

1. `median.default()` using `partial_sort`.

1. `%in%` using `unordered_set` and the `find()` or `count()` method.
