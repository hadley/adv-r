...
* In C++, vectors indices start at 0. I'll say this again because it's so 
...
If you're familiar with roxygen2, you might wonder how this relates. `Rcpp::export` controls whether a function is exported from C++ to R, where `@export` controls whether a function is exported from a package and made available to the user.
...
What type of object does an R function return? We don't know, so we use the catch-all type `RObject`. An alternative is to return a `List`. For example, the following code is a basic implementation of `lapply` in C++:
...
With doubles, you may be able to get away with ignoring missing values and working with NaNs (not a number). This is because R's NA is is a special type of IEEE 754 floating point number NaN. So any logical expression that involves a NaN (or in C++, NAN) always evaluates as FALSE:
