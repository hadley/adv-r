...
You'll also (obviously!) need a working C++ compiler. To get it:
...
C++ is a large language, and there's no way to cover it all in just one chpater. The aim here is to give you the basics so you can start writing useful functions that allow you to address bottlenecks in your R code. We won't spend much time on advanced features like object oriented programming or templates. Our focus is on writing small, self-contained function, not on writing big programs.

The following sections will teach you the basics of C++ by translating simple R functions to their C++ equivalents. We'll start simple with a function that has no inputs and a scalar output, and then get progressively more complicated:
