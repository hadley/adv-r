...
The STL provides a large set of data structures: `array`, `bitset`, `list`, `forward_list`, `map`, `multimap`, `multiset`, `priority_queue`, `queue`, `dequeue`, `set`, `stack`, `unordered_map`, `unordered_set`, `unordered_multimap`, `unordered_multiset`, and `vector`. The most important of these are `vector`, `unordered_set`, and `unordered_map`. We'll focus on these three in this section, but using the others work in similar fashion: they just have different performance tradeoffs. For example, `deque` (pronounced "deck") has a very similar interface to `vector` but a different implementation that has different performance trade-offs. You may want to experiment with them for your specific problem. A good reference for STL data structures is <http://www.cplusplus.com/reference/stl/> - I recommend you keep this page open while working with the STL.

Rcpp knows how to convert many STL data structures to their R equivalents. So you can return them from your functions without explicitly converting to R data structures.
