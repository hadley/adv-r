...
The following illustrate some real life case studies of using C++ to replace slow R code.

### Gibbs sampler

The following case study updates an example from Dirk Eddelbuettel's [blog](http://dirk.eddelbuettel.com/blog/2011/07/14/). It illustrates the conversion of code for a Gibbs sampler from R to C++. The R and C++ code, shown below, are very similar (in fact, the conversion only took a few minutes), but runs about 20 times faster on my computer. Dirk's blog post also shows how using the faster random number generator functions in GSL (easily accessible from R through the RcppGSL package) can add a further 2-3x improvement.
...
This conversion to C++ is straightforward.
...
If you're familiar with R, your gut will tell you that this will be slow. Indeed it is. There are two ways we could attack this problem. If you have a good R vocabulary, you might immediately see how you can use use `ifelse()`, `pmin()` and `pmax()` to vectorise the function. Alternatively, using our knowledge that loops and function calls have much lower overhead in C++, we could rewrite `vacc1a()` and `vacc1()` in C++.
...
Not surprisingly, our original approach with loops is very slow. Vectorising in R gives a huge speedup, but we can eke out even more performance (~10x) with the C++ loop. I was a little surprised that the C++ code was so much faster, but the reason for the difference is that the R version has to create 11 vectors to store intermediate results, whereas the C++ code only needs to create 1.
...
* Make sure your `NAMESPACE` includes:
    ...
    We need to import something (actually, anything) from Rcpp so that internal 
    Rcpp code is properly loaded. This is a bug in R and hopefully will be fixed 
    in the future.
...
This chapter has only touched the surface of Rcpp. It gives you the basic tools to rewrite poorly performing R code in C++. The [Rcpp book](http://www.rcpp.org/book) is single best reference to learn more about Rcpp. As noted, Rcpp has many other capabilities that make it easy to interface R to existing C++ code. These include:

* Additional features of attributes including specifying default arguments,
  linking to external C++ dependencies, and exporting C++ interfaces to 
...
* [C++ Annotations](http://www.icce.rug.nl/documents/cplusplus/cplusplus.html) 
  is aimed at "knowledgeable users of C (or any other language using a C-like 
