...
There are three ways to handle conditions signalled by other code. You can ignore them, you can handle the, or you can observe them.
...
`tryCatch()` is a general tool for handling conditions. It allows you to map conditions to __handlers__, functions that are called with the condition as an input. `tryCatch()` defines __exiting__ handlers; after the condition is captured control returns to the context where `tryCatch()` was called. This means that it's generally most suitable for working with errors, as errors have to exit the code anyway. In the next section, you'll learn about `withCallingHandlers()` which plays a similar role to `tryCatch()` for other types of condition.
...
An alternative to `tryCatch()` is `withCallingHandlers()`. The difference between the two is that the former establishes *exiting* handlers while the latter registers *in-place* handlers. 

This makes `withCallingHandlers()` particularly suitable for working with messages and warnings.
...
* The handlers in `withCallingHandlers()` are called in the context of the
  ...
  leading to the current function.
    ...
    tryCatch(f(), error = function(e) rlang::calltrace(globalenv()))

    withCallingHandlers(f(), error = function(e) rlang::calltrace(globalenv()))
    ```
