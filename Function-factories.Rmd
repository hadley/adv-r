...
* Section \@ref(stat-fact) uses function factories to tackle three challenges from
  ...
  problems, and drawing boostrap resamples. 
...
In other words, the primary interface is a function factory. At first glance, this seems like this adds extra complexity for little gain. But this makes for a nice interaction with with ggplot2 scales, because they accept functions in the `label` argument:
...
Here each facet has the same number of observations, but the variability is very different. It would be nice if we could request that the binwidth vary so we get approximately the same number of points in each bin. One way to do that is with a function factory that inputs the desired number of bins (`n`), and outputs a function that takes a numeric vector and returns a binwidth:
...
All these examples can be tackled without function factories, but I think function factories are a good fit for these problems and admit elegant solutions. These examples expect some statistical background, so feel free to skip if they don't make much sense to you.
...
Function factories are a useful way for thinking of bootstrapping because instead of thinking about a single bootstrap (you always need more than one!), you than think about a bootstrap __generator__, a function that yields a fresh boostrap every time it is called:
...
To finish off the chapter, I'll show how you might combine functionas and function factories to turn data into many functions. The following code creates many specially named power functions by iterating over a list of arguments:
...
They're currently trapped inside the list, but it's easy to assign into the environment of your choice:
