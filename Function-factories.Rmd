...
Our first motivating examples for function factories comes from statistics, as I think they're a good match for solving maximum likelihood problems, and working with bootstraps. In both cases, you can solve without function factories, but I think they're a good fit to the problems and lead to slightly more elegant solutions.
...
The goal of maximum likelihood estimation (MLE) is to find the "most likely" parameters of a distribution given some data. To do MLE, you start with a probability function. Let's work through a simple example, using the Poisson distribution. For example, if we know $\lambda$ we can compute the probability of getting a vector of `x` values ($x_1$, $x_2$, ..., $x_n$) by multiplying the Poisson probability function as follows:

\[ P(\lambda, \mathbf{x}) = \prod_{i=1}^{n} \frac{\lambda ^ {x_i} e^{-\lambda}}{x_i!} \]

In statistics, we almost always work with the log of this function. The log is a monotonic transformation which preserves important properties (i.e. the maxima are in the same place), but has two important properties:

* Logs turn products in to sums, which make it possible to radically simplify
  the probability function.

* Multiplying small numbers yields smaller numbers, which can cause numerical
  complications when working with floating point math. Logged values are much
  larger.
...
The key idea of maximum likelihood is to think about this function in a different way. So far we've been thinking of `lambda` as fixed and known, and the function tells us the probability of getting different `x` values. But in real-life, we have the `x` and we want to figure out the `lambda`. With MLE, we say, given `x`, what value of `lambda` gives us the highest value of `lprob_poisson()`? 
...
We can try it out and see how likely it is to have come from different values of `lambda`. 
...
The likelihood is the probability function, just seen through a different lens: we want to fix `x` and find the `lambda` that yields the highest result. In R, we can make this distinction more clear by using a function factory. We provide `x` and generate a function with a single parameter, `lambda`:
...
Now we can use this function to find the highest value of `lambda`:
...
Rather than trial and error, we can automate the process of finding the best value with `optimise()`. The results tells us that the highest value is `-30.27` which occurs when `lambda = 32.1`:
...
These advantages get bigger for more complex MLE where you have multiple parameters and multiple data vectors.
