...
The execution environment of a function is usually ephemeral. But functions bind their enclosing argument, and that reference will keep the execution environment alive until the child function is garbage collected.

This property makes function factories work, but requires a little care when you have more complicated code as it's easy to keep variables alive that you don't care about. In the following code, note that `f2()` is so large because it's environment contains the million element vector `x`:
