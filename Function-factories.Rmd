...
* In Section \@ref(the-function-environment), you learned that a function
  captures, or encloses, the environment in which it is created.
...
In other words, the primary interface is a function factory. At first glance, this seems to add extra complexity for little gain. But it enables a nice interaction with ggplot2 scales, because they accept functions in the `label` argument:
...
Function factories are a useful approach for bootstrapping. Instead of thinking about a single bootstrap (you always need more than one!), you can think about a bootstrap __generator__, a function that yields a fresh boostrap every time it is called:
...
They're currently trapped inside the list, but it's easy to assign them into the environment of your choice:
