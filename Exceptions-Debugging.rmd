...
* Use errors, i.e., `stop()`, to force functions to terminate and ensure that there is no way for them to continue.

* Use warnings, i.e., `warning()`, to display potential problems. For example, when some elements of a vectorised input   are invalid, like `log(-1:2)`.

* Use messages, i.e., `message()`, to give informative but easily suppresible, with `?suppressMessages()`, output to   
  the user. For example, I often use messages to let the user know what value a function has chosen for an important 
  missing argument.
