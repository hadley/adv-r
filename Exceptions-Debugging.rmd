...
* By raising a fatal error with `stop()`. Fatal errors force all execution to stop, 
...
Sometimes when a function you know that there are 
...
A handler function can do anything, but typically it will either return a value, or pass the condition along, by re-signalling the error with `stop(c)`, or `signalCondition(c)` for non-error conditions. 
...
## Conditions, restarts, and calling handlers

R offers an exceptionally power exception system, but it's not particularly well document. If you want to learn more about the internals, I recommend the following two primary sources:
...
but to understand how it works, we first need to learn a little about conditions, the S3 objects that represent errors, warnings and messages.
