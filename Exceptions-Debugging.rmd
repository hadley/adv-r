...
Debugging is the art and science of fixing unexpected problems in your code. In this section you'll learn the tools and techniques that help you get to the root cause of an error. You'll learn general strategies for debugging, as well as RStudio and R specific tools like `traceback()` and `browser()`. 

Not all problems are unexpected. When writing a function, you can often anticipate potential problems (like a nonexistent file or the wrong type of input). Communicating these problems to the user is the job of __conditions__: errors, warnings and messages.

* Use errors, i.e., `stop()`, to force functions to terminate and ensure that 
  there is no way for them to continue.

* Use warnings, i.e., `warning()`, to display potential problems. For example, 
  when some elements of a vectorised input are invalid, like `log(-1:2)`.

* Use messages, i.e., `message()`, to give informative but easily suppresible, 
  with `?suppressMessages()`, output to the user. For example, I often use 
  messages to let the user know what value a function has chosen for an 
  important missing argument.

Conditions are usually displayed prominently in bold or red depending on your R interface. You can tell them apart because, true to their nature, errors always start with "Error" and warnings with "Warning message". The authors of functions can also communicate problems to users with `print()` or `cat()`. But I think that's a bad idea. It's hard to capture and selectively ignore this sort of output. Moreover, since printed output is not a condition, so you can't use any of the useful condition handling tools you'll learn about below.

Condition handling tools, like `try()`, `tryCatch()` and `withCallingHandlers()`, allow you to take specific actions when a condition occurs. For example, if you're fitting many models, you might want to continue fitting the others even if one fails to converge. R offers an exceptionally powerful condition handling system based on ideas from Common Lisp, but it's currently neither very well documented nor often used. This chapter will introduce you to the most important basics, but if you want to learn more, I recommend the following two sources:

* [A prototype of a condition system for R](http://homepage.stat.uiowa.edu/~luke/R/exceptions/simpcond.html) by Robert Gentleman and Luke Tierney. This describes an early version of R's condition system. While the implementation has changed somewhat since this was written, it provides a good overview of how the pieces fit together, and some motivation for its design.
...
3. __Figure out where it is__
    ...
    idea to adopt the scientific process. Generate hypotheses, design 
