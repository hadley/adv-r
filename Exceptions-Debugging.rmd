...
* Use `stop()` to force functions to terminate and ensure that there is no way 
  for them to continue.

* Use `warning()`to display potential problems. For example, when some elements 
  of a vectorised input are invalid, like `log(-1:2)`.

* Use `message()` to give informative but easily suppresible, with 
