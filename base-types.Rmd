...
In the next chapter you'll also learn about the `class()` function. For S3 objects, `class()` returns the class attribute. For base objects, `class()` returns the first element of its __implicit__ class, which you'll learn more about in Section \@ref(implicit-class).
...
**GVW: I think the para below is the first use of the word "dispatch".  Should it be defined in OO.Rmd or defined here?
