...
*   The vectors, Chapter \@ref(vectors-chap): `NULL`, `logical`, `integer`, 
    `double`, `complex`, `character`, `list`, `raw`.
...
*   Functions, Chapter \@ref(functions): `closure` (regular R functions), 
    `special` (internal functions), `builtin` (primitive functions).
...
*   Environments, Chapter \@ref(environments): `environment`.

    ```{r}
    typeof(globalenv())
    ```

*   Language components, Chapter \@ref(expressions): `symbol` (aka names), 
    `language` (usually called calls), and `pairlist` (used for function arguments).
...
*  `S4`, Chapter \@ref(S4), which are used for S4 classes that don't inherit 
   from an existing  base type.

*  There are a few esoteric types that are important for C code but not 
   generally available at the R level: `externalptr`, `weakref`, `bytecode`,
   `promise`, `...`, and `any`.
