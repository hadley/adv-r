...
You can also reach out to the community for help. Stackoverflow can be a useful place to ask, but you'll need to put some effort into creating an example that captures the salient features of your problem while being easily digestible. If it's too complex few people will have the time and motivation to attempt a solution. But if you make the problem too simple you'll get answers that only solve your toy problem.

The remainder of this chapter is my attempt to give some general advice on writing faster R code. I've tried to identify the most generally applicable techniques:
...
Two other techniques are less commonly used but are good to know about:

1. Byte-code compile
1. Trade space for time

Another extremely useful technique is re-writing bottlenecks in a faster language like C++. This is the subject of the [Rcpp](#rcpp) chapter.

Don't forget that clarity and readibility are more important than speed. Your intution for bottlenecks is likely to be bad, so don't sacrifice readability for performance unless you know it will have a significant impact on run-time.

### Code organisation

When experimenting with alternative strategies in order to get faster code you will need to be organised.

* Create a representative test case. This should ideally take a couple of
...
* Know the right answer. As you work on faster alternatives, you'll need
  some way to check that they're correct.

* Record everything you try. I recommend writing a function for each
  variation that you try. It can be useful to work in a Rmarkdown file
  so that you can easily mingle your code and your thoughts/explanations/
  comments.

* Use the microbenchmark package to compare run times. Make the `times`
  paramater smaller so that it only takes a couple of seconds to run.
  You'll need to run the code multiple times to make sure that there
  really is a different, and the microbenchmark package will take care
  of this for you.
...
One of the easiest ways to speed up your code is to find existing faster code. You are part of a large community, and it's quite possible that someone else has had the same problem as you.

If your bottleneck is in another package, two good places to start looking at:
...
Otherwise the challenge is describing your bottleneck in a way that's likely to find related problems. You will be aided if you know what the name of the problem is, and some common synonyms. It's hard to build up this knowledge, but general reading about statistics and algorithms can help. Make sure you know good ways of restricting your search to R. For google, try [rseek](http://www.rseek.org/); for stackoverflow, restrict your searches to the R tag with `[R]`.

Don't just look for solutions that are already faster than your code. Also look for alternative approaches. These might be slower to start with, but it's possible that by using the next techniques they'll end up faster than your original technique. You may also be able to combine the fastest parts of different approaches to get something faster than the individual pieces.

### Do as little as possible

The most important technique for making code faster is to make it do less work. This is my most commonly used optimisation trick. The easiest way to do less work is use a more specific function. For example, `vapply()` is usually faster than `sapply()`, because you tell type of the output to expect in advance. Instead of using `apply()`, use `rowSums()`, `colSums()`, `rowMeans()`, and `colMeans()` if you can.
...
Other base functions can be made to do less work by giving them more informatiion about the problem. Often if you read the documentation carefully, you'll find ways to make the function faster. For example, to make `read.csv()` faster you can tell it up front what sort of columns you expect with `colClasses`.
...
A riskier way to do less is to make stronger assumptions about the input and do less error checking. For example, we can make `mean()` considerably faster by stripping away the error checking code and calling the internal C function directly:
...
`mean()` is safest but slowest. `mean.default()` assumes that `x` is a numeric vector, and will fail in possibly surprising ways if not. `.Internal(mean(x))` assumes that the vector is numeric and has no missing values.

Also note that the speed-up is closely related to the length of the vector. If we make the input a hundred times bigger, the difference barely matters. This is why you shouldn't optimise unless you've correctly identified the code. You may save hardly any time at the cost of introducing bugs.
...
This technique for rewriting base R function is very useful. Often base R functions take many different types of input and spends time figuring out what input you've given it. If you know in advance the type of input, rewrite the function to expect that.

Or perhaps the function produces a rich output structure, and you're just extracting a single piece. You might be able to make the code much faster by only computing the bits that you need.
...
Working with indices instead of with data frames is about 8 times faster. Data frames are one of the slowest data structures in R, so if you can avoid using them do it. One way is to work with columns instead.

```{r}
boot_col_indices <- function(df, col, i) {
  df[[col]][sample.int(nrow(df), i, replace = TRUE)]
}
boot_col <- function(df, col, i) {
  sample(df[[col]], i, replace = TRUE)
}

microbenchmark(
  indices = mean(mtcars$cyl[boot_col_indices(mtcars, "cyl", 10)]),
  rows = mean(boot_col(mtcars, "cyl", 10))
)
```
