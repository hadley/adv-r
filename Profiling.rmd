...
* don't repeatedly paste together strings, or `cbind()` columns, or
  `c()` vectors

* `df$x <- y` copies entire data frame.
...
* Work with the simplest object possible. Data frames are particularly
  ...
  Instead of splitting a data frame, split a vector of
...
* If you're fitting many linear models (for example bootstrap replicates),
  `lm()` is quite slow. Use `model = FALSE` to avoid copying the input data
  in. Look at the biglm package. Try `RcppArmadillo::fastLm()`.
...
microbenchmark(
  factor(x, letters[1:5]),
  my_factor(x, letters[1:5])
)
```
