...
Instead of looking at the individual records, we'll aggregate and display them using the lineprof package. There are a number of ways to visualise this data. For example, `summaryRprof()`, the proftools and the profr package. These are useful, but relatively sophisticated tools. I think the lineprof package is most useful, because it shows you performance information in the same context as your code. When profiling, you want to find the slowest parts of your code and understand their context. lineprof is relatively unsophisticated (e.g. doesn't deal well with recursion) but is great at showing you context.

To use lineprof, we'll first put the code above in a separate file.  We do this because it makes it easier to see the context of the code (normally this step isn't necessary because your code is already in a file). We then use `lineprof()` to run out function and capture the timing output. As the name suggests, the fundamental unit of analysis in `lineprof()` is a line of code. This is a bit more complex than a function call, but it's easier to see the code in its context. If you have a lot going on one line, you probably want to break it up into multiple steps anyway. Printing this object shows that we haven't exactly captured how long it takes each to run each function, but the ratios look about right. For now, we'll just focus on the time column which estimates how long each line took to run, and the ref, which tells us which line of code was run.

```{r, messsage = FALSE, cache = TRUE}
library(lineprof)
...
Lineprof does provide some tools to navigate through this data structure (like `focus()`), but we'll instead work with an interactive explorer which can be run with `shine(l)`. `shine()` starts a shiny app which will "block" your R session. To exit, you'll need to stop the process using escape or ctrl + break. `shine()` will also open a new web page (or if you're using RStudio, a new pane) that shows your source code annotated with information about how long each line took to run:

```{r, echo = FALSE}
png("profiling-lineprof-f.png", dpi = 220)
```

This view shows how much time is spend on each line of `f()`. At a glance, you can see that twice as much time is `spent()` in `g()` as `h()`, so that it would make sense to drill down into that function to find a bottleneck.

You can click on the function names to drill down into `g()`:
...
* you only see the top-level C/C++ function, not what functions they call

* primitive functions or byte code compiled code. 

* anonymous functions can be hard to track down (because they don't have names)
