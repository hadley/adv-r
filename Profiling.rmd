...
lineprof provides some funtions to navigate through this data structure, but they're a bit clumsy. Instead, we'll start an interactive explorer using the shiny package. `shine(l)` will open a new web page (or if you're using RStudio, a new pane) that shows your source code annotated with information about how long each line took to run. `shine()` starts a shiny app which "blocks" your R session. To exit, you'll need to stop the process using escape or ctrl + c. 
...
1.  Make a faster version of `chisq.test()` that only computes the Chi-square
    ...
    simpler, or with the formal [definition][chi-square].
...
There is one special case of vectorisation. With matrix algebra, loops are executed by highly tuned external libraries like BLAS. So, if you can figure out a way to use matrix algebra to solve your problem, you'll often get a very fast solution. Unfortunately, the ability to recognise that your problem can be solved using matrix algebra is a product of experience. While this skill is something you'll develop over time, a good place to start may be to ask for help from those with experience in your domain.
...
Modifying an object in a loop, e.g. `x[i] <- y`, can also create copies, depending on `x`'s class. [Modification in place]{#modification-in-place} discusses this problem in more depth and gives you tools to determine when you're making copies. Data frames are particularly bad offenders.
