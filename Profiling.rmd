...
* [Other techniques](#more-techniques) finished the chapter with pointers to
  more resources that will help you write fast code.

##### Prerequisites

In this chapter we'll be using the `lineprof` package to understand the performance of R code. Get it with `devtools::install_github("hadley/lineprof")`.

## Measuring performance {#measure-perf}

To understand performance, you use a profiler. There are a number of different types of profiler. R uses a fairly simple type called a sampling or statistical profiler. A sampling profiler stops the execution of code every few milliseconds and records which function is currently executing (along with function called that function, and so on). For example, consider `f()`, below:
...
If we profiled the execution of `f()`, stopping the execution of code every 0.1s, we'd expect a profile like below. Each line represents one "tick" of the profiler (0.1s in this case). You can see the code spends 0.1s running `f()`, then 0.2 running `g()`, then 0.1s running `h()`.
...
Use this test case to quickly check that all variants return the same result. An easy way to do so if with `stopifnot()` and `all.equal()`. For real problems with fewer possible outputs, you may need more tests to make sure that an approach doesn't accidentally return the correct answer. That's unlikely for the mean.
...
(You might be surprised by the results: `mean(x)` is considerably slower than `sum(x) / length(x)`. This is because, among other resons, `mean(x)` makes two passes over the vector to be more numerically accurate.)
...
The easiest way to make a function faster is to let it do less work. One way to do that is use a function tailored to a more specific type of input, ouput or more specific problem. For example:
...
First, I take output of `diff` and reformat it to my style:
...
Next, I assume assume vector input. This allows me to remove the `is.matrix()` test and the method that use matrix subsetting.
...
Now `diff4()` is both consider simpler, and considerably faster than `diff1()`:
...
R 2.13.0 introduced a byte code compiler which can increase the speed of some code. Using the compiler is an easy way to get improvements in speed. Even if doesn't work well for your function, you won't have invested a lot of time in the effort. The following example shows the pure R version of `lapply()` from [functionals](#lapply). Compiling it gives a considerable speedup, although it's still not quite as fast as the C version provided by base R.
...
What I want to show is a simple application of parallel computing to what are called "embarrassingly parallel problems". An embarrassingly parallel problem is one that's made up of many simple problems that can be solved independently. A great example of this is `lapply()` because it operates on each element indepdent of the others. It's very easy to parallelise `lapply()` on Linux and the Mac because you simply substitute `mclapply()` for `lapply()`. The following code snippet runs a trivial (but slow) function on all cores of your computer.
...
* Read other R programming books, like Norm Matloff's
  ["The Art of R Programming"](http://amazon.com/1593273843) or Patrick Burns'
