...
* [Selecting multiple elements](#subset-multiple) starts by teaching you about `[`. You'll start
...
* [Selecting a single element](#subset-single) expands your knowledge of 
  subsetting operators to include `[[` and `$`, focusing on the important
...
It's easiest to learn how subsetting works for atomic vectors, and then how it generalises to higher dimensions and other more complicated objects. We'll start with `[`, the most commonly used operator which allows you to extract any number of elements. [Selecting a single element](#subset-single) will cover `[[` and `$`, used to extract a single element from a data structure.
...
To help with visualization, the decimal points of our example's values give the original position in the vector.

There are five things that you can use to subset a vector: 

*   __Positive integers__ return elements at the specified positions, or indices: 
...
*   __Logical vectors__ select elements where the corresponding logical 
    ...
    x[x > 3]                            # Note that here 3 represents the value, not the index
    ```				
...
The most common way of subsetting matrices (2d) and arrays (>2d) is a simple generalisation of 1d subsetting: you supply a 1d index for each dimension, separated by a comma. Blank subsetting is now useful because it lets you keep all rows or all columns (or all dimensions).
...
By default, `[` simplifies the results to the lowest possible dimension. See [simplifying vs. preserving](#simplify-preserve) to learn how to avoid this.

Because matrices and arrays are implemented as vectors with special attributes, you can subset them with a single vector. In that case, they behave like a vector. Arrays in R are stored in column-major order:
...
You can also subset higher-dimensional data structures with an integer matrix (or, if named, a character matrix). Each row in the matrix specifies the location of one value, where each column corresponds to a dimension in the array being subset. This means that you can use a 2 column matrix to subset a matrix, a 3 column matrix to subset a 3d array, and so on. The result is a vector of values:
...
By default, any subsetting 2d data structures with a single number, single name, or a logical vector containing a single `TRUE` will simplify the returned output as described below. To preserve the original dimension, you must use `drop = FALSE`
...
| `pluck(col, row)` | Zero-length | OOB (int)  | OOB (chr) | Missing  |
|-------------------|-------------|------------|-----------|----------|
| `NULL`            | `NULL`      | `NULL`     | `NULL`    | `NULL`   |
...
The behaviour of `pluck()` makes it well suited for indexing into deeply nested data structures where the component you want does not always exist (this is common when working with JSON data from web APIs). `pluck()` also allows you to mingle integer and character indexes, and to provide an alternative default value if the item does not exist:
...
With lists, you can use `[[` + assignment + `NULL` to remove components from a list. To add a literal `NULL` to a list, use `[` + assignment + `list(NULL)`: \index{lists!removing an element}
...
`order()` takes a vector as input and returns an integer vector describing how the subset vector should be ordered: \indexc{order()} \index{sorting}
...
For two or more dimensions, `order()` and integer subsetting makes it easy to order either the rows or the columns of an object:
...
Sometimes you get a data frame where identical rows have been collapsed into one and a column count has been added. `rep()` and integer subsetting make it easy to uncollapse the data by subsetting with a repeated row index:
...
For all other cases, avoid switching from logical to integer subsetting.
