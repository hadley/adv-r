...
* the environment that's created when a function is run
...
We can access this environment using the confusingly named `parent.frame()`. This function returns the __environment__ where the function was called. We can use this to look up the value of names in the environment where the function was called.
...
Sometimes it's useful to be able to create a new scope without embedding inside a function. The `local` function allows you to do exactly that: you make temporary variables to make an operation easier to understand but want to throw them away afterwards:
