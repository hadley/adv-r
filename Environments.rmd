...
In fact, it will return the same value each and every time it is called. This is because each time a function is called, a new environment is created to host execution. We can see this by examining the results of `environment()` called within a function. That said, note that all those newly created environments have the same parent environment: the environment where the function was created:
