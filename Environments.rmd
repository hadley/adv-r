...
The environment is the data structure that powers scoping in R. This chapter dives deep into environments, describing their structure in depth, and using them to explain in detail the four rules of scoping described in [lexical scoping](#lexical-scoping).

Environments are can also be useful data structures because they have reference semantics. This is not needed most of the time, but can be handy for some problems that are hard to solve otherwise. There are also a few details of names and binding 
...
1.  How do you determine the which environment a function was called from?
...
* [Environment basics](#env-basics) introduces you to the basic properties
  of an environment and shows how to create your own.
  
* [Recursing over environments](#env-recursion) provides a useful recursive
  template for computing with environments, illustrating the idea with a useful
  function.
...
*   Environments have reference semantics. When you modify a binding in an
    ...
    [Explicit environments](#explicit-envs) discusses the
    implications.
...
`<<-` is most often used in conjunction with a closure, as described in [Closures](#closures).
...
*  Rather than assigning the result of an expression immediately, a 
  ...
  may need to use directly if you need more control. 
  
  Delayed bindings are used to implement `autoload()`, which makes R behave as 
  if the package data is in memory, even though it's only loaded from disk when
  you ask for it.
