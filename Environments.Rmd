...
Each section will begin by illustrating code for working with environments from rlang, because it allows you to focus on the essence of environments, rather than the 
...
The `global_env()`, the `base_env()`, the search-path environments, and the `emptyenv()` are connected as shown below. Each time you load a new package with `library()` it is inserted between the global environment and the package that was previously at the top of the search path.
...
Environments have reference semantics: unlike most R objects, when you modify them, you modify in place, and don't create a copy. We'll explore the fuller implications in [Explicit environments], but here I wanted to show one important implication: environments can contain themselves.

This means that environments go one step further in their level of recursion than lists. An enviroment can any object, including itself!
...
Or you can create a mutually recursive pair of environments:

```{r}
loop <- env()
loop$dedoop <- env()
loop$dedoop$loop <- loop
```
...
Generally, you should avoid creating complicated graphs (that way lies madness!) but it is sometimes useful.

### Creating and inspecting
\index{environments!creating}

To create an environment manually, use `rlang::env()`. 
...
The default print method is not very useful, so you can call `env_names()` to get the names of the contents, or `env_parent()` to find out the parent environment.
...
* `new.env()` equivalent to `env()`; but you have to assign values separately.

* `names()` equivalent to `env_names()`. You can also use `ls()` (which was
  recommended previously), but note by default `all.names = FALSE` so you
  don't see any bindings that start with `.`.
  
* `parent.env()` is equivalent to `env_parent()`.
...
You can create an environment like a list:
...
(`_poke` not `_set` because we don't return a modified environment, we modify in place.)
...
Environments form a tree, so it's often convenient to write a recursive function. This section shows you how by applying your new knowledge of environments to write a helpful function that given a name, finds the environment `where()` that name is defined, using R's regular scoping rules.

The definition of `where()` is straightforward. It has two arguments: the name to look for (as a string), and the environment in which to start the search. (We'll learn later why `caller_env()` is a good default in [calling environments](#calling-environments).)
...
The top-level `x` (bound to 20) is a red herring: using the regular scoping rules, `i()` looks first where it is defined and finds that the value associated with `x` is 10. However, it's still meaningful to ask what value `x` is associated with in the environment where `i()` is called: `x` is 10 in the environment where `i()` is defined, but it is 20 in the environment where `i()` is called.
