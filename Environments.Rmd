...
The call stack is an extremely important internal data structure. It holds exit handlers created with `on.exit()`, restarts and handlers for the the condition system, and which context to `return()` to when a function completes. The call stack is captured when there's an error and exposed in the `traceback()`. R code can only access a small part of the data structure because it's so important: making changes without using the official API could be disastrous.
