...
Most environments are not created by you (with `env()`) but are instead created by R. In this section, you'll learn about the most important environments, starting with the global and package environments. You'll then learn then about the function environment that is bound to the function when it is created, and the (usually) ephemeral execution environment that is created every time the function is called. Finally, you'll see how the package and function environments interact to support namespaces, which ensure that a package always behaves the same way, regardless of what other packages the user has loaded.
...
Working with the global environment object is equivalent to getting and setting values at the console:
...
The parent of the global environment is the last package you loaded:

```{r}
env_parent(global_env())
```

And the parent of that package is the second to last package you loaded:

```{r}
env_parent(env_parent(global_env()))
```

If you follow all the parents back, you see every package that's attached in the current session:

```{r, eval = FALSE}
env_parents(global_env())
#> [[1]]  <environment: package:rlang>
#> [[2]]  <environment: package:methods>
#> [[3]]  <environment: package:stats>
#> [[4]]  <environment: package:graphics>
#> [[5]]  <environment: package:grDevices>
#> [[6]]  <environment: package:utils>
#> [[7]]  <environment: package:datasets>
#> [[8]]  <environment: Autoloads>
#> [[9]]  <environment: base>
#> [[10]] <environment: R_EmptyEnv>
```

This is the so-called __search path__ because all objects in these environments can be found from the top-level interactive workspace. You can access the names of the 
...
The last three environments are always the same:
...
* The base environment, or `base_env()`, is the environment of the base package.
  It is special because it has to be able to bootstrap the loading of all other
  packages.
...
A function binds the current environments when it is created. This is called the __function environment__, and is used for lexical scoping. Across computer languages, functions that capture their environments are called __closures__.
...
In this case, the environment of `f()` is the same environment that binds the name `f` to the function object. But that's not always true: in the following code `g` is bound in a new environment `e`, but its environment will still be the global environment. The difference is how we find `g` vs. how `g` finds variables. This distinction is important to making namespaces work, as described below.
