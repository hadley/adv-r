...
One important application of this distinction is __namespaces__. For example, take `sd()`:
...
For example, take the base function `sd()`. Its enclosing and binding environments are different:
...
Namespaces are implemented using environments, taking advantage of the fact that functions don't have to live in their enclosing environments. 
...
What happens when we call `plus_one()`? It's execution environment will have an execution environment of `plus()` as its parent, and it will find the 
