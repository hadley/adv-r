...
\StringTok{  return out;}
\StringTok{"}\NormalTok{)}
\KeywordTok{add_one}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  return out;}
\StringTok{"}\NormalTok{)}
\KeywordTok{add_two}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\FloatTok{1e6}\NormalTok{)}
...
\hypertarget{character-vectors-and-lists}{%
...
differ primarily in how you access and name elements. \index{pairlists}
...
\NormalTok{add <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(a, b) \{}
  ...
  \KeywordTok{stopifnot}\NormalTok{(}\KeywordTok{length}\NormalTok{(a) }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{, }\KeywordTok{length}\NormalTok{(b) }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{)}
...
\NormalTok{  b <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(b)}
  ...
  \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{length}\NormalTok{(a) }\OperatorTok{>}\StringTok{ }\DecValTok{1}\NormalTok{) }\KeywordTok{warning}\NormalTok{(}\StringTok{"Only first element of a used"}\NormalTok{)}
  \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{length}\NormalTok{(b) }\OperatorTok{>}\StringTok{ }\DecValTok{1}\NormalTok{) }\KeywordTok{warning}\NormalTok{(}\StringTok{"Only first element of b used"}\NormalTok{)}
...
\texttt{PROTECT(new\ =\ coerceVector(old,\ SEXPTYPE))}. This will return
an error if the \texttt{SEXP} can not be converted to the desired type.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pryr}\OperatorTok{::}\KeywordTok{show_c_source}\NormalTok{(}\KeywordTok{.Internal}\NormalTok{(}\KeywordTok{tabulate}\NormalTok{(bin, nbins)))}
...
\NormalTok{  SEXP in = CAR(args), nbin = CADR(args);}
  \ControlFlowTok{if}\NormalTok{ (TYPEOF(in) != INTSXP)  error(}\StringTok{"invalid input"}\NormalTok{);}

\NormalTok{  R_xlen_t n = XLENGTH(in);}
  \CommentTok{/* }\AlertTok{FIXME}\CommentTok{: could in principle be a long vector */}
  \DataTypeTok{int}\NormalTok{ nb = asInteger(nbin);}
  \ControlFlowTok{if}\NormalTok{ (nb == NA_INTEGER || nb < }\DecValTok{0}\NormalTok{)}
...
\NormalTok{  memset(y, }\DecValTok{0}\NormalTok{, nb * }\KeywordTok{sizeof}\NormalTok{(}\DataTypeTok{int}\NormalTok{));}
  \ControlFlowTok{for}\NormalTok{(R_xlen_t i = }\DecValTok{0}\NormalTok{ ; i < n ; i++) \{}
    \ControlFlowTok{if}\NormalTok{ (x[i] != NA_INTEGER && x[i] > }\DecValTok{0}\NormalTok{ && x[i] <= nb) \{}
...
\item
  \texttt{SEXP\ args}: a pairlist containing the unevaluated arguments
  to the function.
\item
...
\NormalTok{tabulate3 <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(bin, nbins) \{}
\NormalTok{  bin <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(bin)}
  \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{length}\NormalTok{(nbins) }\OperatorTok{!=}\StringTok{ }\DecValTok{1} \OperatorTok{||}\StringTok{ }\NormalTok{nbins }\OperatorTok{<=}\StringTok{ }\DecValTok{0} \OperatorTok{||}\StringTok{ }\KeywordTok{is.na}\NormalTok{(nbins)) \{}
