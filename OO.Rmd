...
**GVW:** After going through this doc, base-types.Rmd, and S3.Rmd, my impression is that this material is written for people who've done OO before in some other language and are encountering generic function-style OO in R for the first time.
...
* S3 and S4 implement a paradigm different from that used in to most languages that are popular today. 
  The underlying ideas are the same, but their expression draws on a different tradition:
...
* There is disagreement about the relative importance of the three systems. 
  ...
  S4 is most important, followed by RC (a base alternative to R6), and that S3 
  should be avoided. This means that different R communities use different 
  systems.

* There are no books on the effective use of OOP in R. This book
  only helps a little because it focusses on the underlying theory and 
  implementation, and provides relatively little advice on how OOP
  helps you design and evolve complex programs.

In R generally, functional programming is much more important than object oriented programming, because generally you solve complex problems decomposing them into functions, not objects. Nevertheless, there are important reasons to learn each of the three systems:
...
* S4 is a rigorous system that forces you to think carefully about program design.
  ...
  and will receive contributions from many programmers, which is why projects like
  Bioconductor use it.
...
**GVW:** I keep stumbling over R6 vs RC: if I understand correctly, the latter is a extension of the former, but it would help to pick name early on and stick to it, even if that's not 100% accurate.

This chapter will give you a rough lay of the land along with a field guide to help you identify OOP systems in the wild. The following four chapters (Base types, S3, S4, and R6) will dive into the details, starting with R's base types. While not technically an OOP system, they are the fundamental building block of the true OOP systems.

**GVW:** I think there should be a note here to say that these chapters are about the mechanics of OOP, but _not_ about how to design programs that make use of OOP.  If readers haven't created object-oriented programs before, they should probably read the section on S3 and then come back to the rest later.
...
**GVW:** rather than calling this a brain dump, maybe say, "Different people use terms in different ways, so this section is a quick overview of how the following chapters will use them.  The explanations are necessarily compressed, but we will come back to these ideas multiple times."
