...
Now let's use `safely()` with a functional:
...
* `possibly()`: returns a default value when there's an error.
  It provides no way to tell if an error occurred or not, so it's best
...
When we memoise this function, it's slow when we call it with new arguments. But when we call it with arguments that it's seen before, it's instantaneous because it retrieves the previous value of the computation.
...
`memoise()` and `safely()` are very useful but also quite complex. In this case study you'll learn how to create your own simpler function operators. Imagine you have a named vector of URLs and you'd like to download each one to disk. That's pretty simple with `walk2()` and `download.file()`:
