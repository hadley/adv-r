...
In R, it is important to understand the distinction an object and its name. An correct mental model is important because it will help you:
...
Note that the object, or value, doesn't have a name; it's the name that has a value. To make that distinction more clear, I'll draw diagram's like this: 

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/name-value/binding-1.png", dpi = 300)
```

The name, `x`, is drawn with a rounded rectangle, and it has an arrow that points to the  value, the vector `1:3`. Note that the arrow points in opposite direction to the assigment arrow: `<-` creates a binding from the name on the left-hand side to the object on the right-hand side.
...
Names that don't follow these rules are called __non-syntactic__ names, and if you try to use them, you'll get an error:
...
Typically you won't go around creating such crazy names. Instead, it's important to understand how to use `` ` `` to escape named when working with data frames that have been created outside of R, and don't follow R's rules.
...
1.  The following code accesses the mean function in multiple different ways.
    Do they all point to the same underyling function object? Verify with
    `lobstr::obj_addr()`.
...
1.  By default, base R data import functions, like `read.csv()` will automatic
...
1.  I slightly simplified the rules that govern syntatic names. Why is `.123e1`
    not a syntactic name? Read `?make.names`.

## Copy-on-modify

Take the following code which binds `x` and `y` to the same underlying value, then modifies `y`.
...
Clearly modifying `y` also doesn't modify `x`, so what happens to the shared binding? While the value associated with `y` changes, the original object does not. Instead, R creates a new object, `0xcd2`, a copy of `0x74b` with one value changed, then rebinds `y` to that object.

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/name-value/binding-3.png", dpi = 300)
```

This behaviour is called __copy-on-modify__, and understanding it makes your intuition for the performance of R code radically better. A related way to describe this phenomenon is to say the R objects __immutable__; I'll generally avoid that term because there are a couple of important exceptions to copy-on-modify that you'll learn about in [modify-in-place]. 
...
```{r, include = FALSE}
# Needed for following block to show correct results when run from
# rmarkdown::render(). I don't know what's causing the difference.
l1 <- list(1, 2, 3)
l2 <- l1
l2[[3]] <- 4
```
...
Most of the time, modifying an R object will create a copy. There are two exceptions when R will actually modify-in-place:
...
If an object only has a single binding to it, R will modify it in plane:
...
* When it comes to bindings, R can currently[^refcnt] only count 0, 1, 
  ...
  the reference count does not get decremented.
...
Together, this make it hard to predict whether or not a copy will occur. Instead, it's better to determine empirically with `tracemem()`. Lets explore the subtleties with a case study using for loops. In R, for loops have a reputation for being slow. Often that slowness is because every cycle of the loop is creating a copy. Consider the following code. It subtracts the median from each column of a large data frame: \index{loops!avoiding copies}
...
This loop is surprisingly slow because every iteration of the loop copies the data frame. We can see that more clearly by using `tracemem()`:
...
We create two objects, but by the end of code neither object is bound to a name. How do these objects get deleted? That's the job of the __garbage collector__, or GC, for short. It walks through every object in memory and counting how many references it has. If there are no references, the object is deleted. We'll come back to this in more detail below.

### When does the garbage collector run?
\index{garbage collector} \indexc{gc()}

The garbage collector is run automatically whenever a new R object is created and R needs more memory. If you want to see when that is, call `gcinfo(TRUE)`. Running the GC will free up memory by deleting R objects that are no longer used, and if needed, requeset more memory from the operating system. 

You can force the garbage collector to run by calling `gc()`. Despite what you might have read elsewhere, there's never any need to call `gc()` yourself. You might want to call `gc()` to ask R to return memory to your operating system, or for its side-effect of telling you how much memory is currently being used:  
