...
You'll learn more about environmetns in [Environments], but it's important to mention them here because they behave different to every other object in R. Environments have reference semantics which means that they are modified in place. Take this environment, which we bind to `e1` and `e2`:
...
For technical reasons, there is unfortunately no way to tell when most R objects are garbage collected with `tracemem()`.
