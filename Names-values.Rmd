...
Objects in R don't have names; names have objects. But it's difficult to talk about things that don't have names, so in diagrams we'll give objects labels like `0x001`. These are unique identifiers which look a little bit like the memory location of objects that we'll talk about later.
...
That's because most R objects are __immutable__. That means you can't change the object; instead you create a modified copy. Now `x` refernces object `0x001` and `y` references a new object, `0x002`.
...
It's not just top-level variables that point to other objects. Lists do too. Take this list, which superficially is very similar to the vector above:

```{r}
l <- list(1, 2, 3)
```

In memory, it's actually quite different. A list is really a list of refences:
...
Like vectors, lists are immutable; you do not modify the list, but instead create a modified copy. Note that the copy is __shallow__: only the list object is copied (and modified), the individual elements are not.
...
## Mutable objects / reference semantics

Unlike most objects in R, when you modify an environment, it does not make a copy. For example, look at this `modify()` function. \index{copy-on-modify!exceptions} \index{reference semantics}
