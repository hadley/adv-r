...
It's even misleading to think about the sizes of `x` and `y` individually. If you want to know how much space they take up together, you have to supply them to the same `object_size()` call:

```{r}
object_size(x, y)
```

In this case, `x` and `y` together take up the same amount of space as `y` alone: `object_size(x) + object_size(y)` will only equal `object_size(x, y)` if there are no shared values.
...
To understand memory usage in R, we will use `pryr::object_size()`. This function tells you how many bytes of memory an object occupies: \index{object\_size()}
