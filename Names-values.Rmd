...
Note that the arrow in the diagram points in the opposite direction to the assigment arrow. That's because `<-` creates the binding, then we look up the value associated with the name in the opposite direction.

You can think of a name as a pointer. If you run code like `y <- x` we get two bindings to the same value:
...
That's because most objects are __immutable__. While the values associated with `y` change, the object does not. Instead, R creates a new object, `0xcd2`, a copy of `0x74b` with one value changed, then rebinds `y` to that object.
...
NB: because vectors are contiguous in memory, in most case, modifying a single value will require the complete vector to be copied. This is one reason that you should prefer using vectorised operations to modify multiple values at once as only one copy is needed.
...
If `f()` modified `x`, R would create a new copy, and then `z` would bind that object. There would be one copy because of the modification, but there's no copy when the object is returned from the function.
...
Like vectors, lists are immutable; the original list is left unchanged, but instead create a modified copy. Note that the copy is __shallow__: only the list object is copied (and modified), the individual elements are not. This has a big impact on performance.

### Object size

This behaviour has implications for the size of an object:
...
It's even misleading to think about the sizes of `x` and `y` individually. If you want to know how much space they take up together, you have to supply them to the same `obj_size()` call:

```{r}
obj_size(x, y)
```

In this case, `x` and `y` together take up the same amount of space as `y` alone: `obj_size(x) + obj_size(y)` will only equal `obj_size(x, y)` if there are no shared values.
...
One other surprising wrinkle is character vectors. You might think of a character vector like this:
...
But R has a global string pool, so each element of a character vector is actually a pointer to a unique string:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/name-value/character-2.png", dpi = 300)
```

This means that character vectors take up less memory than you might expect: \index{string pool}
...
Generally, however, this detail is not important, so we'll typically draw character vectors as if the strings live inside the vectors.
...
No other object in R can contain itself because all other objects are modified.

### Mononymous objects

There second exception to the copy-on-modify rule applies to all objects in R: objects that only have single binding can be modified in place. I think it's best to think of this as a performance optimisation; if an object only has one name that points to it, then this has the same behaviour as copy-on-modify, but is obviously much faster.
...
(Note that the value `0x207` is modified.)

It's challenging to predict exactyl when R applies this optimisation because of two challenges:
...
Together, this means that it's very hard to create a value that only has a single binding. We'll discuss this in detail in [Modification in place](#modification).
...
## Object size {#object-size-details}

To understand memory usage in R, we will use `lobstr::obj_size()`. This function tells you how many bytes of memory an object occupies: \index{object\_size()}
...
library(pryr)
x <- 1:10
c(pryr::address(x), pryr::refs(x))
# [1] "0x103100060" "1"

y <- x
c(pryr::address(y), pryr::refs(y))
# [1] "0x103100060" "2"
```
...
for (i in 1:10) {
  y[[i]] <- y[[i]] - medians[[i]]
  print(c(pryr::address(y), pryr::refs(y)))
}
```
