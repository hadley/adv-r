...
You should also be careful when calling C functions. Apart from the basic constructor functions and duplicate, you should never assume it's ok to modify an object in place without first checking that `NAMED(x)` is 0.  If you're doing this sort of check in many places, you may want to define the following macro:

```c
#define MAYBE_SHARED(x) (NAMED(x) > 0)
```

This will also protect you if the interface for checking if an object is shared elsewhere changes.  (Thanks to Luke Tierney for suggesting this macro and providing more details about when you need to make a copy).
...
An alternative to using `.Call` is to use `.External`.  It is used almost identically, except that the C function will recieve a single argument containing a `LISTSXP`, a pairlist from which the arguments can be extracted. This makes it possible to write functions that take a variable number of arguments.
