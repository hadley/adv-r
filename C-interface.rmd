...
Reading R source code is an extremely powerful way to improve your programming skills. However, because many base R functions, as well as many functions in older packages, are written in C, to understand how those functions work, you'll need some knowledge of C and an understanding of R's C API. You can learn the basics of C from a standard C text (e.g. "[Kernigan and Ritchie](http://amzn.com/0131101633?tag=devtools-20)") or from [Rcpp](#rcpp). With a little effort and patience, you'll be able read R's C source code. In the process, you'll really learn a lot.

The contents of this chapter draw heavily upon Section 5 ("System and foreign language interfaces") of [Writing R extensions](http://cran.r-project.org/doc/manuals/R-exts.html). However, the focus is on best practices and modern tools. This means the chapeter does not cover the old `.C` interface, the old API defined in `Rdefines.h` or other rarely used language features. You're unlikely to see these in modern R code. Moreover, I do not recommend using C to write new high-performance code. Instead, you should be using `Rcpp` to write code in C++. The Rcpp API protects you from many of the historical idiosyncracies of the R API, takes care of memory management, and provides many useful helper methods.

To see R's C API, look at the header file `Rinternals.h`. It's easiest to find and display this file from within R:
...
* [Pairlists](#c-pairlists) shows you how to work with pairlists. You need
  to know this because the distinction between pairlists and lists is more 
  important in C than in R.
...
* [Finding the C source for a function](#c-find-source) shows you how to find 
  the C source code for internal and primitive R functions.

##### Prerequisites

To understand existing C code, it's useful to generate simple examples of your own that you can experiment with. To that end, all examples in this chapter use the `inline` package, which makes it extremely easy to compile and link C code to your current R session. You can get it by running `install.packages("inline")` in your R session. To easily find the C code associated with internal and primitive functions, you'll need a function from `pryr`. Get that package by running `install_github("hadley/pryr")`
...
Generally, calling a C function from R requires two parts: a C function and an R wrapper function that uses `.Call()`. The simple function below adds two numbers together and illustrates some of the complexities of coding in C:
...
(An alternative to using `.Call` is to use `.External`. `.External` is used almost identically to `.Call`, except that the C function will receive a single argument containing a `LISTSXP`, a pairlist from which the arguments can be extracted. This makes it possible to write functions that take a variable number of arguments. However, it's not commonly used in base R and `inline` does not currently support `.External` functions so I won't discuss it further).

In this chapter we'll produce the two parts in one step by using the `inline` package. This allows us to write:
...
Before we begin reading and writing C code, we need to know a little about the basic data structures in C.
...
Character vectors are a little more complicated than the other atomic vectors. A `STRSXP`s contains a vector of `CHARSXP`s, where each `CHARSXP` points to C-style string stored in a global pool. This design allows individual `CHARSXP`'s to be shared between multiple character vectors, which reduces memory usage. See [object size](#object-size) for more details.
...
The simplest way to create a new R-level object is `allocVector()`. It takes two arguments, the type of `SEXP` (or `SEXPTYPE`) to create, and the length of the vector. The following code creates a three element list containing a logical vector, a numeric vector and an integer vector, all of length four:
...
If you run `dummy()` a few times, you'll notice the output varies. This is because `allocVector()` assigns memory to each output, but it doesn't clean it out first. For real functions, you'll may want to loop through each element in the vector and set it to a constant. The most efficient way to do that is to use `memset()`:
...
If you're working with lists, use `shallow_duplicate()` to make a shallow copy; `duplicate()` will also copy ever element of the list.
...
And helpers for the opposite direction:
...
As of R 3.0.0, R vectors can have length greater than 2 ^ 32. If you want your code to work with long vectors, this means you shouldn't write code like `int n = length(x)`. Instead use the `x_len_t` type and the `xlength()` function, and write `x_len_t = xlength(x)`.
