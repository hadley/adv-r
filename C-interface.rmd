...
If you're working with lists, use `shallow_duplicate()` to make a shallow copy; `duplicate()` will also copy every element of the list.
...
Note that some of these functions behave differently to the R-level functions with similar names. For example `isVector()` is true for atomic vectors, lists, and expressions, where `is.vector()` returns `TRUE` only if its input has no attributes apart from names.

## Finding the C source code for a function {#c-find-source}

R code in the base package doesn't use `.Call()`. Instead it uses two special functions: `.Internal()` or `.Primitive()`. Finding the source code for these functions is a somewhat arduous process (you first need to look up the C function name in `src/main/names.c` then search the R sources), but `pryr::show_c_source()` automates the process (using github code search).
