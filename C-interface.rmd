...
The simplest way to create a new R-level object is `allocVector()`. It takes two arguments, the type of `SEXP` (or `SEXPTYPE`) to create, and the length of the vector. The following code creates a three element list containing a logical vector, a numeric vector, and an integer vector, all of length four: \indexc{PROTECT()}
...
R code in the base package doesn't use `.Call()`. Instead it uses two special functions: `.Internal()` or `.Primitive()`. Finding the source code for these functions is a somewhat arduous process (you first need to look up the C function name in `src/main/names.c` then search the R sources), but `pryr::show_c_source()` automates the process (using github code search). \indexc{.Internal()} \index{primitive functions}
