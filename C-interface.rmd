...
Note that some of these functions behave differently to similarly named R-level functions. For example, `isVector()` is true for atomic vectors, lists and expressions, but `is.vector()` returns `TRUE` only if its inputs have no attributes apart from names.

## Finding the C source code for a function {#c-find-source}

In the base package, R doesn't use `.Call()`. Instead, it uses two special functions: `.Internal()` or `.Primitive()`. Finding the source code for these functions is a an arduous task: you first need to look for their C function name in `src/main/names.c` and then search the R source code. `pryr::show_c_source()` automates this task (using GitHub code search).
...
Internal and primitive functions have a somewhat different interface than `.Call()` functions. Also, while internal and primitive functions always have four arguments, `.Call()` functions can have any number: 

* `SEXP call`: the complete call to the function. `CAR(call)` gives
  the name of the function (as a symbol), while `CDR(call)` gives the arguments.
...
The final version below moves more of the coercion logic into an accompanying R function, as does some minor restructuring to make the code a little easier to understand. I also added a `PROTECT()`; this is probably missing in the original because the author knew it would be safe.
