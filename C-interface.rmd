...
To understand existing C code, it's useful to generate simple examples of your own that you can experiment with. To that end, all examples in this chapter use the `inline` package, which makes it extremely easy to compile and link C code to your current R session. Get it by running `install.packages("inline")` in your R session. To easily find the C code associated with internal and primitive functions, you'll need a function from pryr. Get the package with `install_github("hadley/pryr")`
...
(An alternative to using `.Call` is to use `.External`.  It is used almost identically, except that the C function will recieve a single argument containing a `LISTSXP`, a pairlist from which the arguments can be extracted. This makes it possible to write functions that take a variable number of arguments. However, it's not commonly used in base R and `inline` does not currently support `.External` functions so I don't discuss it further in this chapter).
...
Because inputs and output will always be R data structures (`SEXP`s), the heart of every C function you might read or write will include a conversion between an R and a C data structure. This section focusses on vectors because they're the most frequently used type of object.

An additional complication when dealing with C is garbage collection: if you don't protect every R object you create, the garbage collector will think they are unused and delete them.

### Creating vectors and garbage collection

The simplest way to create a new R-level object is to use `allocVector()`. This functions takes two arguments: the type of `SEXP` (or `SEXPTYPE`) to create and the length of the vector. The following code creates a list of three elements, each of length four, containing a logical vector, a numeric vector and an integer vector:
...
You might wonder what all the `PROTECT()` calls do. They tell R that that object is in use and to not delete it if the garbage collector is activated. (We don't need to protect objects that R already knows we're using, like function arguments.)

You also need to make sure that every protected object is unprotected. `UNPROTECT()` takes a single integer argument, `n`, and unprotects the last n objects that were protected. The number of protects and unprotects must match. If not, R will warn about a "stack imbalance in .Call".  Other specialised forms of protection are needed in some circumstances: `UNPROTECT_PTR()` unprotects the object pointed to by `SEXP`s, `PROTECT_WITH_INDEX()` saves an index of the location of protected objects. This allows us to use that index to replace the protected value with `REPROTECT()`. Consult the R externals section, [garbage collection](http://cran.r-project.org/doc/manuals/R-exts.html#Garbage-Collection), for more details.
...
If you run `dummy()` a few times, you'll notice the output varies. This is because while `allocVector()` assigns memory to each output, it doesn't first clean it out. For real functions, you should loop through each element in the vector and set them to a constant. The most efficient way to do that is to use `memset()`:
...
Missing values are somewhat more complicated for `REALSXP` because there is an existing protocol for missing values defined by the floating point standard ([IEEE 754](http://en.wikipedia.org/wiki/IEEE_floating_point)). With doubles, an `NA` is `NaN` with a special bit pattern (the lowest word is 1954, the year Ross Ihaka was born). There are other special values for positive and negative infinity. Use `ISNA()`, `ISNAN()`, and `!R_FINITE()` macros to check for missing, NaN or non-finite values. And use the constants `NA_REAL`, `R_NaN`, `R_PosInf` and `R_NegInf` to set those values.
...
Strings and lists are more complicated because the individual elements of a vector are `SEXP`s, not basic C data structures. Each element of a `STRSXP` is a `CHARSXP`s, an immutable object that contains a pointer to C strings stored in a global pool. Use `STRING_ELT(x, i)` to extract the `CHARSXP`, and `CHAR(STRING_ELT(x, i))` to get the actual `const char*` string. Set values with `SET_STRING_ELT(x, i, value)`. Use `mkChar()` to turn a C string into a `CHARSXP`, and `mkString()` to turn a C string into a `STRSXP`. Use `mkChar()` to create strings to insert into an existing vector, use `mkString()` to create a new (length 1) vector.
...
Things are a little harder if you want to modify the strings in the vector. This is because you need to know a lot about string manipulation in C (which is hard, and harder to do right). For any problem that involves any kind of string modification, you're better off using Rcpp.

The elements of a list can be any other `SEXP`. However, this generally makes them hard to work with in C (you'll need lots of `switch` statements to deal with the possibilities). The accessor functions for lists are `VECTOR_ELT(x, i)` and `SET_VECTOR_ELT(x, i, value)`.

### Modifying inputs

You must be very careful when modifying function inputs. The following function has some rather unexpected behaviours:
...
There are a few helper functions that turn length one R vectors into a C scalars:
...
As of R 3.0.0, R vectors can have a length greater than 2^32. If you want your code to work with long vectors, this means you shouldn't write code like `int n = length(x)`. Instead use the `x_len_t` type and the `xlength()` function, and write `x_len_t = xlength(x)`.

## Pairlists {#c-pairlists}

In R code, there are few places where you need to care about the difference between a pairlist and a list (as described in [Pairlists](#pairlists)). Working with pairlists is much more important for C code because they are used for calls, unevaluated arguments, attributes and in `...`. In C, lists and pairlists differ primarily in how you access and name elements.

Unlike lists (`VECSXP`s), pairlists (`LISTSXP`s) have no way to index into an arbitrary location. Instead, R provides a set of helper functions that navigate along a linked list. The basic helpers are `CAR()`, which extracts the first element of the list, and `CDR()`, which extracts the rest of the list. These can be composed to get `CAAR()`, `CDAR()`, `CADDR()`, `CADDDR()` and so on. Corresponding to the getters, R provders setters `SETCAR()`, `SETCDR()` etc.
...
Alternatively, if we wanted to be more accepting of diverse inputs we could do:
...
* For other more esoteric objects: `isEnvironment()`, `isExpression()`, 
...
R code in the base package doesn't use `.Call()`. Instead it uses two special functions: `.Internal()` or `.Primitive()`. Finding the source code for these functions is a somewhat arduous process (you first need to look there C function name in `src/main/names.c` then search the R sources), but `pryr::show_c_source()` automates the process (using github code search).
...
This reveals the following C source code (slighted edited for clarity):
...
Interval and primitive functions have a somewhat different interface than `.Call()` functions. They all have four arguments: 
...
* `SEXP op`: a "offset pointer". This is used when multiple R functions use the
  same same C function. For example `do_logic()` implements `&`, `|` and `!`.
...
The final version below moves more of the coercion logic into an accompanying R function, as does some minor restructuring to make the code a little easier to understand. I also added a `PROTECT()`; this is probably missing in the original because the author knew that it would be safe.

```{r, cache = TRUE}
tabulate_ <- cfunction(c(bin = "SEXP", nbins = "SEXP"), '  
  int nb = asInteger(nbins);

  // Allocate vector for output - assumes that there are less than 2^32 bins,
  // and that each bin has less than 2^32 elements in it.
