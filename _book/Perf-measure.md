...
#>   <chr>      <bch:t> <bch:t> <bch:t> <bch:t>     <dbl> <bch:byt>
#> 1 sqrt(x)    630.9ns  1.04µs 841.9ns 463.8µs   962410.      848B
#> 2 x^0.5       13.5µs  15.3µs  14.4µs  86.3µs    65340.      848B
...
#> 1 sqrt(x)     630.9ns  841.9ns   962410.     0
#> 2 x^0.5        13.5µs   14.4µs    65340.     0
```
...
As with all microbenchmarks, pay careful attention to the units: here, each computation takes about 630 ns, 630 billionths of a second. To help calibrate the impact of a microbenchmark on run time, it's useful to think about how many times a function needs to run before it takes a second. If a microbenchmark takes:
...
The `sqrt()` function takes about 630 ns, or 0.63 µs, to compute the square roots of 100 numbers. That means if you repeated the operation a million times, it would take 0.63 s, and hence changing the way you compute the square root is unlikely to significantly affect real code. This is the reason you need to exercise care when generalising microbenchmarking results.
