...
Closely related to metaprogramming is __non-standard evaluation__, NSE for short. This term is commonly used to describe the behaviour of R functions, but there are two problems with this. Firstly, NSE is actually a property of a function's argument (or arguments), so talking about NSE functions is a little sloppy. Secondly, it's confusing to define something by what it's not (standard), so in this book I'll teach you a more precise vocabulary.
