...
We defined `$name`, `$age`, `$print`, and `$initialize`. As suggested by the name `.__enclos_env__` is an internal implementation detail that you shouldn't touch; we'll come to `$clone()` in Section \@ref(reference-semantics).
...
The finalize method will be run when the object is deleted (or more precisely, by by the first garbage collection after the object has been unbound from all names) or when R exits.
...
Forcing garbage collection here is useful, as it allows you to see the finalisation message.
