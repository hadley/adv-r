...
Unlike S3 and S4, with R6 a single function call creates both the class and its methods. In fact, there's actually only one function in R6 that you will ever use: `R6::R6Class()`. The following example shows the two most important arguments:
...
You use it to generate objects from the class by calling its `new()` method. Methods belong to R6 objects, so you use `$` to access it:
...
The `add()` method is called primarily for its side-effect of updating `self$sum`. For R6 methods called primarily their side-effects you should always return `self` invisible. This returns the "current" object and makes it possible to chain together multiple method calls.

```{r}
x$add(10)$add(10)$sum
```

This __method chaining__ is commonly used in encapsulated OO languages (like Python and JavaScript) to create fluent interfaces, in a similar way to the use of the pipe (`%>%`) in R.

### Important methods

There are two important methods that most classes will define: `$initialize()` and `$print()`. They are not required but will tend to make your class easier to use.

Similar to S4, `$initialize()` overrides the default behaviour of `$new()`. For example, the following code defines an R6 Person class, similar to the S4 equivalent in [S4].
...
Defining a print method allows you to control precisely how your R6 class is printed:
...
As in S3 print methods, you should always return the input object, which is `self` in this case.

This code illustrates an important aspect of R6. Methods are bound to individual objects. This means that the `hadley` object I created previously does not get this new method. Indeed, from the perspective of R6, there is no relationship between `hadley` and `hadley2`.

```{r}
hadley
```

Instead of providing `$print()`, you could instead provide `$format()`. This method should return a character vector.
...
One of the big differences between R6 and most other objects in R is that they have reference semantics. This is because they are built on top of environments:
