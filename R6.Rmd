...
* RC mingles variables and fields in the same stack of environments so that you
  ...
  (`self$field <- vaue`) with a prefix. The R6 approach is more verbose but is
...
The S3 hierarchy includes the the base "R6" class. This provides common behaviour, including an `print.R6()` method which calls `$print()` or `$format()`, as described above.
...
The the finalise method will be run when R exits, or by the first garbage collection after the object has been removed. Generally, this will happen when it happens, but it can occassionally be useful to force a run with an explicit call to `gc()`.
