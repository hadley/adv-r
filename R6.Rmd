...
Active fields make allow you to define components that look like fields from the outside, but are defined with functions, like methods, from the inside. For example, we can define a field called `x` that returns a different value every time you access it:
...
Activate fields are particularly useful in confunction with private fields, because they allow you to implement read-only fields, and fields that validate before setting. For example, the following code makes `age` readonly, and checks that `name` is a length 1 character vector before setting.
...
Note the structure of the active field functions: they take a single argument `value`, and branch on whether or not the argument is missing. If it's missing, the user is performing a get, and if it's not missing the user is performing a set.
...
Instead, if you want a copy, you'll need to explicitly `$clone()` the objecg.
...
The basic `$clone()` method only copies by value methods and data. That means that if you have any nested R6 object, it will be copied by reference. To also clone R6 objects contained in your object, you'll need to use `$clone(deep = TRUE)`. Note that this only clones R6 objects: if you have other fields with reference semantics (e.g. environments) you'll need to define your own `$clone()`.
