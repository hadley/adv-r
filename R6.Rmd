...
A change in one of the instance is not reflected inside another. That's usually the behaviour you want your objects to have. But sometime you don't. For that, you need to create the objects to be share in an environment inside the `shared` method in your `private` field. You can then access it with active binding. 
