...
However, most of the overhead is due to setting up the data mask so if you need to evaluate code repeatedly, it's a good idea to define the data mask once then reuse it:
...
This considerably reduces the overhead.
...
1.  Why do you not need to worry about ambiguous argument names with `...` in
    `arrange()`? Why is is a good idea to use `.` prefixes anyway?
...
Once you have identified the repeated code, you need to figure out what the argument you argument will be, and which of them are quoted.
...
We'll need three argument: a data frame (`df`), a grouping variable (`group_var`), and the variable to summarise (`summary_var`). Both the grouping and summary variable are quoted. Quoting is infectious, so in your function you'll also need to quote `group_var` and `summary_var`. This gives the following basic skeleton:
...
Next, we need to provide the implementation, unquoting the quoted variables in the correct places:
...
We'll focus on wrapping models because this is a common need, and illustrates the spectrum of challenges you'll need to overcome for any other base funtion.
