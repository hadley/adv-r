...
To finish up this chapter we're going to show how to create functions that use tidy evaluation (and all the advantages it brings) to wrap around base NSE functions. Here we'll focus on models, since since standard NSE doesn't provide unquoting tool. But you can use the same ideas with base graphics and any other function.

### Basics

Let's start with a simple wrapper around `lm()`. `lm()` is particularly challenging because it captures and prints the actual call. Ideally we want this to be useful.

(I'm going to change the order of the arguments to bring data first because I can't help but make the interface easier to pipe with.)

```{r}
lm2 <- function(data, formula) {
  lm(formula, data = data)
}
lm2(mtcars, mpg ~ disp)
```
This works but yields an unappealing model call.

We can avoid that:

```{r}
lm3 <- function(data, formula) {
  data <- enexpr(data)
...
Note that we have to capture expressions here rather than quosures, because `lm()` does not use tidy evaluation.

This technique works even with arguments that have non-standard evaluation, like `subset()`:

```{r}
lm4 <- function(data, formula, subset = NULL) {
  data <- enexpr(data)
  formula <- enexpr(formula)
...
coef(lm4(mtcars, mpg ~ disp))
coef(lm4(mtcars, mpg ~ disp, subset = cyl == 4))
```
...
Note that you'll need to unquote any objects that you create in the function. 

```{r}
lm5 <- function(data, formula, subset = NULL) {
  subset <- enexpr(subset)
  lm_call <- expr(lm(formula, data = data, subset = !!subset))

  env <- child_env(caller_env(), data = data, formula = formula)
...
```{r}
lm6 <- function(data, formula, subset = NULL) {
  data <- enexpr(data)
...
lm4(mtcars, mpg ~ disp)$call
lm6(mtcars, mpg ~ disp)$call
```
