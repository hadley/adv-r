...
To finish up this chapter we're going to show how to create functions wrap around base NSE functions. Here we'll focus on models, since since standard NSE doesn't provide unquoting tool. But you can use the same ideas with base graphics and any other function. Because of the semantics of base R functions, it's not possible to generate a 100% correct solution. So I wouldn't recommend including these in a package. But generally, your own code is unlikely to execute every issue that tidy eval protects you from so these are often enough to reduce duplication in your own data analysis code. These wrappers are end points, not composable - you can't rewrap easily.
...
This wrapper is not useful but it reveals a challenge of working with `lm()`: it captures and prints the actual call:

```{r}
lm2(mpg ~ disp, mtcars)$call
```

Ideally we want the call to be useful, i.e. we don't want our wrapper to be worse than `lm()` in some way. To overcome this problem, we need to capture the arguments, create the call to `lm()` using unquoting, then evaluate that call:
...
Note that we need to manually supply an evaluation environment, `caller_env()`. We'll discuss that in more detail shortly.

This function has one small advantage over `lm()`: we can use quasiquotation.

```{r}
resp <- expr(mpg)
lm3(!!resp ~ disp, mtcars)
```

Note that this technique works for all the arguments in works even with arguments that have non-standard evaluation, like `subset()`:
...
1. We use `maybe_missing()` in `expr()`: if we don't do that the 
   weirdness of the missing argument crops up and generates an error
...
Note that we need to manually supply the evaluation environment. We can't use quosures because base R functions don't understand them. This means that there's no easy way to wrap `lm3()` inside another function. 

You could make one small improvement:

```{r}
lm3a <- function(data, formula, .env = caller_env()) {
  data <- enexpr(data)
  formula <- enexpr(formula)
  
  lm_call <- expr(lm(!!formula, data = !!data))
  eval_bare(lm_call, .env)
}
```

Another option for modelling functions is to use the environment captured by the formula:

```{r}
lm3b <- function(data, formula) {
  data <- enexpr(data)
  formula <- enexpr(formula)
  
  lm_call <- expr(lm(!!formula, data = !!data))
  eval_bare(lm_call, f_env(formula))
}
```

It's not clear whether it's worth doing either of these options. The model is fundamentally wrong because it assumes all arguments need the same environment which is not true in general. Probably best to not even try to make these functions callable anything other than interactively.

### Mixing user supplied and function created arguments
...
Why doesn't this code work? We're evaluating the call in caller environment, where `boot_data` does not exist. In this case we get a clear error, but if `boot_data` also existed it might silently succeed. There are two basic way to overcome this challenge:

1.  Unquote the data frame into the call. This means that no look up has
    to occur, but has all the problems of inlining expressions. In particular
    this isn't great for any function that captures the call:
...
1.  Another way to way to write `boot_lm()` would be to include the
    boostrapping expression (`data[sample(nrow(data), replace = TRUE), , drop = FALSE]`) in to the data argument. Implement that approach.!
