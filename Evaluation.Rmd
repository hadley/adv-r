...
You'll need to be familiar with the content of Chapter \@ref(expressions) and Chapter \@ref(quotation), as well as the basics of environments as covered in Section \@ref(env-basics). 
...
Now that you understand tidy evaluation, it's time to come back to the approaches taken by base R. Here we'll explore two approaches that you might see in base R. 

* The first approach is used by `subset()`, and I'll use it as a motivation 
  to explore the warnings in the `subset()` documentation. 

* The second approach is used by `write.csv()` and `lm()`, and I'll use it 
  as motivation to show how quasiquotation and (regular) evalation can help
  you write wrappers around base functions that use NSE.

### `substitute()` and `subset()`

We can extract out the core of the `subset()` implementation in a very simlarly form to the tidy evaluation version I showed in Section \@ref(subset). (Repeated below for ease of comparison).
...
Implementing `subset()` captures the input expression with `substitute()` and then evaluates a in a data mask using the 3-argument form of `eval()`. 
...
You might wonder if all this rigamorale is worth it when you can just use `[`. Firstly, it seems unappealing to have functions that can only be used safely in an interactive context. That would mean that every interactive function needs to be paired with function suitable for programming. 
