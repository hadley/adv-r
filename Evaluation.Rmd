...
These functions work because internally R represents function arguments with a special type of object called a __promise__. A promise captures the expression needed to compute the value and the environment in which to compute it. \index{promises}

Even though a promise is an object you can't work directly with it in R, you can't even tell if an object is a promise or not. But there is no need to. You can just treat a promise as a value. The reason is that if you use an a object that just happens to be a promise it will evaluate the expression it stored in the environment it stored and produce a value.  

You can't make a promise directly but you can make an object that is very similar, a quosure. One funcion that does this is enquo and here is an example of using it:
...
str(e)
#> <quosure: frame 0x7faab02ca720>
#> ~a + b
```
The str() function shows you the internal structure of an object and here we can see that the e object which, was created by enquo, contains an environment and an expression. Those two pieces of information are all you need to execute an expression the way that a promise does.
