...
Now that you understand tidy evaluation, it's time to come back to the alternatives taken by base R. Here we'll explore two approaches that you might see in base R. The first approach is used by `subset()`, and I'll use it as a motivation to explore the warnings in the `subset()` documentation. The second approach is used by `write.csv()` and `lm()`, and I'll use it as motivation to show how tidy evaluation can be useful to create wrappers.
