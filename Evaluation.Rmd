...
In the previous section, we used `eval_tidy()` in a very similar way to  `eval()`. The real power of `eval_tidy()` comes with the second argument: `data`. This lets you set up a __data mask__, where variables in the environment are potentially masked by variables in a data frame. This allows you to mingle variables from the environment and variables from a data frame: 
...
How does this work? Unlike environments, data frames don't have parents, so we can effectively turn it into an environment using the environment of the quosure as its parent. You can think of the code above as being equivalent to this, although the reality is more complex:
...
As well as supporting quosures, `eval_tidy()` also provides some pronouns, which you'll learn about shortly.
...
threshold_var1(df, x, 8)
```

`threshold_var1()` uses `$`. This forces us to the prefix form of `$`, because neither `.data$!!var` nor `.data$(!!var)` is valid R syntax. In `threshold_var2()` we use `[[`, converting the symbol to a string.
...
If you need to wrap a function that quasi-quotes one of its arguments, it's simple to wrap. You just need to quote and unquote. Take this repeated code:
...
`subset()` is a useful tool, but still simple enough to submit to analysis. The documentation of `subset()` includes the following warning:
...
Remember we picked `subset()` because it's easy to evaluate, not particularly featureful.
