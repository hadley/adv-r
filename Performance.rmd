...
Each additional environment between `f()` and the base environment makes the function slower by about 50ns.  Most other languages have many more built in constants that you can't override. This means that they always know exactly what `+`, `-`, `{` and `(` and they don't need to waste time repeatedly looking up their definitions. The cost of that decision is it means make writing the interpreter a little harder (because there are more special cases), and the language isn't quite as flexible. It would be possible to change this property of the R-language, and provide more built-in constants. Such a change would be unlikely to affect much code (it's a bad idea to override `{` or `(`!), but it would require work by the R core team.
...
In most other programming languages there is little overhead for adding extra arguments. Many compiled languages will even warn you if arguments are never used, and automatically remove them from the function.
