...
R is not a fast computer language. This is not an accident: R has been thoughtfully designed to make it easier for you to solve data analysis and statistics challenges, not to make your computer's life easier. While R is slow compared to other programming languages, it certainly can be fast enough for most purposes. The goal of this part of the book is to give you a deeper understanding into R's performance characteristics. You'll learn where R is fast, where it's slow, and how you can improve performance when it becomes a problem.
...
`microbenchmark()` times each expression 100 times, it doesn't time 100 expressions and return only the average time. This is important because it can randomise the order of the timings controlling for systematic variability. While the default print method only displays a five number summary, all individual timings are stored in the output object. You can visualise the variability by using the `boxplot()` method, or if ggplot2 is loaded, with `autoplot()`.
