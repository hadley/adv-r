...
* Section \@ref(quoting) gives you tools to quote expressions, whether
...
Quasiquotation was first used in a programming language, LISP, in the mid-1970s [@bawden-1999]. LISP has one quoting function `` ` ``, and uses `,` for unquoting. Most languages with a LISP heritage behave similarly. For example, racket (`` ` `` and `@`), clojure (`` ` `` and `~`), and julia (`:` and `@`) all have quasiquotation tools that differ only slightly from LISP's. These languages have a single quoting function and you must call it explicitly. 
...
<!-- GVW: replaced "quotating" with "quoting" below. -->
