...
Talking about whether an argument is quoted or evaluated is a more precise way of stating whether or not a function uses NSE. I will sometimes use "quoting function" as short-hand for a "function that quotes one or more arguments", but generally, I'll refer to quoted arguments since that is the level at which the different occurs.
...
To make these ideas concrete, this section contains a few smaller case studies that show how quasiquotation can be used in practice to solve real problems. We'll also see a couple of examples combining quasiquotation with purrr to generate complex expressions programmatically. These case studies will use purrr to iterate over lists of expressions to transform them in various ways, and dplyr to motivate some of the challenges.
...
It's often useful to quote one or more arguments and then mildly manipulate them with purrr. For example, imagine that you want to extend `dplyr::select()` to return two data frames: one with the variables you selected, and one with the variables that remain. (This problem was inspired by <https://stackoverflow.com/questions/46828296/>.) 

There are plenty of ways to attack this problem, but one way is to take advantage of `select()'s` ability to negate column selection expression in order to remove those columns. We can capture the inputs with quasiquotation, then invert each selection call by negating it. We start by practice interactively with a list of variables created with `exprs()`:
...
One useful tool missing from base R is the ability to extract an array slice given a dimension to slice along and an index into that dimension. For example, we'd like to write `slice(x, 2, 1)` to extract the first slice from dimension two, which you can write as `x[, 1, ]`. How can generate this call?

The key idea is to generate call to `[` where most of the arguments are missing. To do so, we'll create a list of missing arguments, then unquote-splice them into a call:
...
We can then wrap this into a function, using subset-assignment to replace the index in the desired position:
...
A real `slice()` would evaluate the generated call, as described in the next chapter, but here I think it's more illuminating to see the code that's generated, as that's the hard part of the challenge.

### Creating functions

Another powerful function to use in combination with unquoting is `rlang::new_function()`: it allows us to create a function by supplying the arguments, body, and environment:
...
One application is to create functions that work like `graphics::curve()`. `curve()` allows you to plot a mathematical function without creating an explicit R function.

```{r curve-demo, fig.width = 3.5, fig.height = 2.5, small_mar = TRUE}
curve(sin(exp(4 * x)), n = 1000)
```

Here `x` is a pronoun. `x` doesn't represent a single concrete value, but is instead a placeholder that varies over the range of the plot. Functions that use a pronoun are known as anaphoric functions[^anaphora]. One way to implement `curve()` is to turn the expression into a function with a single argument, then call that function:
...
You can also `new_function()` as an alternative way of implementing simple function factories and function operators. The primary advantage is that the generated functions are immediately readable:
...
1.  Re-implement the Box-Cox transform using unquoting and `new_function()`:
...
There's another way to solve these problems if we look at them in a different way: we need some way to splice lists and to unquote argument names. For example, the tidyverse equivalents of `rbind()` and `data.frame()`, `dplyr::bind_rows()` and `tibble::tibble()`, do not require an additional helper. Instead, they use the same syntax as unquoting:
...
When used in this context, the behaviour of `!!!` is known as spatting in Ruby, Go, PHP, and Julia. It is closely related to `*args` (star-args) and `**kwarg` (star-star-kwargs) in Python, which are sometimes called argument unpacking. LISP and derivatives (like clojure) work like R with a function that behaves like `do.call()` called `apply()`.
...
Both `dplyr::bind_rows()` and `tibble::tibble()` are powered by `rlang::list2(...)`. This function is very similar to `list(...)`, but it understands `!!!` and `!!`. If you want to take advantage of this behaviour in your own function, all you need to do is use `list2()` in your own code.
...
Note that we call the first argument `.x`: whenever you use `...` to take arbitrary data, it's good practice to give the other argument names a `.` prefix. This eliminates any ambiguity about who owns the argument.

`list2()` provides one other handy feature: by default it will ignore any empty argument at the end. This is useful in functions like `tibble::tibble()` because it means that you can easily change the order of variables without worrying about the final comma:
...
Closely related to `invoke()` is `rlang::call2()`:

```{r}
call2("mean", 1:10, !!arg_name := arg_val)
```

The chief advantage of `call2()` over `expr()` is that can use `:=`.

### Other approaches

Apart from `rlang::list2()` there are several other techniques used to overcome the difficulties described above. One technique is is to take `...` but also accept a single unnamed argument that is a list, making `f(list(x, y, z))` equivalent to `f(x, y, z)`. The implementation looks something like:
...
Base functions that use this technique include `interaction()`, `expand.grid()`, `options()`, and `par()`. Since these functions take either a list, or a `...`, but not both, they are slightly less flexible than functions powered by `list2()`.
...
I found this technique particular compelling so you can see it used throughout the tidyverse. Now, however, `rlang::list2()` dots solves more problems, more elegantly, by using the ideas from tidy eval so the tidyverse is slowly migrating to that style.
...
Occassionally it is useful to unquote a missing argument, but the naive approach doesn't work:
...
You'll need to use the `maybe_missing()` helper:
