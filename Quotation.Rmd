...
Quasiquotation is a key component of tidy evaluation, a system used throughout the tidyverse, and elsewhere. As a system, tidy evaluation is important because it provides a consistent set of tools that you can learn once and apply in many places.

### Outline {-}

### Prerequisites {-}

Make sure you're familiar with the tree structure of expressions as described in Section \@ref(expressions). You'll also need the rlang package, and purrr.
...
Base R has one function that implements quasiquotation: `bquote()`, which uses `.()` for unquoting. Unfortunately `bquote()` has had little impact: it not used by any other function in base R. There are three reasons:
...
Instead, we need another function: `enexpr()`. This captures what the user supplies to the function by looking at the internal promise object that powers lazy evaluation (see Section \@ref(promises) for more details).
...
Sometimes you only want to allow the user to specify a variable name, not an arbtirary expression. In this case, you can use `ensym()` or `ensyms()`. are variants of enexpr() and enexprs() that check the captured expression is either a string (which they convert to symbol) or a symbol. If anything else is supplied they throw an error.
...
`substitute()` provides a sort of automatic unquoting for any symbol that is bound to a value. However, this can make code hard to read, because, taken out of context, you can't tell if the goal of `substitute(x + y)` is to replace `x`, or, `y`, or both. If you do want to use `substitute()` in this way, I recommend that you use the 2nd argument to make your goal clear:
...
There the equivalent to `enexprs()` is an undocumented feature of `substitute()`[^peter-meilstrup]: you can pretend `...` is the name of a function to capture all arguments in `...`:
...
This leads to two families of quoting functions:
...
1.  The documentation for `substitute()` says:
    ...
    >   or explicitly created using delayedAssign(), the expression slot of
        the promise replaces the symbol. 
    > * If it is an ordinary variable, its value is substituted, 
    >   __unless__ env is .GlobalEnv in which case the symbol is left 
    >   unchanged.
...
Quoting has two inverses:

* Unquoting, the topic of this section, happens inside calls to `expr()`,
  i.e. `expr(!!x)` is eqvuivalent to `x`.
  
* Evaluation, the topic of the next chapter, happens outside calls to `expr()`,
  i.e. `eval(expr(x))` is eqvuivalent to `x`.

Quoting functions that also support unquoting are called  __quasiquoting__ functions. All quoting functons in rlang, and hence the tidyverse, are quasiquoting functions. Only one function in base R is quasiquoting (`bquote()`), so functions have to take other approaches to achieve the same goal.

### Unquoting one argument

Unquoting is an extremely useful way of building complex expression from simple pieces, and forms a natural complement to `rlang::call2()` covered in Section \@ref(call2).

All quoting functions in rlang (`expr()`, `enexpr()`, and friends) support unquoting with `!!` (called "unquote", and pronounced bang-bang) . They both replace nodes in the AST. 

`!!` is a one-to-one replacement. It takes a single expression and inlines the AST at the location of the `!!`. 
...
Importantly, `!!` respects operator precedence:
...
If we simply pasted the expressions together, we'd end up with `x + 1 / x + 2` which has a very different AST:
...
`!!` is most commonly used to replace the arguments to a function, but you can also use it to replace the function to be called. The only trick here is operator precedence: `expr(!!f(x, y))` inserts the result of `f(x, y)`, so you need to use `expr((!!f)(x, y))`.
...
Very occasionally it is useful to unquote a missing argument (as described in Section \@ref(empty-symbol)), but the naive approach doesn't work:
...
`!!` inserts a single argument (or function). You can use `!!!` (called "unquote-splice", and pronounced bang-bang-bang) to insert many arguments. Where `!!` is a one-to-one replace, `!!!` is a one-to-many replacement. It takes a list of expressions and inserts them at the location of the `!!!`:
...
`!!` and `!!!` have special behaviour inside all quoting functions powered by rlang, where they behave like their own operators. You might wonder why rlang does not use a regular function call. Indeed, early versions of rlang provided `UQ()` and `UQS()` as alternatives to `!!` and `!!!`. However, these looked like regular function calls, rather than special syntactic operators, and evoked a misleading mental model, which made them harder to use correctly. In particular, function calls only happen (lazily) at evaluation-time; unquoting always happens at quotation-time. We adopted `!!` and `!!!` as the best compromise: 
...
Once you have this expression, you could evaluate it with new data (which you'll learn about in Chapter \@ref(evaluation)), or turn it into a function (which you'll learn about in Section \@ref(new-function)). 
...
One use of `new_function()` is as an alternative to function factories that only have simple arguments. The primary advantage to `new_function()` over a function factory is the source code is readable:
...
Another application is for functions functions that work like `graphics::curve()`. `curve()` allows you to plot a mathematical expression, without creating a function:

```{r curve-demo, fig.width = 3.5, fig.height = 2.5, small_mar = TRUE}
curve(sin(exp(4 * x)), n = 1000)
```

Here `x` is a pronoun. As with `.` in pipelines and `.x` and `.y` in purrr functionals, `x` doesn't represent a single concrete value, but is instead a placeholder that varies over the range of the plot. One way to implement `curve()` is to turn the expression into a function with a single argument, `x`, then call that function:
...
1.  In the linear-model example, we could replace the `expr()` in 
    `reduce(summands, ~ expr(!!.x + !!.y))` with the `call2()`:
...
One useful application of `list2()` is `exec()`, which is a wrapper around `list2()` and `do.call()`:
