...
Evaluating works with the entire expression. We can selectively choose to evaluate, or __unquote__, inside an expression with either `!!` called unquote, and pronounced bang-bang, or `!!!` called unquote-splice, and pronounced bang-bang-bang. They both replace nodes in the AST. `!!` is a one-to-one replacement. It takes a single expression and inlines the AST at the location of the `!!`. 
...
There's one final component to quasiquotation: `:=`, called define, and pronounced colon-equals. This is needed because in function arguments the LHS of `=` is always quoted:
...
Instead we can use `:=`. `:=` is a weird quirk of R: it's a built in binary operator that doesn't have a default implementation. data.table and rlang use it in a similar way to get more flexibility. (The implementations are compatible.)
