...
One downside of quoting functions is that they are not __referentially transparent__. A function is referentially transparent if you can replace its arguments with their values and its behaviour doesn't change. For example, if a function, `f()`, is referentially transparent and both `x` and `y` are 10, then `f(x)`, `f(y)`, and `f(10)` will all return the same result. This is a really important property because it's what allows you to reduce duplication in code by extracting a repeated pattern into a function.
...
(And reducing the duplication is a good idea because when you realise that your rescaling technique doesn't handle missing values gracefully, you only have one place to fix it)

This works because `rescale01()` is referrentially transparent: only the values of the inputs matter, not how they are computed. Quoting functions are not referentially transparent. For example, if you notice take this repeated code:
...
```{r}
scatterplot <- function(df, x, y) ggplot(df, aes(x, y)) + geom_point()
```

Because regardless of how you call it, it `aes()` will always try to plot `x` vs `y`. However, quasiquotation gives us a standard way to wrap these functions: you quote and then unquote:
...
(NOTE: This requires the very latest development version of ggplot2)

This is a powerful pattern that allows you to wrap functions that quoting one or more arguments, providing a standard way to work around the referentical integrity absent in quoting functions.
