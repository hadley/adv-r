...
It's challenging to see the AST of code like `!!x + !!y` because  `ast()` does unquoting. We can work around this by using the base equivalent of `expr()` that doesn't do unquoting: `quote()`. 
...
Unfortunately while this is R's operator precedence, it is not convenient unquoting - in the above expression you want to unquote `x` and `y`, but R's default rules interpret this as unquoting `x + y`, i.e. `!!(x + y)`. This is unlikely to be useful so rlang bends R's usual operator precedence rules to work the way you might expect. 

The root cause of this problem is that the operator precedence for `!` is suprisingly low: unlike it unary `+` and `-`, it has lower precedence that than of the binary algebraic and logical operators. Most of the time this doesn't matter: expressions that mix `!` and binary operators, like `!x + y` or !x > y`, are unusual. For this reason, we thought it ok to bend the usual rules here.

The other downside of quasiquotation is that you can no longer double negate a value, `!!x`. In the unusual case you both need this and want to do quasiquotation, just add a pair of parentheses `!(!x)`.
...
(Neither `purrr::invoke()` nor `rlang::invoke()` currently use this definition)
...
make_model <- function(resp, preds) {
  pred_sum <- purrr::reduce(preds, function(x, y) expr(!!x + !!y))
