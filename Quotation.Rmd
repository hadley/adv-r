...
However, `bquote()` is not used by any other function in base R. Instead functions that quote an arugment must use some other technique to allow indirect specification (none of which are unquoting). There are four basic forms seen in base R:
...
Using unquoting, it is easy to create non-standard ASTs, i.e. ASTs that contain components that are not constants, symbols, or calls. (It is also possible to create non-standard ASTs by directly manipulating the underlying objects, but it's harder to do so accidentally) These are valid, and occassionally useful, but their correct use is beyond the scope of this book. However, it's important to learn about them because they can be printed in misleading ways. 
...
Reducing the duplication here is a very good idea, because when you realise that your rescaling technique doesn't handle missing values gracefully, you only have one place to fix it. It is easy to create `rescale01()` because `min()`, `max()`, and `-` , are regular functions: they don't quote any of their argumetns.
...
Because regardless of how you call it, it `aes(x, y)` will always try to plot `x` vs `y`. However, because (as of very recently) ggplot2 support quasiquotation, there's a standard way to wrap these functions: you quote and then unquote:
...
# scatterplot(mtcars, cyl, vs)
```

This is a powerful pattern that allows you to wrap functions that quote one or more arguments.
...
A little `map()` is often usefully combined with `enexprs(...)`. For example, imagine that you want to extend `dplyr::select()` to return two data frames: one with the variables you selected, and one with the variables that remain. (This problem was inspired by <https://stackoverflow.com/questions/46828296/>.) There are a plenty of ways to attack this problem, but one way is to use quasiquotation, then invert each selection call by negating it. 
...
You can use this idea to generate your wrappers around dplyr functions. For example, imagine we want to group by one variable and compute the mean of another. We use `enexpr()` to capture the expression, and then unquoting to selectively evaluate in the dplyr call:
...
We'll come back to this case study in more detail in the next chapter. 

But for now, imagine that we want to extend this function to use `...`. There are three possible ways we could use `...` it:
...
Implementing each one of these is relatively straightforward, but what if we want to be able to group by multiple variables, summarise multiple variables, and pass extra args on to `mean()`. T
