...
You'll most often see `substitute()` used to capture unevaluated arguments. However, as well as quoting, `substitute()` also handles substitution. If you give it an expression, rather than a symbol, it will substitute in the values of symbols defined in the current environment. 
...
* Is it supplied by the developer of the code or the user of the code?
  Is it fixed (supplied in the body of the function) or varying (supplied
  via an argument)?
  
* Do you want to capture a single expression or multiple expressions?

This leads to 2 $\times$ 2 tables of functions for rlang, Table \@ref(tab:quoting-rlang), and base R, Table \@ref(tab:quoting-base).
...
<!-- Here is not clear when is "next" -->
...
In that expresssion, `x` is a pronoun: it doesn't represent a single concrete value, but is instead a placeholder that varies over the range of the plot. One way to implement `curve()` is to turn that expression into a function with a single argument, `x`, then call that function:
